/* Namo Chemical Q-MES Enterprise Inventory auto mode v9, 2026-08-20 */
...
function InventoryTransactionDetailModal({tx,onClose}){
  ...
  const barcodeValue=txBarcodeValue(tx);
  React.useEffect(()=>{
    if(!barcodeRef.current||!window.JsBarcode)return;
    window.JsBarcode(barcodeRef.current,barcodeValue,{format:'CODE128',displayValue:true,height:64,margin:8,fontSize:13,lineColor:'#0f172a'});
  },[barcodeValue]);
  React.useEffect(()=>{
  ...
  const printBarcode=()=>{
    ...
    doc.title=`${tx.item_code||txMaterialName(tx)} ${tx.lot_no||''} 바코드`;
    const wrap=doc.createElement('main');
    wrap.style.cssText='font-family:Arial,sans-serif;width:620px;margin:32px auto;color:#0f172a;text-align:center;';
    const title=doc.createElement('h1');
    title.textContent=txMaterialName(tx);
    title.style.cssText='font-size:24px;margin:0 0 8px;';
    const meta=doc.createElement('p');
    meta.textContent=`LOT ${tx.lot_no||'-'} · ${txDirectionLabel(tx)}`;
    meta.style.cssText='font-size:15px;margin:0 0 20px;';
    const barcode=barcodeRef.current.cloneNode(true);
    barcode.style.maxWidth='100%';
    wrap.append(title,meta,barcode);
    doc.body.appendChild(wrap);
    printWindow.focus();
    printWindow.print();
    printWindow.close();
  };
  return <div className="inv-tx-detail-overlay" role="dialog" aria-modal="true" aria-labelledby="inv-tx-detail-title" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><section className="inv-tx-detail-sheet"><div className="inv-tx-detail-head"><div><span>INVENTORY TRANSACTION</span><h3 id="inv-tx-detail-title">입출고 처리 상세</h3></div><button type="button" onClick={onClose} aria-label="닫기">×</button></div><div className="inv-tx-detail-status"><b>처리 완료</b><span>{new Date(tx.created_at).toLocaleString('ko-KR')}</span></div><dl className="inv-tx-detail-grid"><div><dt>문서번호</dt><dd>{txDocumentNo(tx)}</dd></div><div><dt>구분</dt><dd>{INV_TYPE_LABEL[tx.transaction_type]||tx.transaction_type}</dd></div><div><dt>원료명</dt><dd>{txMaterialName(tx)}</dd></div><div><dt>원료코드</dt><dd>{tx.item_code||'-'}</dd></div><div><dt>LOT</dt><dd>{tx.lot_no||'-'}</dd></div><div><dt>수량</dt><dd>{invNum(tx.quantity)} {tx.unit}</dd></div><div className="wide"><dt>이동 방향</dt><dd>{txDirectionLabel(tx)}</dd></div><div><dt>작업자</dt><dd>{txOperatorLabel(tx)}</dd></div><div><dt>비고</dt><dd>{txDisplayReference(tx)}</dd></div></dl><div className="inv-tx-barcode"><div><b>원료·LOT·위치 바코드</b><span>ERP 연동용 CODE128</span></div><svg ref={barcodeRef} aria-label={`재고 바코드 ${barcodeValue}`}></svg>{!window.JsBarcode&&<code>{barcodeValue}</code>}</div><div className="inv-tx-detail-actions"><button type="button" onClick={onClose}>닫기</button><button type="button" className="primary" onClick={printBarcode}>바코드 인쇄</button></div></section></div>;
}
