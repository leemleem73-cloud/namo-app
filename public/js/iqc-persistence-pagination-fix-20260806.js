(function(){
  "use strict";
  if(window.__QMES_IQC_PERSISTENCE_PAGINATION_FIX_V5__) return;
  window.__QMES_IQC_PERSISTENCE_PAGINATION_FIX_V5__=true;
  ...
  style.id="qmes-iqc-table-native-fix-v5";
  style.textContent=`
  ...
  function recordForRow(row,map){
    const cells=[...row.cells];
    const lot=clean(cells[map.lot]?.textContent);
    const name=clean(cells[map.name]?.textContent);
    const supplier=clean(cells[map.supplier]?.textContent);
    const records=Array.isArray(window.DB?.iqc)?window.DB.iqc:[];
    return records.find(r=>clean(r.lot)===lot&&clean(r.name)===name&&clean(r.supplier||"-")===supplier)
      ||records.find(r=>clean(r.lot)===lot&&clean(r.name)===name)
      ||records.find(r=>clean(r.lot)===lot);
  }
  ...
  function normalizeLedger(table){
    ...
    [...tbody.rows].forEach(row=>{
      ...
      const record=recordForRow(row,map);
      if(record&&row.cells[map.date])row.cells[map.date].textContent=clean(record.recv||record.inspectedAt||"-").slice(0,10)||"-";
    });
  ...
  window.addEventListener("mousedown",handlePager,true);
