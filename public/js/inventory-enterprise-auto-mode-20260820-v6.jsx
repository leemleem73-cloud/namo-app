/* Namo Chemical Q-MES Enterprise Inventory auto mode v6, 2026-08-20 */
...
function TxTable({rows}){return <table><thead><tr><th>일시</th><th>구분</th><th>품목</th><th>LOT</th><th>수량</th><th>이동 방향</th><th>작업지시/처리내용</th><th>작업자</th></tr></thead><tbody>{rows.map(tx=><tr key={tx.id}><td>{new Date(tx.created_at).toLocaleString('ko-KR')}</td><td>{INV_TYPE_LABEL[tx.transaction_type]||tx.transaction_type}</td><td>{tx.item_code}</td><td>{tx.lot_no}</td><td className="num">{invNum(tx.quantity)} {tx.unit}</td><td>{txDirectionLabel(tx)}</td><td>{txDisplayReference(tx)}</td><td>{tx.operator_name}</td></tr>)}</tbody></table>}
