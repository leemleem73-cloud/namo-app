    ...
    <div className="qmes-iqc-page flex flex-col gap-4">
      ...
      <div className="qmes-iqc-ledger-section">
        <Panel title="수입검사 관리대장" className="qmes-iqc-ledger-panel">
          <div className="qmes-unified-filter-grid" style={{display:"grid",gridTemplateColumns:"350px 150px 150px 135px",columnGap:"12px",alignItems:"end",justifyContent:"start",width:"max-content",maxWidth:"100%"}}>
            ...
            <div className="qmes-filter-select-field" style={{width:"150px",margin:0,minWidth:0}}>
              <span>연도</span>
          ...
          <div className="qmes-iqc-ledger-container">
            <table className="qmes-iqc-ledger-table qmes-iqc-ledger-compact w-full text-[10px]">
              <colgroup>
              ...
              <thead><tr className="text-[10px] text-slate-400 border-b border-slate-800">
                  ...
                  <tr key={r.inNo || r.lot}>
                    ...
                    <td className="qmes-iqc-manage-cell">
                      <div className="qmes-iqc-manage-inline">
                        <button type="button" onClick={()=>setViewingIqc(r)} title="성적서 미리보기 및 출력" className="qmes-iqc-action-btn qmes-iqc-action-print qmes-unified-inspection-action"><Printer size={12} /> 출력</button>
                        <button type="button" onClick={()=>setViewingLabel(r)} title="라벨" className="qmes-iqc-action-btn qmes-iqc-action-label qmes-unified-inspection-action">라벨</button>
                        <button type="button" onClick={()=>editIqc(r)} title="수정" className="qmes-iqc-action-btn qmes-iqc-action-edit qmes-unified-inspection-action">수정</button>
                        <button type="button" onClick={()=>deleteIqc(r)} title="삭제" className="qmes-iqc-action-btn qmes-iqc-action-delete qmes-unified-inspection-action">삭제</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {iqcModalOpen && (
        <div className="qmes-modal-backdrop qmes-iqc-modal-backdrop" onMouseDown={(e)=>{ if(e.target===e.currentTarget){ setIqcModalOpen(false); setEditingInNo(null); setTried(false); } }}>
          <div className="qmes-iqc-modal" role="dialog" aria-modal="true" aria-label={editingInNo ? "수입검사 수정" : "수입검사 등록"}>
            <div className="qmes-iqc-modal-head">
              <div><span>{editingInNo ? "EDIT INCOMING INSPECTION" : "INCOMING INSPECTION"}</span><strong>{editingInNo ? "수입검사 수정" : "수입검사 신규등록"}</strong></div>
