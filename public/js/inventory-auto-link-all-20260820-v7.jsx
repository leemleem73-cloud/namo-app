/* QMES inventory full auto-link v7 - IQC packaging/barcode metadata, 2026-08-20 */
(function(){
  'use strict';
  if(window.__QMES_INV_AUTO_LINK_ALL_V7__) return;
  window.__QMES_INV_AUTO_LINK_ALL_V7__=true;
  ...
  async function migrateLegacyLocationStock(stockRows,refs){
    const legacy=(stockRows||[]).filter(row=>{
      const source=upper(row?.location_code||row?.locationCode);
      return (source==='RM'||source==='UNASSIGNED')&&num(row?.quantity)>0;
    });
    for(const row of legacy){
      const itemCode=text(row?.item_code||row?.itemCode)||'RM';
      const itemName=text(row?.item_name||row?.itemName)||itemCode;
      const lotNo=upper(row?.lot_no||row?.lotNo);
      const status=upper(row?.quality_status||row?.qualityStatus)||'AVAILABLE';
      const quantity=num(row?.quantity);
      const fromLocation=upper(row?.location_code||row?.locationCode);
      if(!lotNo||quantity<=0) continue;
      const toLocation=locationMap.canonicalRawLocation(`${itemCode} ${itemName}`);
      if(toLocation===fromLocation) continue;
      const ref=`PHOTO-RACK-MIGRATION-20260820:${fromLocation}:${upper(itemCode)}:${lotNo}:${status}`;
      try{
        await post({transactionType:'MOVE',itemCode,itemName,category:'RM',lotNo,quantity,unit:text(row?.unit)||'kg',fromLocation,toLocation,fromStatus:status,toStatus:status,workOrderNo:'',productionLot:'',referenceNo:ref,supplier:'',receivedAt:'',expiryDate:'',reason:'사진 기준 원료 실제 위치 정정',remark:toLocation==='UNASSIGNED'?'사진 미확인 원료 · 위치확인':'창고 사진 표찰 기준 실제 랙 이전'});
        refs.add(ref);
        row.location_code=toLocation;
        row.locationCode=toLocation;
      }catch(e){
        console.warn('기존 원료 위치 자동정정 실패',itemCode,lotNo,e.message);
      }
    }
  }
  ...
  async function syncInventory(){
    ...
    try{
      ...
      for(const row of iqcRows()){
        ...
        const unitWeight=num(row?.unitWeight??row?.unit_weight);
  ...
  function installHint(){
    document.querySelectorAll('.inv-shell .inv-title-row').forEach(row=>{
      let hint=row.querySelector('[data-qmes-inv-auto-v6="1"]');
      if(!hint){hint=document.createElement('div');hint.dataset.qmesInvAutoV6='1';hint.style.cssText='font-size:12px;color:#64748b;margin-top:6px;font-weight:700;';row.querySelector('div')?.appendChild(hint);}
      hint.textContent='실제 랙 자동연동 · PAI A-1-1 · 기존 위치 자동정정 · 원재료 A구역 · 완제품 B구역';
    });
  }
