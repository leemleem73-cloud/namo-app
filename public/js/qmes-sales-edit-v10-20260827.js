/* NAMO QMES - Sales edit V10 - 2026-08-27
 * Single owner for Sales edit. Edited SO number is stored by Work Order key.
 */
(function(){
  "use strict";
  if(window.__QMES_SALES_EDIT_V10__) return;
  window.__QMES_SALES_EDIT_V10__=true;
  ...
  const MODAL_ID="qmes-sales-edit-v10";
  ...
  function ensureStyle(){
    if(document.getElementById("qmes-sales-edit-v10-style"))return;
    const style=document.createElement("style");style.id="qmes-sales-edit-v10-style";
    style.textContent=`
  ...
  function rowFromButton(button){
    ...
    const stored=rowByVisibleId(visibleId);if(stored)return {...stored,id:effectiveId(stored)};
    return {id:visibleId,workOrder:visibleId,customer:cell(1),po:cell(2),product:clean(tr.children?.[3]?.childNodes?.[0]?.textContent)||cell(3),qty:num(cell(4)),due:"",plan:cell(8),shipping:cell(9),deliveryPlace:cell(10),source:"SCREEN"};
  }
  ...
  async function save(form){
    ...
    /* Rebuild from Work Orders. The projection reads salesOrderIdOverride from META_KEY. */
    if(typeof window.qmesSalesFromWorkOrderApply==="function"){try{await window.qmesSalesFromWorkOrderApply();}catch(_){}}
    close();
    /* Reload is intentional: the Sales React state is mounted from local derived rows. */
    setTimeout(()=>window.location.reload(),120);
  }

  const handle=event=>{const button=buttonFromTarget(event.target);if(!button)return;event.preventDefault();event.stopImmediatePropagation();const row=rowFromButton(button);if(row)open(row);};
  ...
  ensureStyle();
  window.qmesSalesEditV10={open,openFromButton:button=>{const row=rowFromButton(button);return row?open(row):false;},close};
})();
