 ...
 * Additive only. Mounts by the inventory table itself so it stays inside the inventory screen.
 */
(function(global){
  'use strict';
  ...
  let lastMarkup='';
  function render(){ensureStyle();const m=mountPoint();if(!m)return false;let box=document.getElementById(id);if(!box){box=document.createElement('section');box.id=id;const anchor=m.anchor;if(anchor&&anchor.parentElement===m.host)m.host.insertBefore(box,anchor);else m.host.insertBefore(box,m.host.firstChild)}const x=stats(),v=validation(),errors=n(v?.counts?.errors),warnings=n(v?.counts?.warnings);const markup=`<div class="qf-title">실시간 재고 요약 · QMES 연동</div><div class="qf-grid"><div class="qf-card"><div class="qf-label">현재고 합계</div><div class="qf-value">${fmt(x.current)} kg</div></div><div class="qf-card"><div class="qf-label">가용재고</div><div class="qf-value">${fmt(x.available)} kg</div></div><div class="qf-card"><div class="qf-label">홀드재고</div><div class="qf-value hold">${fmt(x.hold)} kg</div></div><div class="qf-card"><div class="qf-label">재고 LOT 수</div><div class="qf-value">${x.lots} LOT</div></div><div class="qf-card"><div class="qf-label">금일 IQC 합격 입고</div><div class="qf-value">${fmt(x.iqc)} kg</div></div><div class="qf-card"><div class="qf-label">금일 작업지시 실투입</div><div class="qf-value">${fmt(x.used)} kg</div></div></div><div class="qf-foot"><span>LOT 수불 자동검증</span><b class="${errors?'qf-warn':'qf-ok'}">오류 ${errors}건</b><span>경고 ${warnings}건</span><span>· MES 연동 결과 표시</span></div>`;if(markup===lastMarkup&&box.innerHTML===markup)return false;lastMarkup=markup;box.innerHTML=markup;return true}
  let timer=null;function schedule(){clearTimeout(timer);timer=setTimeout(()=>{render();},100)}
  ...
  new MutationObserver((mutations)=>{if(mutations.every(m=>m.target.closest?.('#'+id)))return;schedule();}).observe(document.documentElement,{childList:true,subtree:true});
