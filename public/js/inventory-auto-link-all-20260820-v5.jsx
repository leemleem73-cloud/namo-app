/* QMES inventory full auto-link v5 - RM recovery and physical racks, 2026-08-20 */
(function(){
  'use strict';
  if(window.__QMES_INV_AUTO_LINK_ALL_V5__) return;
  window.__QMES_INV_AUTO_LINK_ALL_V5__=true;
  ...
  async function migrateLegacyRmStock(stockRows,refs){
    const legacy=(stockRows||[]).filter(row=>upper(row?.location_code||row?.locationCode)==='RM'&&num(row?.quantity)>0);
    for(const row of legacy){
      ...
      const ref=`PHOTO-RACK-MIGRATION-20260820:${upper(itemCode)}:${lotNo}:${status}`;
      try{
        await post({transactionType:'MOVE',itemCode,itemName,category:'RM',lotNo,quantity,unit:text(row?.unit)||'kg',fromLocation:'RM',toLocation,fromStatus:status,toStatus:status,workOrderNo:'',productionLot:'',referenceNo:ref,supplier:'',receivedAt:'',expiryDate:'',reason:'사진 기준 기존 RM 위치 이전',remark:toLocation==='UNASSIGNED'?'사진 미확인 원료 · 위치확인':'창고 사진 표찰 기준 실제 랙 이전'});
        refs.add(ref);
        row.location_code=toLocation;
        row.locationCode=toLocation;
      }catch(e){
        console.warn('기존 RM 위치 자동이전 실패',itemCode,lotNo,e.message);
      }
    }
  }
  ...
  async function syncInventory(){
    ...
    try{
      ...
      await migrateLegacyRmStock(stockRows,refs);
  ...
  function installHint(){
    document.querySelectorAll('.inv-shell .inv-title-row').forEach(row=>{
      let hint=row.querySelector('[data-qmes-inv-auto-v5="1"]');
      if(!hint){hint=document.createElement('div');hint.dataset.qmesInvAutoV5='1';hint.style.cssText='font-size:12px;color:#64748b;margin-top:6px;font-weight:700;';row.querySelector('div')?.appendChild(hint);}
      hint.textContent='실제 랙 자동연동 · 기존 RM 자동이전 · 원재료 A구역 · 완제품 B구역 · 미확인 원료는 위치확인';
    });
  }
