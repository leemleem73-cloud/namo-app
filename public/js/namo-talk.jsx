...
const NAMO_PROFILE_PRESETS=[
  {id:"drop-blue",label:"파랑",color:"#60a5fa",face:"•ᴗ•"},
  {id:"drop-purple",label:"보라",color:"#a78bfa",face:"◕ᴗ◕"},
  {id:"drop-mint",label:"민트",color:"#34d399",face:"•◡•"},
  {id:"drop-pink",label:"분홍",color:"#f9a8d4",face:"˶ᵔᵕᵔ˶"},
  {id:"drop-yellow",label:"노랑",color:"#fbbf24",face:"•‿•"},
  {id:"drop-sky",label:"하늘",color:"#38bdf8",face:"^ᴗ^"},
  {id:"drop-navy",label:"네이비",color:"#345675",face:"•ᴗ•"},
  {id:"drop-coral",label:"코랄",color:"#fb7185",face:"◠‿◠"},
  {id:"drop-angry",label:"화남",color:"#f87171",face:"`皿´"},
  {id:"drop-surprise",label:"놀람",color:"#60a5fa",face:"⊙o⊙"},
  {id:"drop-laugh",label:"웃음",color:"#fbbf24",face:">ᴗ<"},
  {id:"drop-sleepy",label:"졸림",color:"#a78bfa",face:"-ᴗ-"},
  {id:"drop-curious",label:"궁금",color:"#38bdf8",face:"•?•"},
  {id:"drop-cheer",label:"응원",color:"#34d399",face:"ง•ᴗ•ง"},
  {id:"drop-focus",label:"집중",color:"#64748b",face:"•̀ᴗ•́"},
  {id:"drop-thanks",label:"감사",color:"#f9a8d4",face:"˘◡˘"}
];
...
const NAMO_STICKERS=NAMO_STICKER_CATEGORIES.flatMap(category=>{
  const xs=category.side==="left"?[24,143,263,383,503,623]:[788,908,1028,1148,1268,1388];
  const ys=category.vertical==="top"?[61,191,321]:[566,696,826];
  return category.labels.map((label,index)=>({id:`${category.id}-${String(index+1).padStart(2,"0")}`,category:category.id,label,x:xs[index%6],y:ys[Math.floor(index/6)],w:105,h:120}));
});
const NAMO_EMOJI_PAGE_SIZE=8;
const NAMO_STICKER_SHEET="/assets/namo-emoticons-qmes-20260731.webp";
...
function NamoProfileAvatar({profile,size=36,name=""}){
  ...
  return <span aria-label={`${name||"사용자"} ${preset.label} 물방울 프로필`} style={{width:size,height:size,flex:`0 0 ${size}px`,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:"50%",background:"#f8fafc",border:"1px solid #dbe3ea",overflow:"hidden"}}><span style={{width:size*.66,height:size*.66,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:"55% 55% 60% 60% / 68% 68% 42% 42%",transform:"rotate(45deg)",background:`linear-gradient(145deg,#ffffff,${preset.color})`,boxShadow:"inset 0 1px 3px rgba(255,255,255,.8),0 1px 3px rgba(15,39,64,.18)"}}><span style={{transform:"rotate(-45deg)",fontSize:Math.max(7,size*.18),fontWeight:900,color:"#24364b",whiteSpace:"nowrap"}}>{preset.face}</span></span></span>;
}

function NamoSticker({sticker,width=70}){
  if(!sticker)return null;
  const scale=width/sticker.w,height=Math.round(sticker.h*scale);
  return <span role="img" aria-label={sticker.label} style={{width,height,display:"inline-block",flex:`0 0 ${width}px`,backgroundImage:`url(${NAMO_STICKER_SHEET})`,backgroundRepeat:"no-repeat",backgroundSize:`${1536*scale}px ${1024*scale}px`,backgroundPosition:`${-sticker.x*scale}px ${-sticker.y*scale}px`,borderRadius:8}}/>;
}
...
function NamoTalkTab({onClose,initialRoom=""}){
  ...
  return <section ref={panelRef} aria-label="NAMO Talk" onWheel={event=>event.stopPropagation()} style={{...panelStyle,zIndex:12000,display:"flex",flexDirection:"column",minWidth:compact?0:340,minHeight:compact?0:480,maxWidth:compact?"none":"calc(100vw - 8px)",maxHeight:compact?"none":"calc(100vh - 8px)",background:"#ffffff",color:"#172033",fontFamily:"'Pretendard','Noto Sans KR',sans-serif",boxShadow:"0 18px 50px rgba(15,23,42,.32)",border:"2px solid #d4a017",borderRadius:compact||maximized?0:14,overflow:"hidden",overscrollBehavior:"contain"}}>
    ...
    {profileOpen&&<div role="dialog" aria-modal="true" aria-label="프로필 수정" onClick={()=>setProfileOpen(false)} style={{position:"fixed",inset:0,zIndex:2147483000,display:"flex",alignItems:"center",justifyContent:"center",padding:16,background:"rgba(15,23,42,.52)"}}><div onClick={event=>event.stopPropagation()} style={{width:"min(430px,100%)",maxHeight:"90vh",overflowY:"auto",padding:20,boxSizing:"border-box",borderRadius:16,background:"white",boxShadow:"0 22px 60px rgba(15,23,42,.35)"}}><div style={{display:"flex",alignItems:"center"}}><strong style={{fontSize:19,color:"#172033"}}>프로필 수정</strong><button type="button" onClick={()=>setProfileOpen(false)} aria-label="프로필 수정 닫기" style={{marginLeft:"auto",width:32,height:32,border:0,borderRadius:8,background:"#eef2f6",fontSize:20,cursor:"pointer"}}>×</button></div><div style={{display:"flex",alignItems:"center",gap:13,marginTop:16,padding:13,border:"1px solid #e2e8f0",borderRadius:12,background:"#f8fafc"}}><NamoProfileAvatar profile={profileDraft} size={58} name={currentUser.name}/><div><strong style={{display:"block",fontSize:15,color:"#172033"}}>{currentUser.name}</strong><span style={{display:"block",marginTop:4,fontSize:12,color:"#64748b"}}>{currentUser.dept||"부서 미지정"} · 나모 물방울 프로필</span></div></div><div style={{marginTop:16,fontSize:13,fontWeight:900,color:"#334155"}}>캐릭터 선택</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:9,marginTop:9}}>{NAMO_PROFILE_PRESETS.map(preset=><button key={preset.id} type="button" onClick={()=>setProfileDraft({type:"preset",value:preset.id})} title={preset.label} style={{height:68,display:"flex",alignItems:"center",justifyContent:"center",border:`2px solid ${profileDraft?.type==="preset"&&profileDraft.value===preset.id?"#0284c7":"#e2e8f0"}`,borderRadius:11,background:profileDraft?.type==="preset"&&profileDraft.value===preset.id?"#f0f9ff":"white",cursor:"pointer"}}><NamoProfileAvatar profile={{type:"preset",value:preset.id}} size={44}/></button>)}</div><input ref={profileFileRef} type="file" accept="image/*" onChange={uploadProfileImage} style={{display:"none"}}/><button type="button" onClick={()=>profileFileRef.current?.click()} style={{width:"100%",height:40,marginTop:13,border:"1px solid #94a3b8",borderRadius:9,background:"white",color:"#334155",fontSize:13,fontWeight:900,cursor:"pointer"}}>🖼 내 그림 업로드</button><div style={{display:"flex",gap:8,marginTop:16}}><button type="button" onClick={()=>setProfileDraft({type:"preset",value:"drop-blue"})} style={{height:39,padding:"0 13px",border:"1px solid #cbd5e1",borderRadius:9,background:"#f8fafc",color:"#475569",fontWeight:850,cursor:"pointer"}}>기본 이미지</button><button type="button" onClick={()=>setProfileOpen(false)} style={{height:39,marginLeft:"auto",padding:"0 15px",border:"1px solid #cbd5e1",borderRadius:9,background:"white",fontWeight:850,cursor:"pointer"}}>취소</button><button type="button" onClick={submitProfile} disabled={savingProfile} style={{height:39,padding:"0 17px",border:0,borderRadius:9,background:"#0f2740",color:"white",fontWeight:900,cursor:"pointer"}}>{savingProfile?"저장 중":"저장"}</button></div></div></div>}
    {!compact&&!maximized&&[
    ...
    {mode==="attendance"&&!chatRoomOpen?<AttendancePanel currentUser={currentUser} users={users}/>:<div style={{flex:"1 1 auto",minHeight:0,display:"flex",width:"100%",overflow:"hidden",padding:compact?8:10,boxSizing:"border-box",background:"#ffffff"}}>
      ...
      <div style={{flex:"1 1 auto",minWidth:0,minHeight:0,display:chatRoomOpen?"flex":"none",flexDirection:"column",background:"#ffffff",border:"1px solid #cbd5e1",borderRadius:compact?10:12,boxShadow:"0 2px 8px rgba(15,39,64,.08)",overflow:"hidden",position:"relative"}}>
        ...
        <div ref={scrollRef} onScroll={event=>{const element=event.currentTarget;stickToBottomRef.current=element.scrollHeight-element.scrollTop-element.clientHeight<=60;}} style={{flex:"1 1 auto",minHeight:0,overflowY:"auto",overscrollBehavior:"contain",padding:15}}>
          ...
          {visibleMessages.map(msg=>{const mine=msg.sender===currentUser.name;const info=mine?receiptInfoFor(msg):null;const label=info?.read.length?"읽음":"전송됨";return <div id={`namo-message-${msg.id}`} key={msg.id} style={{marginBottom:14,textAlign:mine?"right":"left"}}>
            ...
            <div style={{display:"inline-block",maxWidth:compact?"90%":"84%",background:msg.pinned?"#fffdf7":mine?"#0284c7":"white",color:msg.pinned?"#3f3a2d":mine?"white":"#172033",border:msg.pinned?"1px solid #f3d38a":"1px solid transparent",borderRadius:14,padding:msg.kind==="emoticon"||msg.kind==="sticker"?"10px 14px":"10px 12px",fontSize:msg.kind==="emoticon"?(compact?26:30):(compact?13:15),lineHeight:1.55,boxShadow:msg.pinned?"0 2px 7px rgba(180,130,20,.12)":"0 1px 3px rgba(15,23,42,.1)",wordBreak:"break-word",opacity:msg.deleted?.72:1}}>
              {msg.replyToId&&<div style={{marginBottom:7,padding:"6px 8px",borderRadius:7,background:msg.pinned?"#fff7df":mine?"rgba(255,255,255,.16)":"#f1f5f9",borderLeft:`3px solid ${msg.pinned?"#e7b94f":mine?"#bae6fd":"#38bdf8"}`,fontSize:11,lineHeight:1.35}}><strong>{msg.replySender}</strong><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{msg.replyText}</div></div>}
        ...
        {emojiOpen&&<div style={{position:"absolute",right:10,bottom:84,width:"min(320px,calc(100% - 20px))",maxHeight:"calc(100% - 96px)",overflowY:"auto",boxSizing:"border-box",padding:"18px 12px 12px",background:"white",border:"1px solid #cbd5e1",borderRadius:14,boxShadow:"0 14px 35px rgba(15,23,42,.2)",zIndex:20}}><div style={{display:"flex",alignItems:"center",minHeight:30,marginBottom:8,overflow:"visible"}}><strong style={{display:"block",fontSize:compact?13:14,lineHeight:1.8,padding:"3px 0 1px",overflow:"visible"}}>나모 이모티콘</strong><button onClick={()=>setEmojiOpen(false)} style={{marginLeft:"auto",border:0,background:"transparent",fontSize:18,lineHeight:1,cursor:"pointer"}}>×</button></div><div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:5,marginBottom:9}}>{NAMO_STICKER_CATEGORIES.map(category=><button key={category.id} type="button" onClick={()=>{setStickerCategory(category.id);setEmojiPage(0);}} style={{height:30,minWidth:0,border:`1px solid ${stickerCategory===category.id?"#0284c7":"#dbe3ea"}`,borderRadius:8,background:stickerCategory===category.id?"#e0f2fe":"#f8fafc",color:stickerCategory===category.id?"#0369a1":"#475569",fontSize:10,fontWeight:900,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{category.number} {category.title}</button>)}</div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:7}}>{categoryStickers.slice(emojiPage*NAMO_EMOJI_PAGE_SIZE,emojiPage*NAMO_EMOJI_PAGE_SIZE+NAMO_EMOJI_PAGE_SIZE).map(sticker=><button key={sticker.id} onClick={()=>selectSticker(sticker)} title={sticker.label} style={{height:76,minWidth:0,padding:2,display:"inline-flex",alignItems:"center",justifyContent:"center",overflow:"hidden",border:`2px solid ${selectedSticker?.id===sticker.id?"#0284c7":"#e2e8f0"}`,borderRadius:9,background:"#f8fafc",cursor:"pointer"}}><NamoSticker sticker={sticker} width={compact?55:61}/></button>)}</div><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginTop:10}}><button type="button" onClick={()=>setEmojiPage(page=>Math.max(0,page-1))} disabled={emojiPage===0} aria-label="이전 이모티콘" style={{width:36,height:28,border:"1px solid #cbd5e1",borderRadius:7,background:"white",color:emojiPage===0?"#cbd5e1":"#334155",cursor:emojiPage===0?"default":"pointer"}}>◀</button><span style={{fontSize:11,color:"#64748b",fontWeight:800}}>{emojiPage+1}/{Math.ceil(categoryStickers.length/NAMO_EMOJI_PAGE_SIZE)}</span><button type="button" onClick={()=>setEmojiPage(page=>Math.min(Math.ceil(categoryStickers.length/NAMO_EMOJI_PAGE_SIZE)-1,page+1))} disabled={emojiPage>=Math.ceil(categoryStickers.length/NAMO_EMOJI_PAGE_SIZE)-1} aria-label="다음 이모티콘" style={{width:36,height:28,border:"1px solid #cbd5e1",borderRadius:7,background:"white",color:emojiPage>=Math.ceil(categoryStickers.length/NAMO_EMOJI_PAGE_SIZE)-1?"#cbd5e1":"#334155",cursor:emojiPage>=Math.ceil(categoryStickers.length/NAMO_EMOJI_PAGE_SIZE)-1?"default":"pointer"}}>▶</button></div></div>}
