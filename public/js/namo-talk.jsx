...
function NamoProfileAvatar({profile,size=36,name=""}){
  ...
  return <span aria-label={`${name||"사용자"} ${preset.label} 물방울 프로필`} style={{width:size,height:size,flex:`0 0 ${size}px`,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:"50%",background:`radial-gradient(circle at 30% 20%,#fff 0 18%,${preset.color3} 72%)`,border:`1.5px solid ${preset.color2}`,boxShadow:`inset 0 1px 2px white,0 2px 7px ${preset.color2}66`,overflow:"hidden"}}><span style={{width:size*.84,height:size*.78,position:"relative",display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:"51% 49% 55% 55% / 60% 60% 44% 44%",transform:"rotate(45deg)",background:`radial-gradient(circle at 28% 24%,${preset.color3} 0 14%,${preset.color2} 45%,${preset.color} 88%)`,border:"1.5px solid rgba(255,255,255,.94)",boxShadow:`inset 3px 3px 7px rgba(255,255,255,.96),inset -3px -3px 6px ${preset.accent}38,0 2px 6px ${preset.accent}55`}}><span style={{position:"absolute",top:"10%",left:"14%",width:"29%",height:"17%",borderRadius:"50%",background:"rgba(255,255,255,.9)",filter:"blur(.2px)"}}/><span style={{position:"absolute",right:"14%",top:"22%",width:"9%",height:"9%",borderRadius:"50%",background:"rgba(255,255,255,.7)"}}/><NamoAvatarFace mood={preset.mood} size={size}/></span></span>;
}
...
function NamoAvatarFace({mood="happy",size=36}){
  ...
  const eyeWidth=Math.max(3.8,size*.105),eyeHeight=Math.max(5.2,size*.155);
  const eye=(side,extra={})=><i style={{position:"absolute",top:isAngry||isFocus?"23%":isCurious&&side==="right"?"9%":"13%",[side]:"11%",width:isCurious&&side==="right"?eyeWidth*1.16:eyeWidth,height:isCurious&&side==="right"?eyeHeight*1.12:eyeHeight,borderRadius:"48% 48% 52% 52%",background:`linear-gradient(180deg,#416b91 0%,${ink} 72%)`,boxShadow:"inset 0 -1px 2px rgba(0,0,0,.18),0 1px 1px rgba(255,255,255,.55)",...extra}}><i style={{position:"absolute",left:"15%",top:"9%",width:"40%",height:"34%",borderRadius:"50%",background:"white",boxShadow:"0 0 2px white"}}/><i style={{position:"absolute",right:"16%",bottom:"17%",width:"18%",height:"16%",borderRadius:"50%",background:"rgba(255,255,255,.8)"}}/></i>;
  ...
  const mouthStyle=isSurprise?{width:size*.105,height:size*.125,background:"#7f2947",border:`1px solid ${ink}`,borderRadius:"50%",boxShadow:"inset 0 2px 0 #ffb8cb"}:isAngry?{width:size*.17,height:size*.075,borderTop:`${Math.max(2,size*.04)}px solid ${ink}`,borderRadius:"50%"}:mood==="soft"?{width:size*.12,height:size*.065,borderBottom:`${Math.max(2,size*.04)}px solid ${ink}`,borderRadius:"0 0 50% 50%"}:{width:size*.2,height:size*.105,background:"#923655",border:`1px solid ${ink}`,borderRadius:"30% 30% 58% 58%",boxShadow:"inset 0 -3px 0 #ff9bb5"};
  return <span style={{position:"relative",width:size*.52,height:size*.42,transform:"rotate(-45deg) translateY(9%)",display:"block"}}>
    ...
    <i style={{position:"absolute",left:"50%",top:"53%",width:size*.035,height:size*.025,transform:"translateX(-50%)",borderRadius:"50%",background:"rgba(255,255,255,.55)"}}/>
    <i style={{position:"absolute",left:"50%",bottom:"0",transform:"translateX(-50%)",...mouthStyle}}/>
    <i style={{position:"absolute",left:"-5%",bottom:"10%",width:size*.145,height:size*.075,borderRadius:"50%",background:"rgba(255,126,164,.68)",boxShadow:"0 0 3px rgba(255,183,203,.7)"}}/><i style={{position:"absolute",right:"-5%",bottom:"10%",width:size*.145,height:size*.075,borderRadius:"50%",background:"rgba(255,126,164,.68)",boxShadow:"0 0 3px rgba(255,183,203,.7)"}}/>
    ...
    {mood==="soft"&&<i style={{position:"absolute",right:"-22%",top:"-22%",fontSize:Math.max(8,size*.18),fontStyle:"normal",color:"#ff7aa8",textShadow:"0 1px 2px white"}}>♥</i>}
    {mood==="sleepy"&&<i style={{position:"absolute",right:"-24%",top:"-28%",fontSize:Math.max(8,size*.17),fontStyle:"normal",fontWeight:950,color:"#eef2ff",textShadow:"0 1px 2px rgba(15,39,64,.35)"}}>Z</i>}
    {isSurprise&&<i style={{position:"absolute",right:"-18%",top:"-27%",fontSize:Math.max(8,size*.19),fontStyle:"normal",fontWeight:950,color:"#fff4a8",textShadow:"0 1px 2px rgba(15,39,64,.3)"}}>!</i>}
    {mood==="thanks"&&<i style={{position:"absolute",left:"-19%",top:"-24%",fontSize:Math.max(8,size*.17),fontStyle:"normal",color:"#fff7a8",textShadow:"0 1px 2px rgba(15,39,64,.25)"}}>✧</i>}
  </span>;
}
    ...
    {mode==="attendance"&&!chatRoomOpen?<AttendancePanel currentUser={currentUser} users={users}/>:<div style={{flex:"1 1 auto",minHeight:0,display:"flex",width:"100%",overflow:"hidden",padding:compact?8:10,boxSizing:"border-box",background:"#ffffff"}}>
      ...
      <div style={{flex:"1 1 auto",minWidth:0,minHeight:0,display:chatRoomOpen?"flex":"none",flexDirection:"column",background:"#ffffff",border:"1px solid #cbd5e1",borderRadius:compact?10:12,boxShadow:"0 2px 8px rgba(15,39,64,.08)",overflow:"hidden",position:"relative"}}>
        ...
        <div style={{flex:"0 0 auto",background:"white",padding:10,borderTop:"1px solid #cbd5e1"}}>
          ...
          {selectedSticker&&<div style={{display:"flex",alignItems:"center",gap:9,marginBottom:8,padding:"6px 9px",border:"1px solid #dbe3ea",borderRadius:9,background:"#f8fafc"}}><NamoSticker sticker={selectedSticker} width={48} withLabel={false}/><strong style={{fontSize:14,color:"#25344a"}}>{selectedSticker.label}</strong><button type="button" onClick={()=>setSelectedSticker(null)} aria-label="선택한 이모티콘 제거" style={{marginLeft:"auto",width:25,height:25,border:"1px solid #dbe3ea",borderRadius:7,background:"white",fontSize:16,cursor:"pointer"}}>×</button></div>}
