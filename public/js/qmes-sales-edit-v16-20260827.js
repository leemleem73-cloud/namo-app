/* NAMO QMES - Sales V16 - stable edit modal + safe visible sales number */
(function(){
  "use strict";
  if(window.__QMES_SALES_V16__)return;
  window.__QMES_SALES_V16__=true;
  ...
  const MODAL_ID="qmes-sales-edit-v16";
  ...
  function metaFor(row){
    const m=map(META_KEY),key=rowKey(row),id=clean(row?.id);
  ...
  function findRow(id){
    ...
    return rows().find(r=>clean(r?.id)===target||rowKey(r)===target||visibleId(r)===target)||null;
  }

  function ensureStyle(){
    if(document.getElementById("qmes-sales-v16-style"))return;
    const s=document.createElement("style");s.id="qmes-sales-v16-style";
    s.textContent=`
      ...
      .qmes-sales-stable .qerp-sales-compact-form .qmes-sales-po-field-v16{display:none!important}
      ...
      #${MODAL_ID} .head{display:flex!important;justify-content:space-between!important;align-items:center!important;padding:18px 20px!important;border-bottom:1px solid #e2e8f0!important;background:#fff!important}
  ...
  function reconcileUi(){
    document.querySelectorAll('.qmes-sales-stable .qerp-sales-compact-form .qerp-field').forEach(field=>{
      if(clean(field.querySelector('label')?.textContent)==='고객 PO 번호')field.classList.add('qmes-sales-po-field-v16');
    });
    ...
    document.querySelectorAll('.qmes-sales-stable table.qerp-table tbody tr').forEach(tr=>{
      const link=tr.querySelector('.qmes-sales-order-link,[data-qso-id]');
      if(!link)return;
      const raw=clean(link.getAttribute('data-qso-id')||link.textContent);
      const row=findRow(raw)||findRow(clean(link.textContent));
      if(!row)return;
      const next=visibleId(row);
      if(next&&clean(link.textContent)!==next)link.textContent=next;
      /* Important: do NOT rewrite data-qso-id. React keeps that as the stable row key. */
    });
  }
  ...
  function rowFromButton(button){
    ...
    return findRow(stableId)||findRow(shownId)||null;
  }

  function open(row){
    if(!row)return false;
    ensureStyle();close();
    const id=visibleId(row),key=rowKey(row),meta=metaFor(row),pm=map(PACK_KEY),rm=map(REMARK_KEY),pack=pm[key]||pm[clean(row.id)]||row.packaging||{},remarks=clean(rm[key]??rm[clean(row.id)]??row.remarks);
    ...
    m.__row={...row,key};document.body.appendChild(m);return true;
  }
  ...
  async function saveEdit(form){
    ...
    if(type||unit||count){
      if(!type||unit<=0||count<=0){alert('포장정보를 모두 입력하세요.');return;}
      pm[key]={type,unitWeight:unit,packageQty:count,total:Number((unit*count).toFixed(3)),savedAt:now};
    }else delete pm[key];
    write(PACK_KEY,pm);
    ...
    close();
    reconcileUi();
    setTimeout(reconcileUi,100);
  }

  document.addEventListener('click',event=>{
    const target=event.target;if(!(target instanceof Element))return;
    const button=target.closest('button');
    if(button&&(button.classList.contains('qmes-sales-edit-btn')||clean(button.textContent)==='수정')&&button.closest('.qmes-sales-stable')){
      const row=rowFromButton(button);if(!row)return;
      event.preventDefault();event.stopImmediatePropagation();open(row);return;
    }
  ...
  document.addEventListener('submit',event=>{
    const form=event.target instanceof Element?event.target.closest('[data-form="1"]'):null;if(!form)return;
    event.preventDefault();event.stopImmediatePropagation();saveEdit(form);
  },true);

  window.addEventListener('qmes:erp-data-changed',()=>setTimeout(reconcileUi,0));
  window.addEventListener('qmes:mes-master-ready',()=>setTimeout(reconcileUi,0),{once:true});
  ensureStyle();reconcileUi();
  let tries=0;const timer=setInterval(()=>{tries++;reconcileUi();if(tries>=20)clearInterval(timer);},500);
  window.qmesSalesEditV16={open,close,reconcileUi};
})();
