(function installUnifiedInventoryV2(global){
  ...
  function UnifiedInventoryV2(){
    ...
    const [safetyDrafts,setSafetyDrafts]=ReactRef.useState({});
    ...
    async function saveSafety(row){
      const raw=safetyDrafts[row.code]??String(row.safety??0);
      const parsed=Number(String(raw).trim());
      if(String(raw).trim()===""||!Number.isFinite(parsed)||parsed<0){
        alert("안전재고는 0 이상의 숫자로 입력하세요.");
        return;
      }
      const safety=round(parsed);
      await runSave(()=>saveRecord(`v2:master:${row.code}`,{
        kind:"inventory-v2-master",type:row.type,code:row.code,name:row.name,unit:row.unit,
        safety,location:row.location||"미지정",iqcRequired:row.iqcRequired!==false,
        inspection:row.inspection||"",note:row.note||"",updatedAt:nowIso(),updatedBy:currentUser()
      }),`${row.code} 안전재고가 ${fmt(safety)} ${row.unit}로 저장되었습니다.`);
    }

    function safetyEditor(row){
      const value=safetyDrafts[row.code]??String(row.safety??0);
      const dirty=round(value)!==round(row.safety);
      return h("div",{className:"qmes-inv2-safety-editor",onClick:event=>event.stopPropagation()},
        h("input",{
          type:"number",min:"0",step:"0.001",value,
          "aria-label":`${row.name} 안전재고`,
          onChange:event=>setSafetyDrafts(current=>({...current,[row.code]:event.target.value})),
          onKeyDown:event=>{if(event.key==="Enter"){event.preventDefault();saveSafety(row);}}
        }),
        h("button",{type:"button",disabled:saving||!dirty,onClick:()=>saveSafety(row)},saving?"저장 중":"저장")
      );
    }
      ...
      return h(ReactRef.Fragment,null,
        ...
        h(Table,{headers:["구분","자재코드","품명","현재고","HOLD","가용재고","안전재고","부족량","단위","LOT","위치","상태"],minWidth:"1260px"},
          filteredItems.length?filteredItems.map(row=>h("tr",{key:row.code,onClick:()=>openLot(row),className:"is-clickable"},
            ...
            h("td",{className:"is-number is-available"},fmt(row.available)),h("td",{className:"is-safety"},safetyEditor(row)),
  ...
  style.textContent=`
    ...
    .qmes-inv2-safety-editor{display:grid;grid-template-columns:minmax(66px,82px) 42px;align-items:center;justify-content:center;gap:5px}.qmes-inv2-safety-editor input{width:100%;height:30px;box-sizing:border-box;border:1px solid #3b5872;border-radius:6px;background:#0a1d30;color:#fff;padding:0 7px;text-align:right;font:800 12px Pretendard,sans-serif;outline:none}.qmes-inv2-safety-editor input:focus{border-color:#38bdf8;box-shadow:0 0 0 2px rgba(56,189,248,.16)}.qmes-inv2-safety-editor button{height:30px;padding:0 6px;border:1px solid #2563eb;border-radius:6px;background:#2563eb;color:#fff;font:800 10px Pretendard,sans-serif;cursor:pointer}.qmes-inv2-safety-editor button:disabled{border-color:#365169;background:#172d42;color:#7890a5;cursor:default}
