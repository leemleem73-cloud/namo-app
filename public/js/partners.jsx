...
function PartnersTab() {
  ...
  return (
    <div className="space-y-5 qmes-partners-page">
      <style>{`
        .qmes-partner-register-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;min-height:34px;padding:7px 12px;border:1px solid #0891b2;border-radius:7px;background:#0e7490;color:#fff;font-size:13px;font-weight:800;line-height:1;cursor:pointer;box-shadow:0 2px 8px rgba(8,145,178,.18)}
        .qmes-partner-register-btn:hover,.qmes-partner-register-btn:focus-visible{background:#0891b2;border-color:#22d3ee;color:#fff;outline:none}
      `}</style>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-700 px-5 py-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <button type="button" className="qmes-partner-register-btn" data-qmes-partner-register="true">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm">
          {activeType === "customer" ? <>
            ...
            <tbody>{filteredSuppliers.map((item,index)=><tr key={item.code} className="border-t border-slate-800"><td className="px-4 py-3 text-center text-slate-400">{index+1}</td><td className="px-4 py-3 font-mono text-sky-300">{item.code}</td><td className="px-4 py-3 font-semibold text-white">{item.company}</td><td className="px-4 py-3 text-white">{item.material}</td><td className="px-4 py-2"><input value={item.lot} onChange={(e)=>updateSupplierLot(item.code,e.target.value)} className="w-full rounded-md border border-slate-600 bg-slate-950 px-3 py-2 font-mono text-cyan-300"/></td><td className="px-4 py-3 text-center text-emerald-300">{item.status}</td><td className="px-4 py-3 text-center"><button type="button" className={btnEdit}>수정</button></td></tr>)}</tbody>
          </>}
        </table></div>
      </div>
    </div>
