/* QMES Stage 12 operational loader v50
 * Current modules remain loaded, but Sales bootstrap stability is installed first
 * so local Sales rows do not flash/disappear when shared ERP data arrives later.
 * Sales SAFE detail drawer is loaded before the older drawer owner.
 * Enterprise new Sales Order modal is loaded before ERP runtime so the legacy inline form never owns the click.
 */
(function(){
  const STYLE_DEFS=[
