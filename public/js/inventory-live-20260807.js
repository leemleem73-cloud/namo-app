 ...
 * SBS+PVdF internal mix is managed under SBS and shown in the remark column.
 */
(function installLiveInventory(global){
  ...
  function isSbsMix(value){
    const v = upper(value);
    return v.includes("SBS") && v.includes("PVDF");
  }

  function materialKey(value){
    const v = upper(value);
    /* SBS+PVdF 혼합재는 재고 화면에서 SBS 재고로 통합 */
    if (isSbsMix(v)) return "SBS";
  ...
  function buildUsage(){
    ...
    workOrders.forEach(([workOrderNo, workOrder]) => {
      (Array.isArray(workOrder?.inputs) ? workOrder.inputs : []).forEach((input) => {
        ...
        if (!lot || !(act > 0)) return;
        const descriptor = [input?.name,input?.materialName,input?.note,input?.remarks,input?.remark,input?.memo]
          .map(text).filter(Boolean).join(" ");
        const key = lotKey(descriptor || input?.name, lot);
  ...
  function buildLotLedger(){
    ...
    iqcRows.forEach((row, index) => {
      ...
      const current = receipts.get(key) || {
        ...
        mix:isSbsMix(name) || /SBS\s*\+\s*PVDF/i.test(text(row?.remarks || row?.note))
      };
      ...
      if (isSbsMix(name) || /SBS\s*\+\s*PVDF/i.test(text(row?.remarks || row?.note))) current.mix = true;
  ...
  function baseInventory(){
    try {
      if (typeof INVENTORY !== "undefined" && Array.isArray(INVENTORY)) {
        return INVENTORY
          .filter((row) => materialKey(row?.name) !== "CAN20" && String(row?.code || "") !== "PK-CAN20")
          .filter((row) => String(row?.code || "") !== "RM-SBSPVDF-MIX" && !isSbsMix(row?.name));
      }
    } catch (_error) {}
    return [
      ...
      { code:"RM-SBS", name:"SBS", stock:0, safety:300, unit:"kg", loc:"A-1-1", cond:"25±5℃ · 습도 50%↓" },
      { code:"RM-PVDF", name:"PVdF", stock:0, safety:400, unit:"kg", loc:"A-1-1 / A-3-1", cond:"25±5℃ · 습도 50%↓" },
      { code:"RM-SBR", name:"SBR", stock:0, safety:300, unit:"kg", loc:"A-2-1 / A-2-2", cond:"25±5℃ · 습도 50%↓" }
    ];
  }
  ...
  function storageLocation(value){
    ...
    if(source.includes("PVDF") && !source.includes("SBS")) return "A-1-1 / A-3-1";
  ...
  function buildInventoryRows(){
    ...
    return bases.map((base) => {
      ...
      const stock = linked ? matched.reduce((sum, lot) => sum + lot.remaining, 0) : Number(base.stock || 0);
      ...
      const hasMix = key === "SBS" && matched.some((lot) => lot.mix);
      return {
        ...base,
  ...
  function LiveInventoryTab(){
    ...
    const table = h("div", { className:"overflow-x-auto -mx-4 px-4" },
      h("table", { className:"w-full text-sm min-w-[1020px]" },
        h("thead", null,
          h("tr", { className:"text-xs text-slate-400 border-b border-slate-800" },
            ["자재코드","품명","현재고","가용재고","안전재고","LOT","재고 수준","보관위치","보관조건","상태","비고"].map((label, index) =>
        ...
        h("tbody", null,
          rows.map((row) => {
            ...
            return h("tr", { key:row.code, className:"border-b border-slate-800/60 hover:bg-slate-800/30" },
              ...
              h("td", { className:"py-2.5 pr-3" }, typeof Badge !== "undefined"
                ? h(Badge, { tone:low ? "amber" : "green" }, low ? "부족" : "정상")
                : h("span", null, low ? "부족" : "정상")),
              h("td", { className:"py-2.5 text-xs text-slate-300 whitespace-nowrap" }, row.remark || "-")
            );
          })
        )
      )
    ...
    return h("div", { className:"flex flex-col gap-4", "data-live-version":version },
      ...
      h(Panel, { title:"원재료 · 부자재 재고 현황", right:h("span", { className:"text-xs text-slate-400" }, `실시간 연동 ${linkedCount}/${rows.length}품목`) },
        table,
        h("p", { className:"text-[11px] text-slate-500 mt-3" }, "현재고 = IQC 합격 입고량 - 작업지시 실투입량 · SBS+PVdF 사내 혼합재는 SBS 재고에 포함하고 비고로 구분 · LOT 단위 자동 집계")
      )
    );
  }
