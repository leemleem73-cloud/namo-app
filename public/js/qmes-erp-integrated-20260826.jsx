...
(function(){
  ...
  function QMESErpPurchaseTab(){
    ...
    const rowVat=row=>Math.round(rowAmount(row)*.1);
    const rowTotal=row=>rowAmount(row)+rowVat(row);
    const rowQtyText=row=>number(row?.qty)>0?number(row.qty).toLocaleString("ko-KR")+" "+(row.unit||"kg"):"미입력";
    const rowReceiptText=row=>number(row?.qty)>0?rowReceived(row).toLocaleString("ko-KR")+" / "+number(row.qty).toLocaleString("ko-KR")+" "+(row.unit||"kg"):(/입고\s*완료/.test(clean(row?.receiptStatus||row?.receiving))?"입고완료":"미입력");
    const pass=value=>/합격|적합|PASS|OK/i.test(clean(value));
    const fail=value=>/불합격|부적합|FAIL|NG|REJECT/i.test(clean(value));
    const stateFor=row=>{
      const manual=clean(row?.status),approval=clean(row?.approvalStatus||row?.approval),receipt=clean(row?.receiptStatus||row?.receiving),iqc=clean(row?.iqcStatus||row?.iqc);
      const qty=number(row?.qty),received=rowReceived(row);
      if(/취소/.test(manual))return "발주취소";
      if(fail(iqc))return "IQC 부적합";
      if(/입고\s*완료/.test(receipt)&&qty<=0)return "입고완료";
    ...
    useEffect(()=>{
      ...
      fetch("/api/auth/me",{credentials:"same-origin"})
        ...
        .then(result=>{if(live)setIsAdmin(String(result?.data?.role||"").toLowerCase()==="admin");})
        .catch(()=>{});
      return()=>{live=false;};
    },[]);
    ...
    useEffect(()=>{
      ...
      style.textContent=".qp-flow{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));margin:0 0 13px;padding:18px 24px;background:#fff;border:1px solid #dbe3ec;border-radius:12px;box-shadow:0 8px 24px rgba(15,23,42,.07)}.qp-stage{position:relative;display:flex;align-items:center;gap:9px;min-width:0}.qp-stage:not(:last-child):after{content:'';position:absolute;left:30px;right:5px;top:14px;height:2px;background:#24b8ae}.qp-stage:nth-child(4):after{background:#e2e8f0}.qp-dot{position:relative;z-index:1;width:32px;height:32px;display:grid;place-items:center;flex:0 0 auto;border-radius:50%;background:#e8fbf9;color:#0f8a83;border:1.5px solid #24b8ae;font-size:12px;font-weight:900}.qp-stage.active .qp-dot{background:#fffaf0;color:#a16207;border:2px solid #f2b84b;box-shadow:0 0 0 4px rgba(242,184,75,.16)}.qp-stage.wait .qp-dot{background:#fff;color:#64748b;border:1.5px solid #d5dee9}.qp-copy{position:relative;z-index:2;background:#fff;padding-right:9px}.qp-copy b{display:block;color:#0f172a;font-size:14px;font-weight:900;line-height:1.35;white-space:nowrap}.qp-copy small{display:block;color:#475569;font-size:11px;font-weight:750;line-height:1.4;white-space:nowrap;margin-top:3px}.qp-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px;margin-bottom:13px}.qp-kpi{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:12px 14px;box-shadow:0 6px 18px rgba(15,23,42,.05)}.qp-kpi span{display:block;color:#64748b;font-size:10px;font-weight:800}.qp-kpi b{display:block;margin-top:5px;color:#0f172a;font-size:20px}.qp-kpi.red b,.qp-kpi.orange b,.qp-kpi.green b{color:#0f172a}.qp-toolbar{display:flex;gap:8px;align-items:center;margin-bottom:10px}.qp-toolbar input,.qp-toolbar select{height:35px;border:1px solid #cbd5e1;border-radius:7px;padding:0 10px;background:#fff;color:#1e293b;font-size:11px}.qp-toolbar input{min-width:260px}.qp-count{margin-left:auto;color:#64748b;font-size:10px;font-weight:800}.qp-pagination{display:flex;align-items:center;justify-content:center;gap:6px;padding:16px 8px 2px;border-top:1px solid #eef2f7}.qp-pagination button{min-width:32px;height:32px;padding:0 10px;border:1px solid #cbd5e1;border-radius:7px;background:#fff;color:#334155;font-size:11px;font-weight:900;cursor:pointer}.qp-pagination button:hover:not(:disabled){border-color:#14b8a6;color:#0f766e;background:#f0fdfa}.qp-pagination button.active{border-color:#0f766e;background:#0f766e;color:#fff}.qp-pagination button:disabled{opacity:.38;cursor:default}.qp-pagination span{margin-left:6px;color:#64748b;font-size:10px;font-weight:800}.qp-po-link{border:0;background:transparent;color:#1d4ed8;font:inherit;font-weight:900;cursor:pointer;padding:0;text-decoration:underline;text-underline-offset:3px}.qp-actions{display:flex;gap:5px}.qp-mini{height:30px;border:1px solid #cbd5e1;border-radius:6px;background:#fff;color:#334155;padding:0 9px;font-size:11px;font-weight:900;cursor:pointer}.qp-mini.primary{border-color:#99f6e4;background:#f0fdfa;color:#0f766e}.qp-mini.danger{border-color:#fecaca;background:#fff1f2;color:#b91c1c}.qp-mini.danger:hover{border-color:#ef4444;background:#fee2e2}.qp-modal-bg{position:fixed;inset:0;z-index:14080;background:rgba(15,23,42,.62);display:grid;place-items:center;padding:18px}.qp-modal{width:min(1100px,calc(100vw - 30px));max-height:calc(100vh - 36px);overflow:auto;background:#f8fafc;border:1px solid #dbe3ec;border-radius:15px;box-shadow:0 28px 80px rgba(15,23,42,.3)}.qp-modal.qp-edit-modal{width:min(1180px,calc(100vw - 30px))}.qp-edit-kicker{margin-bottom:5px;color:#0f8a83;font-size:10px;font-weight:950;letter-spacing:.12em}.qp-edit-status{display:inline-flex;align-items:center;margin-top:9px;padding:6px 9px;border-radius:999px;background:#ecfdf5;color:#047857;font-size:10px;font-weight:900}.qp-edit-modal .qp-field label{font-size:11px;color:#334155}.qp-edit-modal .qp-field input,.qp-edit-modal .qp-field select,.qp-edit-modal .qp-field textarea{font-size:12px}.qp-edit-modal .qp-readonly{background:#f1f5f9;color:#475569;font-weight:850}.qp-grid.qp-edit-items{grid-template-columns:1.3fr .72fr .7fr .64fr .58fr .72fr .9fr}.qp-edit-note{margin:-1px 0 10px;color:#64748b;font-size:10px;font-weight:700}.qp-modal-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;align-items:flex-start;padding:17px 20px;background:#fff;border-bottom:1px solid #e2e8f0}.qp-modal-head h2{margin:0;color:#0f172a;font-size:18px}.qp-modal-head p{margin:4px 0 0;color:#64748b;font-size:10px}.qp-close{width:32px;height:32px;border:0;border-radius:8px;background:#f1f5f9;color:#475569;font-size:18px;cursor:pointer}.qp-body{padding:17px 20px}.qp-section{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:14px;margin-bottom:11px}.qp-section-title{display:flex;justify-content:space-between;align-items:center;margin-bottom:11px}.qp-section-title h3{margin:0;font-size:13px;color:#172033}.qp-section-title span{font-size:9px;color:#94a3b8}.qp-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.qp-grid.items{grid-template-columns:1.35fr .75fr .75fr .7fr .65fr .8fr}.qp-field label{display:block;margin-bottom:5px;color:#475569;font-size:9px;font-weight:900}.qp-field input,.qp-field select,.qp-field textarea{width:100%;border:1px solid #cbd5e1;border-radius:7px;background:#fff;color:#111827;padding:0 9px;font-size:11px;outline:none}.qp-field input,.qp-field select{height:35px}.qp-field textarea{min-height:62px;padding-top:8px;resize:vertical}.qp-field input:focus,.qp-field select:focus,.qp-field textarea:focus{border-color:#14b8a6;box-shadow:0 0 0 2px rgba(20,184,166,.12)}.qp-checks{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:10px}.qp-checks label{display:flex;align-items:center;gap:5px;padding:7px 9px;border:1px solid #cbd5e1;border-radius:999px;color:#334155;font-size:9px;font-weight:800;background:#fff}.qp-summary{display:grid;grid-template-columns:repeat(3,1fr);margin-top:11px;border:1px solid #dbe3ec;border-radius:8px;overflow:hidden}.qp-summary div{padding:10px 12px;background:#f8fafc;border-right:1px solid #dbe3ec}.qp-summary div:last-child{border:0;background:#0f1d32;color:#fff}.qp-summary small{display:block;color:#334155;font-size:11px;font-weight:900;line-height:1.35}.qp-summary div:last-child small{color:#e2e8f0}.qp-summary b{display:block;margin-top:4px;font-size:14px;font-weight:900;line-height:1.35}.qp-modal-foot{position:sticky;bottom:0;display:flex;justify-content:flex-end;gap:7px;padding:12px 20px;background:#fff;border-top:1px solid #e2e8f0}.qp-drawer-bg{position:fixed;inset:0;z-index:14070;background:rgba(15,23,42,.3)}.qp-drawer{position:absolute;right:0;top:0;bottom:0;width:min(470px,95vw);overflow:auto;background:#fff;box-shadow:-20px 0 55px rgba(15,23,42,.2);padding:20px}.qp-drawer h2{margin:0 0 5px;font-size:17px}.qp-detail{margin-top:14px;border:1px solid #e2e8f0;border-radius:9px;overflow:hidden}.qp-detail div{display:flex;justify-content:space-between;gap:15px;padding:9px 11px;border-bottom:1px solid #edf2f7;font-size:10px}.qp-detail div:last-child{border:0}.qp-detail span{color:#64748b}.qp-detail b{text-align:right;color:#1e293b}.qp-drawer-buttons{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:13px}.qp-root .qerp-status.teal{background:#dff8f4;color:#0f766e}.qp-root .qerp-btn:not(.ghost){background:#0f8a83}.qp-root .qerp-btn:not(.ghost):hover{background:#0b746f}.qp-root .qerp-btn.ghost{background:#fff;color:#334155}.qp-root .qp-modal-foot .qerp-btn:not(.ghost),.qp-root .qp-drawer-buttons .qerp-btn:not(.ghost){background:#0f8a83}@media(max-width:1050px){.qp-kpis{grid-template-columns:1fr 1fr}.qp-grid,.qp-grid.items{grid-template-columns:1fr 1fr}.qp-flow{overflow:auto}.qp-stage{min-width:150px}}@media(max-width:650px){.qp-kpis,.qp-grid,.qp-grid.items{grid-template-columns:1fr}.qp-toolbar{flex-wrap:wrap}.qp-toolbar input{min-width:100%;width:100%}.qp-modal-bg{padding:6px}.qp-modal{width:100%;max-height:calc(100vh - 12px)}}";
      document.head.appendChild(style);
    },[]);
    ...
    const closeEdit=()=>{if(editSaving)return;setSelected(null);setEditForm(null);setEditError("");};
    const openEdit=row=>{
      setSelected(row);setEditError("");
      setEditForm({
        purchaseNo:rowNo(row),purchaseType:row.purchaseType||row.type||"정기발주",productionType:row.productionType||"D-양산",
        orderDate:row.orderDate||today(),supplier:row.supplier||"",supplierGrade:row.supplierGrade||row.grade||"",
        warehouse:row.warehouse||"",terms:row.paymentTerms||row.terms||"",priority:row.priority||"일반",
        due:rowDue(row),expected:rowExpected(row),mrp:row.mrpNo||row.mrp||"",workOrderNo:row.workOrderNo||"",
        item:rowItem(row),itemCode:row.itemCode||"",spec:row.spec||"",qty:String(row.qty??""),unit:row.unit||"kg",
        price:String(row.unitPrice??row.price??""),supply:String(row.amount!=null?number(row.amount):number(row.qty)*number(row.unitPrice??row.price)),
        iqcRequired:row.iqcRequired!==false,coaRequired:row.coaRequired!==false,msdsRequired:!!row.msdsRequired,lotRequired:row.lotRequired!==false,
        deliveryAddress:row.deliveryAddress||"",requester:row.requester||row.owner||"",notes:row.notes||""
      });
    };
    const editItemChange=(key,value)=>setEditForm(current=>{
      const next={...current,[key]:value};
      if(key==="qty"||key==="price"){
        const qty=number(key==="qty"?value:current.qty),price=number(key==="price"?value:current.price);
        next.supply=String(qty*price);
      }
      return next;
    });
    const submitEdit=async event=>{
      ...
      setEditError("");
      const qty=number(editForm.qty),price=number(editForm.price),supply=number(editForm.supply);
      const imported=clean(editForm.purchaseType)==="ERP 이관";
      if(!clean(editForm.supplier)||!clean(editForm.item)||qty<=0||price<0||supply<0||!validDate(editForm.orderDate)||(!imported&&!validDate(editForm.due))){
        setEditError(imported?"협력사·품목·발주수량·발주일을 확인하세요.":"협력사·품목·발주수량·발주일·요청납기를 확인하세요.");return;
      }
      if(clean(editForm.due)&&!validDate(editForm.due)){setEditError("요청납기일 형식을 확인하세요.");return;}
      if(clean(editForm.due)&&editForm.due<editForm.orderDate){setEditError("요청납기일은 발주일보다 빠를 수 없습니다.");return;}
      if(clean(editForm.expected)&&!validDate(editForm.expected)){setEditError("협력사 확정 납기일을 확인하세요.");return;}
      const payload={...selected,
        id:rowNo(selected),purchaseNo:rowNo(selected),purchaseType:editForm.purchaseType,productionType:editForm.productionType,
        supplier:clean(editForm.supplier),supplierGrade:clean(editForm.supplierGrade),
        item:clean(editForm.item),material:clean(editForm.item),itemCode:clean(editForm.itemCode),spec:clean(editForm.spec),
        qty,unit:editForm.unit,unitPrice:price,price,amount:supply,
        orderDate:editForm.orderDate,requestedDueDate:clean(editForm.due),due:clean(editForm.due),
        confirmedDueDate:clean(editForm.expected),expected:clean(editForm.expected),priority:editForm.priority,
        mrpNo:clean(editForm.mrp),mrp:clean(editForm.mrp),workOrderNo:clean(editForm.workOrderNo),
        warehouse:clean(editForm.warehouse),paymentTerms:clean(editForm.terms),terms:clean(editForm.terms),
        iqcRequired:editForm.iqcRequired,coaRequired:editForm.coaRequired,msdsRequired:editForm.msdsRequired,lotRequired:editForm.lotRequired,
        deliveryAddress:clean(editForm.deliveryAddress),requester:clean(editForm.requester),owner:clean(editForm.requester),
        notes:clean(editForm.notes),updatedAt:new Date().toISOString(),updatedBy:currentUserName()
      };
      setEditSaving(true);
      try{
        const response=await fetch("/api/purchase-orders/"+encodeURIComponent(rowNo(selected)),{method:"PUT",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
        const result=await response.json();
        if(!response.ok||!result.success)throw new Error(result.message||"수정 저장 실패");
        const changed={...selected,...payload,...(result.data||{})};
        const next=rows.map(item=>rowNo(item)===rowNo(selected)?changed:item);
        setRows(next);writeLocal("purchase",next);
        window.dispatchEvent(new CustomEvent("qmes:erp-data-changed",{detail:{kind:"purchase"}}));
        setSelected(null);setEditForm(null);
      }catch(apiError){setEditError(apiError.message||"구매 발주를 수정하지 못했습니다.");}
    ...
    const submit=async event=>{
      ...
      await persist([nextRow,...rows]);setPage(1);setOpen(false);reset();
    };
    const deleteOrder=async row=>{
      if(!isAdmin){window.alert("관리자만 구매 발주를 삭제할 수 있습니다.");return;}
      if(!window.confirm(rowNo(row)+" 구매 발주를 완전히 삭제하시겠습니까?\n삭제 후 복구할 수 없습니다."))return;
      try{
        const response=await fetch("/api/purchase-orders/"+encodeURIComponent(rowNo(row)),{method:"DELETE",credentials:"same-origin"});
        const result=await response.json();
        if(!response.ok||!result.success)throw new Error(result.message||"삭제 실패");
        const next=rows.filter(item=>rowNo(item)!==rowNo(row));
        setRows(next);writeLocal("purchase",next);setSelected(null);
        window.dispatchEvent(new CustomEvent("qmes:erp-data-changed",{detail:{kind:"purchase"}}));
      }catch(error){window.alert(error.message||"구매 발주를 삭제하지 못했습니다.");}
    };
    ...
    const printOrder=row=>{
      ...
      const supply=rowAmount(row),vat=rowVat(row),total=rowTotal(row);
    ...
    return <div className="qerp qp-root">
      ...
      <div className="qerp-card">
        ...
        <div className="qp-toolbar"><input type="search" value={query} onChange={event=>{setQuery(event.target.value);setPage(1);}} placeholder="발주번호, 협력사, 품목, MRP 검색"/><select value={statusFilter} onChange={event=>{setStatusFilter(event.target.value);setPage(1);}}><option value="all">전체 상태</option><option>결재대기</option><option>발주확정</option><option>납기임박</option><option>입고지연</option><option>부분입고</option><option>IQC대기</option><option>입고완료</option><option>IQC 부적합</option><option>발주취소</option></select><button type="button" className="qerp-btn ghost" onClick={()=>{setQuery("");setStatusFilter("all");setPage(1);}}>초기화</button><span className="qp-count">총 {visible.length}건</span></div>
        <div className="qerp-table-wrap"><table className="qerp-table"><thead><tr><th>발주번호 / 생산구분</th><th>협력사</th><th>품목 / 규격</th><th>발주수량</th><th>공급가액 / 부가세·합계</th><th>발주일 / 요청납기</th><th>결재</th><th>입고 · IQC</th><th>상태</th><th>관리</th></tr></thead><tbody>
          {pageRows.map(row=>{const state=stateFor(row);return <tr key={rowNo(row)}><td><button type="button" className="qp-po-link" onClick={()=>openEdit(row)} title="구매 발주 수정">{rowNo(row)}</button><div className="qerp-muted">{row.productionType||"D-양산"} · {row.purchaseType||row.type||"정기발주"}</div></td><td><b>{row.supplier||"-"}</b><div className="qerp-muted">{row.supplierGrade||row.grade||"-"}</div></td><td><b>{rowItem(row)||"-"}</b><div className="qerp-muted">{row.itemCode||row.spec||"-"}</div></td><td><b>{rowQtyText(row)}</b></td><td>{rowAmount(row)>0?<><b>{fmtWon(rowAmount(row))}</b><div className="qerp-muted">VAT {fmtWon(rowVat(row))} · 합계 {fmtWon(rowTotal(row))}</div></>:"미입력"}</td><td>{row.orderDate||"-"}<div className="qerp-muted">납기 {rowDue(row)||"-"}</div></td><td><StatusPill value={row.approvalStatus||row.approval||"구매검토"}/></td><td>{rowReceiptText(row)}<div className="qerp-muted">IQC · {row.iqcStatus||row.iqc||"계획 대기"}</div></td><td><StatusPill value={state}/></td><td><div className="qp-actions">{isAdmin&&<button type="button" className="qp-mini danger" onClick={()=>deleteOrder(row)}>삭제</button>}<button type="button" className="qp-mini" onClick={()=>printOrder(row)}>인쇄</button></div></td></tr>;})}
        ...
        {visible.length>PAGE_SIZE&&<div className="qp-pagination" aria-label="구매 발주 페이지">
          <button type="button" onClick={()=>setPage(current=>Math.max(1,current-1))} disabled={page===1}>이전</button>
          {Array.from({length:pageCount},(_,index)=>index+1).map(number=><button type="button" key={number} className={page===number?"active":""} aria-current={page===number?"page":undefined} onClick={()=>setPage(number)}>{number}</button>)}
          <button type="button" onClick={()=>setPage(current=>Math.min(pageCount,current+1))} disabled={page===pageCount}>다음</button>
      ...
      {open&&<div className="qp-modal-bg" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false);}}><form className="qp-modal" onSubmit={submit}>
        ...
        <div className="qp-body">
          ...
          <section className="qp-section"><div className="qp-section-title"><h3>3. 입고 · 품질 요구사항</h3><span>발주서 및 IQC 계획에 반영</span></div><div className="qp-checks">
            ...
            <div className="qp-field"><label>납품 장소</label><input value={form.deliveryAddress} onChange={event=>setForm({...form,deliveryAddress:event.target.value})} placeholder="납품 장소 직접 입력"/></div>
      ...
      {selected&&editForm&&<div className="qp-modal-bg" onMouseDown={event=>{if(event.target===event.currentTarget)closeEdit();}}><form className="qp-modal qp-edit-modal" onSubmit={submitEdit}>
        <div className="qp-modal-head"><div><div className="qp-edit-kicker">NAMO CHEMICAL · PROCUREMENT</div><h2>구매 발주 수정</h2><p>발주정보와 품목·금액·납품·품질 조건을 수정합니다.</p><span className="qp-edit-status">{rowNo(selected)} · {stateFor(selected)}</span></div><button type="button" className="qp-close" onClick={closeEdit} disabled={editSaving}>×</button></div>
        <div className="qp-body">
          <section className="qp-section"><div className="qp-section-title"><h3>1. 구매 기본 정보</h3><span>* 필수 입력</span></div>
            <div className="qp-grid">
              <div className="qp-field"><label>발주번호</label><input className="qp-readonly" value={editForm.purchaseNo} readOnly/></div>
              <div className="qp-field"><label>발주일 *</label><input type="date" value={editForm.orderDate} onChange={event=>setEditForm({...editForm,orderDate:event.target.value})}/></div>
              <div className="qp-field"><label>구매 구분</label><select value={editForm.purchaseType} onChange={event=>setEditForm({...editForm,purchaseType:event.target.value})}><option>MRP 자동발주</option><option>정기발주</option><option>긴급발주</option><option>ERP 이관</option></select></div>
              <div className="qp-field"><label>생산 구분</label><select value={editForm.productionType} onChange={event=>setEditForm({...editForm,productionType:event.target.value})}><option>D-양산</option><option>C-Pilot</option><option>B-Lab</option></select></div>
              <div className="qp-field"><label>협력사 *</label><input value={editForm.supplier} onChange={event=>setEditForm({...editForm,supplier:event.target.value})} placeholder="실제 협력사명"/></div>
              <div className="qp-field"><label>입고 창고</label><input value={editForm.warehouse} onChange={event=>setEditForm({...editForm,warehouse:event.target.value})} placeholder="입고 창고 직접 입력"/></div>
              <div className="qp-field"><label>결제 조건</label><input value={editForm.terms} onChange={event=>setEditForm({...editForm,terms:event.target.value})} placeholder="예: 월 마감 후 30일"/></div>
              <div className="qp-field"><label>납기 우선순위</label><select value={editForm.priority} onChange={event=>setEditForm({...editForm,priority:event.target.value})}><option>일반</option><option>긴급</option><option>최우선</option></select></div>
              <div className="qp-field"><label>요청납기 {editForm.purchaseType==="ERP 이관"?"":"*"}</label><input type="date" value={editForm.due} onChange={event=>setEditForm({...editForm,due:event.target.value})}/></div>
              <div className="qp-field"><label>협력사 확정 납기</label><input type="date" value={editForm.expected} onChange={event=>setEditForm({...editForm,expected:event.target.value})}/></div>
              <div className="qp-field"><label>연결 MRP / 구매요청</label><input value={editForm.mrp} onChange={event=>setEditForm({...editForm,mrp:event.target.value})} placeholder="MRP- 또는 PR-"/></div>
              <div className="qp-field"><label>연결 작업지시</label><input value={editForm.workOrderNo} onChange={event=>setEditForm({...editForm,workOrderNo:event.target.value})} placeholder="WO-"/></div>
            </div>
          </section>
          <section className="qp-section"><div className="qp-section-title"><h3>2. 발주 품목 및 금액</h3><span>부가세 10% 자동 계산</span></div>
            <div className="qp-grid qp-edit-items">
              <div className="qp-field"><label>품목명 *</label><input value={editForm.item} onChange={event=>setEditForm({...editForm,item:event.target.value})} placeholder="원료·포장재명"/></div>
              <div className="qp-field"><label>품목코드</label><input value={editForm.itemCode} onChange={event=>setEditForm({...editForm,itemCode:event.target.value})} placeholder="RM- / PK-"/></div>
              <div className="qp-field"><label>규격</label><input value={editForm.spec} onChange={event=>setEditForm({...editForm,spec:event.target.value})}/></div>
              <div className="qp-field"><label>발주수량 *</label><input inputMode="decimal" value={editForm.qty} onChange={event=>editItemChange("qty",event.target.value)}/></div>
              <div className="qp-field"><label>단위</label><select value={editForm.unit} onChange={event=>setEditForm({...editForm,unit:event.target.value})}><option>kg</option><option>g</option><option>L</option><option>EA</option><option>롤</option><option>박스</option></select></div>
              <div className="qp-field"><label>단가 (원)</label><input inputMode="decimal" value={editForm.price} onChange={event=>editItemChange("price",event.target.value)}/></div>
              <div className="qp-field"><label>공급가액 (원)</label><input inputMode="decimal" value={editForm.supply} onChange={event=>setEditForm({...editForm,supply:event.target.value})}/></div>
            </div>
            <div className="qp-summary"><div><small>공급가액</small><b>{fmtWon(number(editForm.supply))}</b></div><div><small>부가세 10%</small><b>{fmtWon(Math.round(number(editForm.supply)*.1))}</b></div><div><small>합계금액</small><b>{fmtWon(number(editForm.supply)+Math.round(number(editForm.supply)*.1))}</b></div></div>
          </section>
          <section className="qp-section"><div className="qp-section-title"><h3>3. 입고 · 품질 요구사항</h3><span>발주서 및 IQC 계획에 반영</span></div>
            <div className="qp-checks">
              <label><input type="checkbox" checked={editForm.iqcRequired} onChange={event=>setEditForm({...editForm,iqcRequired:event.target.checked})}/>IQC 필수</label>
              <label><input type="checkbox" checked={editForm.coaRequired} onChange={event=>setEditForm({...editForm,coaRequired:event.target.checked})}/>CoA 동봉</label>
              <label><input type="checkbox" checked={editForm.msdsRequired} onChange={event=>setEditForm({...editForm,msdsRequired:event.target.checked})}/>MSDS 동봉</label>
              <label><input type="checkbox" checked={editForm.lotRequired} onChange={event=>setEditForm({...editForm,lotRequired:event.target.checked})}/>제조 LOT·유효기한 표시</label>
            </div>
            <div className="qp-grid">
              <div className="qp-field"><label>납품 장소</label><input value={editForm.deliveryAddress} onChange={event=>setEditForm({...editForm,deliveryAddress:event.target.value})} placeholder="납품 장소 직접 입력"/></div>
              <div className="qp-field"><label>협력사 등급</label><input value={editForm.supplierGrade} onChange={event=>setEditForm({...editForm,supplierGrade:event.target.value})} placeholder="예: A등급"/></div>
              <div className="qp-field"><label>구매 담당 / 원가부서</label><input value={editForm.requester} onChange={event=>setEditForm({...editForm,requester:event.target.value})} placeholder="담당자 · 부서"/></div>
              <div className="qp-field"><label>특기사항</label><textarea value={editForm.notes} onChange={event=>setEditForm({...editForm,notes:event.target.value})} placeholder="CoA 사전 송부, 포장·납품 조건 등"/></div>
            </div>
          </section>
          {editError&&<div className="qerp-error">{editError}</div>}
        </div>
        <div className="qp-modal-foot"><button type="button" className="qerp-btn ghost" onClick={closeEdit} disabled={editSaving}>취소</button><button type="button" className="qerp-btn ghost" onClick={()=>printOrder({...selected,...editForm,amount:number(editForm.supply),unitPrice:number(editForm.price)})}>발주서 인쇄</button><button type="submit" className="qerp-btn" disabled={editSaving}>{editSaving?"저장 중...":"수정 저장"}</button></div>
      </form></div>}
    </div>;
  }
