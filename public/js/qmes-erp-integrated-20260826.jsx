  ...
  function QMESErpPurchaseTab(){
    ...
    return <div className="qerp qp-root">
      ...
      {selected&&<div className="qp-drawer-bg" onMouseDown={event=>{if(event.target===event.currentTarget)setSelected(null);}}><aside className="qp-drawer">
        <div style={{display:"flex",justifyContent:"space-between",gap:"10px"}}><div><div className="qerp-muted">PURCHASE ORDER DETAIL</div><h2>{rowNo(selected)} · {selected.supplier}</h2><div className="qerp-muted">{rowItem(selected)} · {rowQtyText(selected)} · 납기 {rowDue(selected)||"미입력"}</div></div><button type="button" className="qp-close" onClick={()=>setSelected(null)}>×</button></div>
        <div style={{marginTop:"13px"}}><StatusPill value={stateFor(selected)}/></div>
        <div className="qp-detail"><div><span>구매 / 생산 구분</span><b>{selected.purchaseType||selected.type||"-"} · {selected.productionType||"D-양산"}</b></div><div><span>MRP / 작업지시</span><b>{selected.mrpNo||selected.mrp||"-"} · {selected.workOrderNo||"-"}</b></div><div><span>품목 / 규격</span><b>{rowItem(selected)} · {selected.itemCode||selected.spec||"-"}</b></div><div><span>공급가액</span><b>{rowAmount(selected)>0?fmtWon(rowAmount(selected)):"미입력"}</b></div><div><span>부가세 10%</span><b>{rowAmount(selected)>0?fmtWon(rowVat(selected)):"미입력"}</b></div><div><span>합계금액</span><b>{rowAmount(selected)>0?fmtWon(rowTotal(selected)):"미입력"}</b></div><div><span>요청 / 확정 납기</span><b>{rowDue(selected)||"-"} · {rowExpected(selected)||"미확정"}</b></div><div><span>입고수량</span><b>{rowReceiptText(selected)}</b></div><div><span>원료 LOT</span><b>{selected.materialLot||selected.lot||"입고 시 생성"}</b></div><div><span>IQC</span><b>{selected.iqcStatus||selected.iqc||"계획 대기"}</b></div><div><span>결제조건</span><b>{selected.paymentTerms||selected.terms||"-"}</b></div><div><span>담당</span><b>{selected.requester||selected.owner||"-"}</b></div></div>
        <div className="qp-drawer-buttons"><button type="button" className="qerp-btn ghost" onClick={()=>printOrder(selected)}>구매 발주서 인쇄</button>{number(selected.qty)>0&&!/입고완료/.test(stateFor(selected))?<button type="button" className="qerp-btn" onClick={()=>registerReceipt(selected)}>입고 등록</button>:<button type="button" className="qerp-btn" disabled style={{opacity:.55,cursor:"default"}}>입고 완료</button>}<button type="button" className="qerp-btn ghost" style={{gridColumn:"1/-1",color:"#b91c1c"}} onClick={()=>cancelOrder(selected)}>발주 취소</button></div>
      </aside></div>}
    </div>;
  }
