...
function IssueWoTab() {
  ...
  const [materialMasterVersion, setMaterialMasterVersion] = useState(0);
  const issueFormRef = React.useRef(null);

  useEffect(() => {
    const refreshMaterialMaster = () => setMaterialMasterVersion((version) => version + 1);
    window.addEventListener("qmes:material-master-changed", refreshMaterialMaster);
    if (window.qmesMaterialNameMaster?.pull) {
      window.qmesMaterialNameMaster.pull().catch((error) => {
        console.warn("원료마스터 공용 동기화 실패:", error.message);
      });
    }
    return () => window.removeEventListener("qmes:material-master-changed", refreshMaterialMaster);
  }, []);
  ...
  const blankPlanItems = (product) => {
    ...
    return (BOM[safeProduct]?.items || []).map((it) => ({
      ...it,
      materialCode:window.qmesMaterialNameMaster?.getByName?.(it.name)?.code || it.materialCode || "",
      materialLot: "", containerNo: "", inputStatus: "신규",
  ...
  const managedRawMaterialNames = (window.qmesMaterialNameMaster?.listActive?.() || []).map((row) => row.name);
  const rawMaterialNames = Array.from(new Set([
    ...MATERIAL_OPTIONS.filter((name) => !String(name).includes("중간배치")),
    ...managedRawMaterialNames
  ]));
  const availableMaterialOptions = rawMaterialNames
    .concat(allowedIntermediateMaterials.length ? ["중간배치 선택", ...allowedIntermediateMaterials] : []);
  void materialMasterVersion;

  const saveTypedMaterialNameForRow = async (rowIndex, typedName) => {
    const entered = String(typedName || "").trim().replace(/\s+/g, " ");
    if (!entered) return;
    const existing = window.qmesMaterialNameMaster?.getByName?.(entered);
    if (existing) {
      setPlanItems((current) => current.map((row, index) => index === rowIndex ? {
        ...row, name:existing.name, materialCode:existing.code
      } : row));
      return;
    }
    if (qmesMaterialType(entered) === "중간재") return;
    try {
      const material = await window.qmesMaterialNameMaster.add(entered);
      setPlanItems((current) => current.map((row, index) => index === rowIndex ? {
        ...row,
        name:material.name,
  ...
  const issue = async () => {
    ...
    try {
      for (const item of planItems) {
        const typedName = String(item.name || "").trim();
        if (!typedName || qmesMaterialType(typedName) === "중간재") continue;
        if (!window.qmesMaterialNameMaster?.getByName?.(typedName)) {
          await window.qmesMaterialNameMaster.add(typedName);
        }
      }
    } catch (error) {
      window.alert(`수기 입력한 원료명을 저장하지 못했습니다.\n${error.message}`);
      return;
    }
    ...
    DB.woDocs[woNo] = {
      ...
      inputs: planItems.map((it, index) => {
        ...
        const materialMaster = window.qmesMaterialNameMaster?.getByName?.(it.name);
        return {
          seq: index + 1,
          materialCode:it.materialCode || materialMaster?.code || "",
          name:it.name,
  ...
  const editWo = (r) => {
    ...
    setPlanItems((d.inputs?.length ? d.inputs : productBom.items).map((it, i) => ({
      seq:i+1,
      name:it.name,
      materialCode:it.materialCode || window.qmesMaterialNameMaster?.getByName?.(it.name)?.code || "",
      materialLot:it.materialLot || it.lot || "",
      containerNo:it.containerNo || "", inputStatus:it.inputStatus || "신규",
            ...
            <table className="qmes-material-table qmes-material-balanced qmes-wo-material-compact w-full text-sm min-w-[1400px]">
              ...
              <tbody>
                {planItems.map((it, idx) => {
                  ...
                  return (
                  <tr key={`material-row-${idx}`} className="border-b border-slate-800/60 align-top">
                    <td className="py-1.5 pr-3 text-slate-500 tabular-nums">{idx + 1}</td>
                    <td className="py-1.5 pr-3">
                      <input
                        list={`qmes-material-options-${idx}`}
                        value={it.name}
                        onChange={(e) => {
                        const nextName = e.target.value;
                        const master = window.qmesMaterialNameMaster?.getByName?.(nextName);
                        ...
                        onBlur={(e) => saveTypedMaterialNameForRow(idx, e.currentTarget.value)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                        ...
                        placeholder="원료명 선택 또는 직접 입력"
                        autoComplete="off"
                        className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500"
                      />
                      <datalist id={`qmes-material-options-${idx}`}>
                        {availableMaterialOptions
                          .filter((name) => name !== "중간배치 선택")
                ...
                <tr>
                  ...
                  <td className="py-1.5 text-right">
                    <button onClick={() => {
                      const firstName = availableMaterialOptions[0];
                      const firstMaster = window.qmesMaterialNameMaster?.getByName?.(firstName);
                      setPlanItems([...planItems, {
                        seq:planItems.length + 1,
                        name:firstName,
                        materialCode:firstMaster?.code || "",
                        materialLot:"",
                        containerNo:"",
                        inputStatus:"신규",
                        availableQty:"",
