...
(function(global){
  'use strict';
  if(global.__QMES_PARTNER_REGISTER_MODAL_V3__) return;
  global.__QMES_PARTNER_REGISTER_MODAL_V3__=true;
  ...
  const typeOf=button=>{const label=text(button&&button.textContent);if(label.includes('고객사 등록'))return'customer';if(label.includes('공급업체 등록'))return'supplier';return'';};
  ...
  style.id='qmes-partner-register-modal-v3-style';
  ...
  function openCustomer(){
    ...
    const code=nextCode('CUS',rows);const codeEl=input('',code);codeEl.readOnly=true;codeEl.className='qpr-code';const name=input('고객사명');const manager=input('담당자명');const phone=input('연락처');const status=select();const note=document.createElement('textarea');note.placeholder='비고 (선택)';
    m.body.append(field('고객사 코드',codeEl),field('고객사명',name),field('담당자',manager),field('연락처',phone),field('거래상태',status),field('비고',note,true));
    m.save.onclick=()=>{const customer=text(name.value);if(!customer){alert('고객사명을 입력하세요.');name.focus();return;}if(rows.some(r=>text(r.name).toLowerCase()===customer.toLowerCase())){alert('이미 등록된 고객사입니다.');return;}rows.push({code,name:customer,manager:text(manager.value),phone:text(phone.value),status:status.value||'거래중',note:text(note.value)});db.partnerCustomers=rows;saveDb();close();setTimeout(()=>location.reload(),80);};requestAnimationFrame(()=>name.focus());
  }

  function openSupplier(){
    const db=getDb();
    const m=baseModal('공급업체 등록');
    if(!db){m.body.innerHTML='<div style="grid-column:1/-1;color:#fca5a5;font-weight:800">데이터 저장소를 불러오지 못했습니다. 새로고침 후 다시 시도해 주세요.</div>';m.save.disabled=true;return;}
    ...
    const code=nextCode('SUP',rows);const codeEl=input('',code);codeEl.readOnly=true;codeEl.className='qpr-code';const company=input('공급업체명');const material=input('원료명');const lot=input('최근 원료 LOT No.');const manager=input('담당자명');const phone=input('연락처');const status=select();const note=document.createElement('textarea');note.placeholder='비고 (선택)';
    ...
    m.save.onclick=()=>{const c=text(company.value),mat=normalizeMaterial(material.value);if(!c||!mat){alert('공급업체명과 원료명을 입력하세요.');return;}rows.push({code,company:c,material:mat,lot:text(lot.value).toUpperCase(),manager:text(manager.value),phone:text(phone.value),status:status.value||'거래중',note:text(note.value)});db.partnerSuppliers=rows;saveDb();close();setTimeout(()=>location.reload(),80);};requestAnimationFrame(()=>company.focus());
  }
  ...
  function handle(event){
    ...
    const type=typeOf(node);if(!type)return;
    event.preventDefault();event.stopPropagation();
    if(type==='customer')openCustomer();else openSupplier();
  }
