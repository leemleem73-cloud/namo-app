...
(function(global){
  ...
  function findEditRow(button){
    ...
    const tr=button.closest('tr'),db=getDb();if(!tr||!db)return null;
    const rowText=text(tr.textContent),customers=Array.isArray(db.partnerCustomers)&&db.partnerCustomers.length?db.partnerCustomers:defaultCustomers,suppliers=Array.isArray(db.partnerSuppliers)&&db.partnerSuppliers.length?db.partnerSuppliers:defaultSuppliers;
  ...
  function handle(event){
    const button=event.target&&event.target.closest?event.target.closest('button'):null;if(!button)return;
    const registerType=registerTypeOf(button),isEdit=text(button.textContent)==='수정';if(!registerType&&!isEdit)return;
    block(event);
    const now=Date.now();if(button===lastButton&&now-lastAt<300)return;lastButton=button;lastAt=now;
    if(registerType){registerType==='customer'?openCustomer():openSupplier();return;}
    const target=findEditRow(button);if(!target)return;target.type==='customer'?openCustomer(target.row):openSupplier(target.row);
  }
