/* QMES FIXED CALENDAR - NATIVE STYLE V2 - 2026-09-22
...
(function(){
  "use strict";
  if(window.__QMES_FIXED_CALENDAR_NATIVE_STYLE_20260922_V2__) return;
  window.__QMES_FIXED_CALENDAR_NATIVE_STYLE_20260922_V2__=true;
  ...
  function renderYears(pop){
    if(!yearPageStart) yearPageStart=Math.max(2000,viewYear-2);
    const years=[];
    for(let y=yearPageStart;y<yearPageStart+7;y++){
      years.push('<button type="button" class="qnv-year'+(y===viewYear?' current':'')+'" data-qnv-year="'+y+'">'+y+'</button>');
    }
    ...
    pop.innerHTML=
      ...
      '</div>'+
      '<div class="qnv-years">'+years.join("")+'</div>'+
      '<div class="qnv-foot">'+
        '<button type="button" class="qnv-action" data-qnv-clear>삭제</button>'+
        '<button type="button" class="qnv-action" data-qnv-today>오늘</button>'+
      '</div>';
  ...
  document.addEventListener("click",event=>{
    ...
    if(year){
      viewYear=Number(year.getAttribute("data-qnv-year"))||viewYear;
      mode="days";
      render();
      return;
    }
    ...
    if(t.closest("[data-qnv-year-prev]")){
      yearPageStart=Math.max(2000,yearPageStart-5);
      render();
      return;
    }
    if(t.closest("[data-qnv-year-next]")){
      yearPageStart=Math.min(2094,yearPageStart+5);
      render();
      return;
    }
  ...
  const observer=new MutationObserver(records=>{
    for(const record of records){
      for(const node of record.addedNodes||[]){
        ...
        if(OLD_POP_IDS.includes(node.id)){
          node.remove();
          continue;
        }
