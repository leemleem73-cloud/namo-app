/* Namo Chemical Q-MES Enterprise Inventory auto mode v7, 2026-08-20 */
...
function InventoryEnterpriseTab({section='overview'}){
  ...
  return <div className="inv-shell">
    ...
    {loading?<div className="inv-loading">재고 데이터를 불러오는 중...</div>:<>
      ...
      {section==='movement'&&<div className="inv-panel"><h3>자동 입출고 처리 내역</h3><TxTable rows={movementRows}/>{!movementRows.length&&<div className="inv-empty">입출고 처리 내역이 없습니다.</div>}</div>}
...
function TxTable({rows}){return <table className="inv-movement-table"><colgroup><col/><col/><col/><col/><col/><col/><col/></colgroup><thead><tr><th>일시</th><th>구분</th><th>품목</th><th>LOT</th><th>수량</th><th>이동 방향</th><th>비고</th></tr></thead><tbody>{rows.map(tx=><tr key={tx.id}><td>{new Date(tx.created_at).toLocaleString('ko-KR')}</td><td>{INV_TYPE_LABEL[tx.transaction_type]||tx.transaction_type}</td><td>{tx.item_code}</td><td>{tx.lot_no}</td><td className="num">{invNum(tx.quantity)} {tx.unit}</td><td>{txDirectionLabel(tx)}</td><td title={txDisplayReference(tx)}>{txDisplayReference(tx)}</td></tr>)}</tbody></table>}
