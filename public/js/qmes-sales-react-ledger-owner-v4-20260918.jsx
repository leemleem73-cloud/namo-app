...
(function(){
  "use strict";
  if(window.__QMES_SALES_REACT_LEDGER_OWNER_V4_20260918__) return;
  window.__QMES_SALES_REACT_LEDGER_OWNER_V4_20260918__=true;
  ...
  function StableSalesLedgerV4V4(){
    ...
    return <div className="qmes-sales-ledger-v4">
      <div className="qslv4-head">
        <div><h1 className="qslv4-title">수주·납기 관리대장</h1><div className="qslv4-sub">총 {data.length}건</div></div>
        <div className="qslv4-head-actions">
          <span className="qslv4-sync">공용 DB 연동</span>
          <button id="qmes-sales-progress-button-20260826" type="button" onClick={()=>data[0]&&openDetail(data[0].id)}>수주 진행현황</button>
          <button type="button" className="qslv4-new-btn" onClick={openNew}>+ 신규 수주</button>
        </div>
      </div>
      ...
      <div className="qrl-list"><div className="qrl-wrap"><table><thead><tr><th>수주일 ↓</th><th>수주번호</th><th>거래처명</th><th>품목명 (규격)</th><th>수주수량</th><th>단위</th><th>요청납기</th><th>납기상태</th><th>생산상태</th><th>출하상태</th><th>관리</th></tr></thead><tbody>
        ...
        </tr>):<tr><td colSpan="11" className="qrl-empty">조회 조건에 해당하는 수주가 없습니다.</td></tr>}
      </tbody></table></div>
  ...
  function install(){
    if(window.QMESErpSalesTab===StableSalesLedgerV4) return;
    window.QMESErpSalesTab=StableSalesLedgerV4;
    window.dispatchEvent(new CustomEvent("qmes:erp-integrated-ready"));
  }

  /* Single deterministic install only. No delayed timers, no late UI replacement. */
  install();
})();
