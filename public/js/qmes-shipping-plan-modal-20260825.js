(function(){
  ...
  function openModal(){
    ...
    overlay.innerHTML=`<div class="qsp-modal" role="dialog" aria-modal="true" aria-label="출하계획 등록"><div class="qsp-head"><h2>출하계획 등록</h2><button type="button" class="qsp-close" aria-label="닫기">×</button></div><form id="qsp-form"><div class="qsp-grid"><div class="qsp-field"><label>출하 예정일</label><input name="shipDate" type="date" required></div><div class="qsp-field"><label>수주번호</label><input name="salesOrder" placeholder="예: SO-260825-01"></div><div class="qsp-field"><label>고객사</label><select name="customer"><option value="">선택</option><option>현대자동차</option><option>삼성SDI</option><option>SK</option><option>기타</option></select></div><div class="qsp-field"><label>Grd 제품명</label><input name="grdProductName" placeholder="Grd 제품명을 입력하세요"></div><div class="qsp-field"><label>완제품 LOT</label><input name="lot" placeholder="예: FG-260825-01"></div><div class="qsp-field"><label>출하 수량 (kg)</label><input name="qty" inputmode="decimal" placeholder="예: 2000"></div><div class="qsp-field full"><label>배송 / 배차 정보</label><input name="delivery" placeholder="차량번호, 운송사, 기사 연락처 등"></div><div class="qsp-field full"><label>비고</label><textarea name="note" placeholder="특이사항을 입력하세요."></textarea></div></div><div class="qsp-actions"><button type="button" class="qsp-cancel">취소</button><button type="submit" class="qsp-save">등록</button></div></form></div>`;
    document.body.appendChild(overlay);overlay.addEventListener('click',e=>{if(e.target===overlay)closeModal();});overlay.querySelector('.qsp-close').addEventListener('click',closeModal);overlay.querySelector('.qsp-cancel').addEventListener('click',closeModal);overlay.querySelector('#qsp-form').addEventListener('submit',e=>{e.preventDefault();const form=new FormData(e.currentTarget);const row=Object.fromEntries(form.entries());try{const saved=JSON.parse(localStorage.getItem('qmes-shipping-plans-v1')||'[]');const list=Array.isArray(saved)?saved:[];list.unshift({...row,id:'SHIP-'+Date.now(),createdAt:new Date().toISOString(),status:'출하예정'});localStorage.setItem('qmes-shipping-plans-v1',JSON.stringify(list));}catch(err){console.warn('[QMES] shipping plan local save failed',err);}closeModal();alert('출하계획이 등록되었습니다.');window.dispatchEvent(new CustomEvent('qmes:shipping-plan-saved'));});setTimeout(()=>overlay.querySelector('input[name="shipDate"]')?.focus(),0);
  }
  document.addEventListener('click',function(e){const btn=e.target.closest('button');if(!btn)return;const text=(btn.textContent||'').replace(/\s+/g,' ').trim();if(text!=='+ 출하계획'&&text!=='출하계획')return;const host=btn.closest('.qmes-business-extension')||btn.closest('#qmes-business-extension-host');const title=host?.querySelector('.qbe-title,h1')?.textContent||'';if(!title.includes('출하')&&!document.body.textContent.includes('출하 · 납품관리'))return;e.preventDefault();e.stopPropagation();openModal();},true);
