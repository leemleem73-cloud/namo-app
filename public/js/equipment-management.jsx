...
(function installEquipmentManagement(){
  ...
  function EquipmentManagementTab(props){
    ...
    return <div className="qmes-equipment-management-layout">
      <style>{`
        ...
        .qmes-ipad-equipment .qmes-equipment-repair-screen .qmes-equipment-table-wrap{width:100%!important;max-width:100%!important;margin:0!important;padding:0!important;overflow:hidden!important;border:1px solid #dce4ec!important;border-radius:10px!important;background:#fff!important;box-shadow:none!important;box-sizing:border-box!important;}
      ...
      <main className="qmes-equipment-management-content">
        ...
        {section==="daily"&&<DailyEquipmentCheckTab {...props}/>}
        {section==="master"&&<div className="qmes-equipment-master-screen" style={{margin:0,padding:0,border:0,borderRadius:0,background:"transparent",boxShadow:"none"}}><Panel title="설비대장" right={<button onClick={saveMaster} className={`${smallButton} border-sky-500/50 bg-sky-500/10 text-sky-300`}>전체 저장</button>}><div className="overflow-x-auto -mx-4 px-4"><table className="qmes-equipment-master-table w-full min-w-[1050px] text-sm" style={{tableLayout:"fixed",borderCollapse:"collapse",border:"1px solid #dce4ec"}}><colgroup><col style={{width:"11%"}}/><col style={{width:"14%"}}/><col style={{width:"12%"}}/><col style={{width:"10%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"12%"}}/><col style={{width:"9%"}}/><col style={{width:"10%"}}/></colgroup><thead><tr className="text-xs text-slate-400">{["관리번호","설비명","설치위치","담당부서","제조사","모델","설치일","상태","비고"].map(label=><th key={label} className="px-1 py-2 text-center font-medium" style={{border:"1px solid #d1d5db"}}>{label}</th>)}</tr></thead><tbody>{master.map(row=><tr key={row.id}>{[["managementNo",true],["name"],["location"],["owner"],["maker"],["model"]].map(([key,mono])=><td key={key} className="px-1 py-2" style={{border:"1px solid #e5e7eb"}}><input className={`${inputClass}${mono?" font-mono":""}`} value={row[key]||""} onChange={e=>updateMaster(row.id,key,e.target.value)}/></td>)}<td className="px-1 py-2" style={{border:"1px solid #e5e7eb"}}><input type="date" className={inputClass} value={row.installDate||""} onChange={e=>updateMaster(row.id,"installDate",e.target.value)}/></td><td className="px-1 py-2" style={{border:"1px solid #e5e7eb"}}><select className={inputClass} value={row.status||"가동"} onChange={e=>updateMaster(row.id,"status",e.target.value)}><option>가동</option><option>점검중</option><option>사용중지</option><option>폐기</option></select></td><td className="px-1 py-2" style={{border:"1px solid #e5e7eb"}}><input className={inputClass} value={row.note||""} onChange={e=>updateMaster(row.id,"note",e.target.value)}/></td></tr>)}</tbody></table></div></Panel></div>}
        {section==="schedule"&&<div className="qmes-equipment-history-screen qmes-equipment-schedule-screen">
          <Panel title="정기점검·교정" right={<button type="button" onClick={()=>setShowScheduleForm(value=>!value)} className={`${smallButton} qmes-equipment-new-register border-slate-700 bg-white text-slate-900`}>{showScheduleForm?"등록 닫기":"신규 등록"}</button>}>
            {showScheduleForm&&<div className="qmes-equipment-register-box">
              <div className="qmes-equipment-schedule-form qmes-equipment-register-grid">
                <label>설비<select className={`${inputClass} mt-1`} value={scheduleForm.equipmentId} onChange={e=>setScheduleForm({...scheduleForm,equipmentId:e.target.value})}>{master.map(row=><option key={row.id} value={row.id}>{row.name}</option>)}</select></label>
                <label>구분<select className={`${inputClass} mt-1`} value={scheduleForm.type} onChange={e=>setScheduleForm({...scheduleForm,type:e.target.value})}><option>정기점검</option><option>교정</option><option>예방보전</option></select></label>
                <label>주기(일)<input className={`${inputClass} mt-1`} value={scheduleForm.cycleDays} onChange={e=>setScheduleForm({...scheduleForm,cycleDays:e.target.value})}/></label>
                <label>마지막 실시일<input type="date" className={`${inputClass} mt-1`} value={scheduleForm.lastDate} onChange={e=>setScheduleForm({...scheduleForm,lastDate:e.target.value})}/></label>
            ...
            <div className="qmes-equipment-table-wrap">
              <table className="qmes-equipment-unified-table qmes-equipment-schedule-table">
                ...
                <tbody>
                  ...
                  {!scheduleRows.length&&<tr><td colSpan="8" className="qmes-equipment-empty-cell">등록된 정기점검·교정 일정이 없습니다.</td></tr>}
                </tbody>
              </table>
            </div>
          </Panel>
          ...
          <Panel className="qmes-equipment-repair-panel" title="고장·수리 이력" right={<button type="button" onClick={()=>setShowRepairForm(value=>!value)} className={`${smallButton} qmes-equipment-new-register qmes-equipment-repair-register-button border-slate-700 bg-white text-slate-900`}>{showRepairForm?"등록 닫기":"신규 등록"}</button>}>
            {showRepairForm&&<div className="qmes-equipment-register-box">
              <div className="qmes-equipment-repair-form qmes-equipment-register-grid">
                ...
                <label>발생일<input type="date" className={`${inputClass} mt-1`} value={repairForm.occurredAt} onChange={e=>setRepairForm({...repairForm,occurredAt:e.target.value})}/></label>
                <label>이상 내용<input className={`${inputClass} mt-1`} value={repairForm.issue} onChange={e=>setRepairForm({...repairForm,issue:e.target.value})}/></label>
                <label>조치 내용<input className={`${inputClass} mt-1`} value={repairForm.action} onChange={e=>setRepairForm({...repairForm,action:e.target.value})}/></label>
