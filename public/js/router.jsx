...
function QMESChemical({user, onLogout}){
  ...
  useEffect(()=>{
    if(openMenu){
      safeStorageSet("qmes_open_menu", openMenu);
    }else{
      safeStorageRemove("qmes_open_menu");
    }
  }, [openMenu]);
  ...
  const visibleTabs = TABS.filter(
    tabItem=>!tabItem.adminOnly || user.role === "admin"
  );
      ...
      const nextUsers = Array.isArray(users)
        ? users.map(item=>{
            const sameUser =
              ...
              String(item.name || "") === String(user.name || "");
  ...
  return (
    ...
    >
      <header className="border-b border-slate-800 bg-slate-900/80 sticky top-0 z-50 backdrop-blur">
        <div className="w-full px-4 lg:px-6 py-3 flex items-center gap-4">
          ...
          <button
            ...
            className={`relative flex items-center gap-2 px-3.5 py-2 rounded-lg border text-sm font-bold transition-colors ${
              talkOpen
                ? "bg-sky-500/20 border-sky-400 text-white"
                : "bg-sky-600/15 border-sky-500/70 text-sky-100 hover:bg-sky-500/25"
            }`}
            aria-label={
              talkOpen
                ? "NAMO Talk 닫기"
          ...
          <div className="qmes-header-controls flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={downloadQmesBackup}
              className="qmes-header-action px-2 py-1 rounded border border-slate-700"
            >
              백업
            </button>
            ...
            {user.role === "admin" && (
              <button
                ...
                className={`qmes-header-action px-2 py-1 rounded border ${
                  tab === "members"
        ...
        <div className="border-t border-white/10 qmes-top-menu-bar">
          <nav className="qmes-top-menu">
            {TOP_MENUS.map(menu=>{
              ...
              const children = (menu.children || [])
                .map(id=>visibleTabs.find(tabItem=>tabItem.id === id))
                .filter(Boolean);
                  ...
                  <button
                    ...
                    onClick={()=>{
                      if(direct){
                        ...
                        setOpenMenu(opened ? null : menu.id);
                    ...
                    className={`qmes-top-menu-button ${
                      active ? "is-active" : ""
                    }`}
                  >
                    <MenuIcon size={15} />
          ...
          {openMenu && (()=>{
            const selected = TOP_MENUS.find(
              menu=>menu.id === openMenu
            );

            const items = (selected?.children || [])
              .map(id=>visibleTabs.find(tabItem=>tabItem.id === id))
              .filter(Boolean);
            ...
            return (
              <div
                className="qmes-submenu-row"
                role="menu"
              >
                <div className="qmes-submenu-title">
                  {selected.label}
                </div>
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                type="button"
                onClick={closeAccountModal}
                className="w-9 h-9 rounded-lg border border-slate-700 text-slate-300 hover:bg-slate-800"
                aria-label="닫기"
              >
                ×
              </button>
            ...
            <button
              ...
              className="w-full h-12 rounded-xl bg-sky-600 px-4 text-left text-sm font-black text-white hover:bg-sky-500"
            >
              비밀번호 변경하기
            </button>
            ...
            <button
              ...
              className="mt-3 w-full h-12 rounded-xl border border-red-500/50 bg-red-500/10 px-4 text-left text-sm font-black text-red-300 hover:bg-red-500/20"
            >
              로그아웃
            </button>
            ...
            <button
              type="button"
              onClick={closeAccountModal}
              className="mt-5 w-full h-11 rounded-xl border border-slate-700 text-sm font-bold text-slate-300 hover:bg-slate-800"
            >
              닫기
            </button>
          </div>
      ...
      {passwordOpen && (
        <div
          className="fixed inset-0 z-[11000] flex items-center justify-center bg-black/70 p-4"
        ...
        >
          ...
          >
            ...
            <input
              ...
              className="w-full h-11 rounded-xl border border-slate-700 bg-slate-950 px-3 outline-none focus:border-sky-500"
            />
            ...
            {passwordError && (
              <div className="mt-3 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm font-bold text-red-400">
                {passwordError}
              </div>
            )}
            ...
            <button
              type="submit"
              className="mt-6 w-full h-11 rounded-xl bg-sky-600 font-black text-white hover:bg-sky-500"
            >
              변경 저장
            </button>
            ...
            <button
              type="button"
              onClick={closePasswordModal}
              className="mt-3 w-full h-11 rounded-xl border border-slate-700 font-bold text-slate-300 hover:bg-slate-800"
            >
              취소
            </button>
          </form>
