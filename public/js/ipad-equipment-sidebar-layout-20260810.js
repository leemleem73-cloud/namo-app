(function qmesEquipmentSidebarLayout(){
  'use strict';
  if(window.__QMES_EQUIPMENT_SIDEBAR_LAYOUT_20260810_V4__) return;
  window.__QMES_EQUIPMENT_SIDEBAR_LAYOUT_20260810_V4__=true;
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .qmes-em-original-menu-shell{
        ...
        margin:0!important;padding:0!important;border:0!important;overflow:hidden!important;opacity:0!important;pointer-events:none!important;
      }
      .qmes-em-fixed-sidebar{
        position:absolute!important;left:14px!important;top:14px!important;width:188px!important;
        display:flex!important;flex-direction:column!important;gap:8px!important;padding:12px!important;
        box-sizing:border-box!important;z-index:50!important;
        border:1px solid #405466!important;border-radius:14px!important;
        background:linear-gradient(180deg,#172c40 0%,#23384a 100%)!important;box-shadow:none!important;
      }
      .qmes-em-fixed-sidebar-title{
        padding:5px 8px 9px!important;color:#f8fafc!important;font-size:16px!important;font-weight:900!important;line-height:1.2!important;
      }
      .qmes-em-fixed-sidebar>button{
        ...
        color:#2c4052!important;font-family:Pretendard,system-ui,sans-serif!important;font-size:16px!important;font-weight:900!important;
        line-height:1.2!important;text-align:left!important;white-space:normal!important;word-break:keep-all!important;box-shadow:none!important;
      }
      .qmes-em-fixed-sidebar>button:hover,.qmes-em-fixed-sidebar>button:focus-visible{
      ...
      @media(max-width:900px){
        .qmes-em-management-root{padding-left:0!important;padding-top:172px!important;}
        .qmes-em-fixed-sidebar{left:10px!important;right:10px!important;top:10px!important;width:auto!important;
          display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;}
        .qmes-em-fixed-sidebar-title{grid-column:1/-1!important;padding-bottom:2px!important;}
        .qmes-em-fixed-sidebar>button{min-height:48px!important;justify-content:center!important;text-align:center!important;font-size:14px!important;}
      }
    `;
  }

  function findTitle(){
    return Array.from(document.querySelectorAll('h1,h2,h3,h4,h5,div,span,strong')).find(function(el){
      return el.children.length===0 && clean(el.textContent)==='설비 관리';
    })||null;
  }

  function findManagementRoot(title){
    var node=title;
    while(node&&node!==document.body){
      var buttons=Array.from(node.querySelectorAll('button')).filter(function(button){return !!groupForText(button.textContent);});
  ...
  function markOriginalHidden(button,root){
    ...
    if(interactive.length===1 && interactive[0]===button && groupForText(text)){
      ...
      if(parent&&parent!==root){
        var parentInteractive=Array.from(parent.querySelectorAll('button,input,select,textarea,a'));
        var parentText=clean(parent.textContent);
        if(parentInteractive.length===1 && parentInteractive[0]===button && groupForText(parentText)){
          parent.classList.add('qmes-em-original-menu-shell');
        }
      }
    }
  }
  ...
  function detectSelected(root){
    var active=originalButtons(root).find(function(button){
      return button.classList.contains('qmes-equipment-nav-selected') || button.classList.contains('is-active') || button.getAttribute('aria-selected')==='true';
    });
    ...
    if(!sidebar){
      ...
      MENU_GROUPS.forEach(function(group){
        ...
        button.addEventListener('click',function(){
          ...
          if(original){
            original.click();
            window.setTimeout(function(){apply();},20);
          }
        });
  ...
  function apply(){
    ...
    var title=findTitle();
    if(!title) return;
    var root=findManagementRoot(title);
    if(!root) return;
  ...
  new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class','aria-selected']});
