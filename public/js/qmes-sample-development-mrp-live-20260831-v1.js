/* NAMO QMES - Live Production Plan / MRP V3 - 2026-08-31
 ...
 * V3 owns the actual .qerp plan page directly; it does NOT depend on a <main> element.
 */
(function(){
  'use strict';
  if(window.__QMES_LIVE_PRODUCTION_MRP_20260831_V3__)return;
  window.__QMES_LIVE_PRODUCTION_MRP_20260831_V3__=true;
  ...
  let state={productionType:'샘플',date:today(),product:PRODUCT,qty:'',products:[PRODUCT],workorders:[],iqcRecords:[],inventoryRecords:[],purchases:[],mrp:[],revision:'-',recipeSource:'-',recipeOfficial:false,error:'',currentPossible:0,currentShortage:0,afterIncomingShortage:0,pendingIqc:0,activeWoCount:0};
  ...
  function calculate(){
    const qty=Math.max(0,num(state.qty)),all=allWorkorders(state.workorders),active=activeWorkorders(state.workorders),recipe=recipeFor(PRODUCT,all,state.productionType),iqc=iqcRows(state.iqcRecords);
