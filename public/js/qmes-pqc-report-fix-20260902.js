...
(function installPqcReportFix(global){
  ...
  function installStyle(){
    ...
    style.id=STYLE_ID;
    style.textContent=`
html body #root .qmes-pqc-doc .qmes-iqc-header-logo{object-fit:contain!important;image-rendering:auto!important;filter:none!important;opacity:1!important;transform:scale(.98)!important;transform-origin:left center!important;max-width:98%!important;height:auto!important;}
...
`;
  ...
  function fixRowsAndBorders(doc){
    doc.querySelectorAll("table.qmes-iqc2-table").forEach(table=>{important(table,"border-collapse","collapse");table.querySelectorAll("tr").forEach(row=>{important(row,"height","35px");important(row,"min-height","35px");});table.querySelectorAll("th,td").forEach(cell=>{important(cell,"height","35px");important(cell,"min-height","35px");important(cell,"padding-top","3px");important(cell,"padding-bottom","3px");important(cell,"border","1px solid #94a3b8");important(cell,"box-sizing","border-box");important(cell,"text-align","center");important(cell,"vertical-align","middle");});});
  ...
  function fixApproval(doc){
    ...
    const table=row.querySelector("table.qmes-iqc2-sign-table");if(!table)return;
