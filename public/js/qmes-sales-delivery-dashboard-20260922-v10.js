/* QMES Sales/Delivery integrated dashboard V10 - DELETE ROLE GUARD ONLY - 2026-09-22
...
(function(){
  "use strict";
  if(window.__QMES_SALES_DELIVERY_DASHBOARD_20260922_V10__) return;
  window.__QMES_SALES_DELIVERY_DASHBOARD_20260922_V10__=true;
  ...
  function canDeleteSales(){
    ...
    const title=clean(user.title||user.position||user.rank);
    // 부장 + 임원급만 허용. 차장/과장/대리/사원 등은 불가.
  ...
  function SalesDeliveryDashboard(){
    ...
    const body=shown.length?shown.map((x,i)=>h("tr",{key:x.id},
      ...
      h("td",null,h("span",{className:"qrl-actions"},
        ...
        canDeleteSales()?h("button",{type:"button",className:"qrl-delete-btn",onClick:()=>deleteOrder(x)},"삭제"):null
      ))
