/* Inventory menu bridge v3.1: stable mount + date detail modal guard, 2026-08-21. */
(function(){
  ...
  function open(section='overview'){
    ...
    setTimeout(decorateSidebar,20);setTimeout(decorateSidebar,180);
  }
  function install(){
    ...
    document.addEventListener('click',event=>{
      ...
      if(top&&top!==button&&!top.closest('[data-qmes-inventory-menu]')&&host)restore();
    },true);
    window.qmesOpenInventorySection=open;return true;
  }
  /* Keep inventory date/detail clicks inside the inventory React tree. Do not remount or restore on detail clicks. */
  document.addEventListener('click',event=>{
    const detail=event.target.closest?.('.inv-tx-detail-link');
    if(!detail)return;
