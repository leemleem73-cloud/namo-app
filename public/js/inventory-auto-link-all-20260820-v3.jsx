/* QMES inventory full auto-link v3 - physical rack locations, 2026-08-20 */
(function(){
  'use strict';
  if(window.__QMES_INV_AUTO_LINK_ALL_V3__) return;
  window.__QMES_INV_AUTO_LINK_ALL_V3__=true;
  ...
  function installHint(){
    document.querySelectorAll('.inv-shell .inv-title-row').forEach(row=>{
      let hint=row.querySelector('[data-qmes-inv-auto-v3="1"]');
      if(!hint){hint=document.createElement('div');hint.dataset.qmesInvAutoV3='1';hint.style.cssText='font-size:12px;color:#64748b;margin-top:6px;font-weight:700;';row.querySelector('div')?.appendChild(hint);}
      hint.textContent='실제 랙 자동연동 · 원재료 A-1-1~A-6-2 · 완제품 B-1-1~B-3-2';
    });
  }
