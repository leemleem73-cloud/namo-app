/* Namo Chemical Q-MES Enterprise Inventory auto mode v8, 2026-08-20 */
...
function InventoryEnterpriseTab({section='overview'}){
  const [stock,setStock]=useState([]),[summary,setSummary]=useState(null),[transactions,setTransactions]=useState([]),[counts,setCounts]=useState([]),[locations,setLocations]=useState([]),[items,setItems]=useState([]),[reservations,setReservations]=useState([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[query,setQuery]=useState(''),[category,setCategory]=useState(''),[status,setStatus]=useState(''),[modal,setModal]=useState(null);
  ...
  return <div className="inv-shell">
    ...
    {loading?<div className="inv-loading">재고 데이터를 불러오는 중...</div>:<>
      ...
      {section==='movement'&&<div className="inv-panel inv-stock-panel inv-movement-panel"><h3>자동 입출고 처리 내역</h3><TxTable rows={movementRows}/>{!movementRows.length&&<div className="inv-empty">입출고 처리 내역이 없습니다.</div>}</div>}
...
function TxTable({rows}){return <table className="inv-stock-table inv-movement-table"><colgroup><col/><col/><col/><col/><col/><col/><col/></colgroup><thead><tr><th>일시</th><th>구분</th><th>원료명</th><th>LOT</th><th>수량</th><th>이동 방향</th><th>비고</th></tr></thead><tbody>{rows.map(tx=><tr key={tx.id}><td>{new Date(tx.created_at).toLocaleString('ko-KR')}</td><td>{INV_TYPE_LABEL[tx.transaction_type]||tx.transaction_type}</td><td title={txMaterialName(tx)}>{txMaterialName(tx)}</td><td>{tx.lot_no}</td><td className="num">{invNum(tx.quantity)} {tx.unit}</td><td>{txDirectionLabel(tx)}</td><td title={txDisplayReference(tx)}>{txDisplayReference(tx)}</td></tr>)}</tbody></table>}
