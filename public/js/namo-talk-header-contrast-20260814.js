/* NAMO QMES reference shell — 2026-09-02
 * Target: attached namochemical_douzone_erp_mes_douzone_color(2).html main screen.
 * This is a layout + palette rebuild, not a color-only patch.
 * Current NAMO Chemical logo asset is preserved.
 */
(function(){
  'use strict';
  if(window.__QMES_REFERENCE_MAIN_SHELL_20260902__) return;
  window.__QMES_REFERENCE_MAIN_SHELL_20260902__=true;

  const STYLE_ID='qmes-reference-main-shell-20260902';
  const oldTheme=document.getElementById('qmes-douzone-final-theme-20260902');
  if(oldTheme) oldTheme.remove();
  const oldContrast=document.getElementById('qmes-namo-talk-header-contrast');
  ...
  style.textContent=`
    ...
    *{box-sizing:border-box}
    html,body{background:#edf2f6!important;color:#22384a!important;}
    body.qmes-reference-shell{margin:0!important;overflow-x:hidden!important;background:#edf2f6!important;}

    /* ===== ATTACHED HTML: ONE 64PX TOPBAR ===== */
    body.qmes-reference-shell #root>div>header{
      ...
      color:#fff!important;
      border:0!important;
      border-bottom:1px solid #235f8e!important;
    ...
    /* Preserve the current NAMO Chemical logo asset, but place it in the reference white brand block. */
    body.qmes-reference-shell #root>div>header>div:first-child>button:first-child{
      flex:0 0 var(--qmes-ref-sidebar)!important;
      width:var(--qmes-ref-sidebar)!important;
      height:64px!important;
      min-height:64px!important;
      margin:0!important;
      ...
      border:0!important;
      border-right:1px solid #b9c9d5!important;
      border-radius:0!important;
      background:linear-gradient(180deg,#f8fafb 0%,#e5ebf0 100%)!important;
    ...
    body.qmes-reference-shell .qmes-submenu-row{
    ...
    body.qmes-reference-shell #qmes-sync-hamburger{
      display:none!important;
      visibility:hidden!important;
      opacity:0!important;
    ...
    #qmes-ref-menu-toggle{
      ...
      font-size:23px!important;line-height:1!important;cursor:pointer!important;
    }
    ...
    #qmes-ref-global-search input{
      width:100%!important;height:36px!important;border:1px solid rgba(255,255,255,.42)!important;
      background:rgba(255,255,255,.94)!important;border-radius:7px!important;
      padding:0 36px 0 13px!important;color:#263d4f!important;-webkit-text-fill-color:#263d4f!important;
      outline:0!important;font-size:13px!important;font-weight:600!important;box-shadow:none!important;
    }
    ...
    #qmes-ref-date-chip{
      flex:0 0 auto!important;height:34px!important;display:flex!important;align-items:center!important;
      padding:0 11px!important;border:1px solid rgba(255,255,255,.38)!important;border-radius:6px!important;
    ...
    body.qmes-reference-shell #root>div>header button[aria-label*="NAMO Talk 알림"]{
      ...
      padding:0!important;border:0!important;background:transparent!important;color:#fff!important;border-radius:6px!important;
    }
    ...
    body.qmes-reference-shell .qmes-header-controls>button[aria-label^="계정 설정"]{
      display:flex!important;align-items:center!important;gap:9px!important;height:46px!important;
      padding:0 2px 0 14px!important;border:0!important;border-left:1px solid rgba(255,255,255,.30)!important;
    ...
    body.qmes-reference-shell .qmes-header-controls>button[aria-label^="계정 설정"]>div:first-of-type{
      width:34px!important;height:34px!important;border-radius:50%!important;background:#f4f8fb!important;color:#2f6f9f!important;
      -webkit-text-fill-color:#2f6f9f!important;border:1px solid rgba(255,255,255,.5)!important;font-size:12px!important;
    }
    body.qmes-reference-shell .qmes-header-controls>button[aria-label^="계정 설정"]>div:nth-of-type(2){
      color:#fff!important;-webkit-text-fill-color:#fff!important;font-size:12px!important;font-weight:800!important;line-height:1.25!important;
    }
    ...
    /* ===== ATTACHED HTML: FIXED LIGHT LEFT SIDEBAR ===== */
    #qmes-ref-sidebar{
      position:fixed!important;left:0!important;top:64px!important;bottom:0!important;width:var(--qmes-ref-sidebar)!important;
      z-index:12400!important;overflow-y:auto!important;overflow-x:hidden!important;
      background:linear-gradient(180deg,#f8fafb 0%,#edf2f6 100%)!important;color:#344d60!important;
      border-right:1px solid #bfcdd8!important;box-shadow:2px 0 5px rgba(51,82,103,.08)!important;
    ...
    .qmes-ref-foot-note{font-size:9px!important;line-height:1.55!important;color:#708596!important;}
    .qmes-ref-foot-note b{color:#3c6684!important;}

    /* ===== ATTACHED HTML: PAGE HEAD UNDER TOPBAR ===== */
    #qmes-ref-pagehead{
      position:fixed!important;left:var(--qmes-ref-sidebar)!important;right:0!important;top:64px!important;height:72px!important;
      z-index:12300!important;display:flex!important;align-items:center!important;padding:0 26px!important;
      background:linear-gradient(180deg,#fff 0%,#f4f7f9 100%)!important;border-bottom:1px solid #becdd8!important;
      box-shadow:0 1px 3px rgba(42,76,99,.05)!important;transition:left .2s ease!important;
    }
    ...
    body.qmes-reference-shell #root>div>main{
      margin-left:var(--qmes-ref-sidebar)!important;width:calc(100% - var(--qmes-ref-sidebar))!important;
      min-height:100vh!important;padding:158px 26px 40px!important;background:#edf2f6!important;color:#22384a!important;
      transition:margin-left .2s ease,width .2s ease!important;box-sizing:border-box!important;
    }
    ...
    /* exact reference table look */
    body.qmes-reference-shell #root main table{background:#fff!important;border-color:#cbd8e2!important;}
    body.qmes-reference-shell #root main table thead,body.qmes-reference-shell #root main table thead tr,body.qmes-reference-shell #root main table thead th{
      background:linear-gradient(180deg,#3d87bd 0%,#2e72a9 100%)!important;color:#fff!important;-webkit-text-fill-color:#fff!important;border-color:#225f8e!important;text-shadow:0 1px 0 rgba(0,0,0,.12)!important;
    }
    ...
    body.qmes-reference-shell #root main table tbody tr:nth-child(even) td{background:#f7fafc!important;}
    body.qmes-reference-shell #root main table tbody tr:hover td{background:#eaf4fb!important;}
    ...
    body.qmes-ref-collapsed #qmes-ref-sidebar .qmes-ref-company-pill span,
    body.qmes-ref-collapsed #qmes-ref-sidebar .qmes-ref-company-pill b,
    body.qmes-ref-collapsed #qmes-ref-sidebar .qmes-ref-group-label,
    body.qmes-ref-collapsed #qmes-ref-sidebar .qmes-ref-menu-text,
    body.qmes-ref-collapsed #qmes-ref-sidebar .qmes-ref-sidebar-foot{display:none!important;}
    ...
    @media(max-width:760px){
      body.qmes-reference-shell{--qmes-ref-sidebar:0px;}
      #qmes-ref-sidebar{transform:translateX(-248px)!important;width:248px!important;transition:transform .2s ease!important;}
      body.qmes-ref-mobile-open #qmes-ref-sidebar{transform:none!important;}
      body.qmes-reference-shell #root>div>header>div:first-child>button:first-child{display:none!important;}
      #qmes-ref-global-search{display:none!important;}
      #qmes-ref-pagehead{left:0!important;padding:0 14px!important;}
    ...
    @media print{
      #qmes-ref-sidebar,#qmes-ref-pagehead,#qmes-ref-menu-toggle,#qmes-ref-global-search,#qmes-ref-date-chip{display:none!important;}
      body.qmes-reference-shell #root>div>header{display:none!important;}
      body.qmes-reference-shell #root>div>main{margin:0!important;width:100%!important;padding:0!important;background:#fff!important;}
      body.qmes-reference-shell #root main table thead,body.qmes-reference-shell #root main table thead tr,body.qmes-reference-shell #root main table thead th{background:#fff!important;color:#111!important;-webkit-text-fill-color:#111!important;text-shadow:none!important;}
    }
  `;
  ...
  const groups=[
    ...
    {label:'MES · QMS',items:[
      {tab:'prod',label:'생산 진행',icon:'M'},
      {tab:'woIssue',label:'작업지시서',icon:'W'},
      {tab:'prodProcess',label:'생산공정 관리',icon:'R'},
      {tab:'iqc',label:'수입검사 (IQC)',icon:'Q'},
      {tab:'pqc',label:'공정검사 (PQC)',icon:'P'},
      {tab:'oqc',label:'출하검사 (OQC)',icon:'O'},
      {tab:'trace',label:'LOT 통합추적',icon:'L'},
      {tab:'coa',label:'출하성적서',icon:'C'},
      {tab:'pop',label:'현장 입력 (iPad)',icon:'T'}
    ]},
    {label:'SYSTEM',items:[
      {tab:'eq',label:'설비관리',icon:'E'},
      {tab:'spc',label:'SPC (Cpk)',icon:'S'},
      {tab:'lock',label:'품질 인터락',icon:'Q'},
      {tab:'ncr',label:'부적합 (8D)',icon:'N'},
      {tab:'cc',label:'고객불만 (GQMS)',icon:'G'},
      {tab:'4m',label:'4M 변경관리',icon:'4'},
      {tab:'members',label:'권한·시스템관리',icon:'A',adminOnly:true}
    ]}
  ];

  const pageMeta={
    dash:['통합 경영 대시보드','홈 〉 통합 대시보드'],
    erpSales:['수주·영업관리','ERP 〉 수주·영업관리'],
    erpPlan:['생산계획·MRP','ERP 〉 생산계획 · MRP'],
    erpPurchase:['구매·발주관리','ERP 〉 구매관리 〉 발주현황'],
    inv:['재고·물류 현황','ERP 〉 재고·물류관리'],
    partners:['거래처 관리','ERP 〉 거래처 관리'],
    erpShipping:['출하·납품관리','ERP · MES 〉 출하·납품관리'],
    prod:['생산진행 현황','MES 〉 생산관리 〉 생산진행'],
    woIssue:['작업지시서','MES 〉 생산관리 〉 작업지시서'],
    prodProcess:['생산공정 관리','MES 〉 생산관리 〉 생산공정'],
    iqc:['수입검사 등록현황','MES · QMS 〉 품질관리 〉 수입검사'],
    pqc:['공정검사 현황','MES · QMS 〉 품질관리 〉 공정검사'],
    oqc:['출하검사 현황','MES · QMS 〉 품질관리 〉 출하검사'],
    trace:['LOT 통합추적','MES · QMS 〉 LOT 추적'],
    coa:['출하성적서','MES · QMS 〉 품질관리 〉 출하성적서'],
    pop:['현장 입력','MES 〉 현장 입력 (iPad)'],
    eq:['설비관리','SYSTEM 〉 설비관리'],
    spc:['SPC (Cpk)','QMS 〉 통계적 공정관리'],
    lock:['품질 인터락','QMS 〉 품질 인터락'],
    ncr:['부적합 관리','QMS 〉 부적합 (8D)'],
    cc:['고객불만 관리','QMS 〉 고객불만 (GQMS)'],
    '4m':['4M 변경관리','QMS 〉 4M 변경관리'],
    members:['권한·시스템관리','SYSTEM 〉 권한 · 시스템관리']
  };

  const currentTab=()=>{try{return sessionStorage.getItem('qmes_current_tab')||'dash';}catch(_){return'dash';}};
  const go=item=>{
    if(!item) return;
    if(item.section){
      try{sessionStorage.setItem('qmes_inventory_section',item.section);}catch(_){ }
      window.dispatchEvent(new CustomEvent('qmes:inventory-section',{detail:{section:item.section}}));
    }
    window.dispatchEvent(new CustomEvent('qmes:navigate-tab',{detail:{tab:item.tab}}));
  };

  function buildSidebar(){
    let side=document.getElementById('qmes-ref-sidebar');
    if(side) return side;
    side=document.createElement('aside');
    side.id='qmes-ref-sidebar';
    ...
    side.addEventListener('click',event=>{
      ...
      const util=event.target.closest('[data-qmes-ref-util]');
      if(!util)return;
      const wanted=util.dataset.qmesRefUtil==='backup'?'백업':'복원';
      const original=Array.from(document.querySelectorAll('.qmes-header-action')).find(btn=>String(btn.textContent||'').trim()===wanted);
      original?.click();
    });
  ...
  function buildHeaderTools(header){
    ...
    if(!row) return;
    if(!document.getElementById('qmes-ref-menu-toggle')){
      const menu=document.createElement('button');menu.id='qmes-ref-menu-toggle';menu.type='button';menu.setAttribute('aria-label','메뉴 접기');menu.textContent='☰';
      menu.addEventListener('click',()=>{if(innerWidth<=760)document.body.classList.toggle('qmes-ref-mobile-open');else document.body.classList.toggle('qmes-ref-collapsed');});
      row.children[0]?.after(menu);
    }
    if(!document.getElementById('qmes-ref-global-search')){
      const search=document.createElement('div');search.id='qmes-ref-global-search';search.innerHTML='<input type="search" aria-label="통합검색" placeholder="메뉴, 발주번호, LOT, 거래처 통합검색"><span id="qmes-ref-search-icon">⌕</span>';
      document.getElementById('qmes-ref-menu-toggle')?.after(search);
      const input=search.querySelector('input');
      input.addEventListener('input',()=>{
        const q=String(input.value||'').trim().toLowerCase();
        document.querySelectorAll('#qmes-ref-sidebar .qmes-ref-menu-item').forEach(btn=>{btn.style.display=!q||String(btn.textContent||'').toLowerCase().includes(q)?'flex':'none';});
      });
      input.addEventListener('keydown',event=>{if(event.key!=='Enter')return;const first=Array.from(document.querySelectorAll('#qmes-ref-sidebar .qmes-ref-menu-item')).find(btn=>btn.style.display!=='none');first?.click();});
    }
    if(!document.getElementById('qmes-ref-date-chip')){
      const chip=document.createElement('div');chip.id='qmes-ref-date-chip';
      const now=new Date();chip.textContent=`기준일 ${now.getFullYear()}.${String(now.getMonth()+1).padStart(2,'0')}.${String(now.getDate()).padStart(2,'0')}`;
      const bell=row.querySelector('button[aria-label*="NAMO Talk 알림"]');
      if(bell) row.insertBefore(chip,bell); else row.appendChild(chip);
    }
  }

  function buildPageHead(){
    let head=document.getElementById('qmes-ref-pagehead');
    if(head) return head;
    head=document.createElement('section');head.id='qmes-ref-pagehead';
    ...
    head.querySelector('[data-qmes-ref-refresh]').addEventListener('click',()=>window.location.reload());
    head.querySelector('[data-qmes-ref-print]').addEventListener('click',()=>window.print());
  ...
  function updateActive(){
    ...
    document.querySelectorAll('#qmes-ref-sidebar [data-qmes-ref-tab]').forEach(btn=>btn.classList.toggle('is-active',btn.dataset.qmesRefTab===tab));
    const meta=pageMeta[tab]||[document.querySelector('.qmes-ref-menu-item.is-active .qmes-ref-menu-text')?.textContent||'QMES','홈 〉 QMES'];
    const head=document.getElementById('qmes-ref-pagehead');
    if(head){head.querySelector('.qmes-ref-page-title').textContent=meta[0];head.querySelector('.qmes-ref-page-breadcrumb').textContent=meta[1];}
  }

  function neutralizeOldSidebar(){
    ...
    if(main){['margin-left','width','box-sizing','transition'].forEach(name=>main.style.removeProperty(name));delete main.dataset.qmesSidebarShift;}
  }

  function install(){
    const app=document.querySelector('#root>div');
    const header=app?.querySelector(':scope>header')||app?.querySelector('header');
    const main=app?.querySelector(':scope>main')||app?.querySelector('main');
    if(!header||!main) return false;
    document.body.classList.add('qmes-reference-shell');
    neutralizeOldSidebar();
    buildSidebar();
  ...
  const sync=()=>{
    ...
    neutralizeOldSidebar();
    const tab=currentTab();
    if(tab!==lastTab){lastTab=tab;updateActive();}
  };
  const observer=new MutationObserver(()=>sync());
  observer.observe(document.documentElement,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',sync,{once:true});
  window.addEventListener('load',sync,{once:true});
  window.addEventListener('qmes:navigate-tab',()=>setTimeout(updateActive,0));
