/* NAMO QMES - Sales edit V11 - 2026-08-27
 * Single owner. Reliable Sales edit button detection without depending on page title DOM.
 */
(function(){
  "use strict";
  if(window.__QMES_SALES_EDIT_V11__)return;
  window.__QMES_SALES_EDIT_V11__=true;
  ...
  const MODAL_ID="qmes-sales-edit-v11";
  let lastOpenAt=0;
  ...
  function salesMeta(row){
    const m=readMap(META_KEY),key=rowKey(row),id=clean(row?.id);
    return m[key]||m[id]||row?.orderMeta||{};
  }
  function effectiveId(row){return clean(salesMeta(row).salesOrderIdOverride)||clean(row?.id);}
  function rowByVisibleId(id){const target=clean(id);return rows().find(row=>clean(row?.id)===target||effectiveId(row)===target)||null;}

  function isSalesTable(table){
    if(!table)return false;
    const headers=Array.from(table.querySelectorAll("thead th")).map(th=>clean(th.textContent));
    return headers.includes("수주번호")&&headers.includes("고객사")&&headers.some(v=>v==="비고");
  }

  function editButtonFromTarget(target){
    if(!(target instanceof Element))return null;
    const button=target.closest("button");
    if(!button)return null;
    if(!button.classList.contains("qmes-sales-edit-btn")&&clean(button.textContent)!=="수정")return null;
    const tr=button.closest("tr"),table=button.closest("table");
    if(!tr||!isSalesTable(table))return null;
    const id=clean(tr.querySelector("[data-qso-id]")?.getAttribute("data-qso-id")||tr.children?.[0]?.textContent);
    return /^SO-/i.test(id)?button:null;
  }

  function rowFromButton(button){
    const tr=button.closest("tr");if(!tr)return null;
    const cell=i=>clean(tr.children?.[i]?.textContent);
    const visibleId=clean(button.dataset.salesId||button.dataset.qmesSalesEdit||tr.querySelector("[data-qso-id]")?.getAttribute("data-qso-id")||cell(0));
    const stored=rowByVisibleId(visibleId);
    if(stored)return {...stored,id:effectiveId(stored)};
    return {id:visibleId,workOrder:clean(button.dataset.workOrder)||visibleId,customer:cell(1),po:cell(2),product:clean(tr.children?.[3]?.childNodes?.[0]?.textContent)||cell(3),qty:num(cell(4)),due:"",plan:cell(8),shipping:cell(9),deliveryPlace:cell(10),source:"SCREEN"};
  }

  function ensureStyle(){
    if(document.getElementById("qmes-sales-edit-v11-style"))return;
    const style=document.createElement("style");style.id="qmes-sales-edit-v11-style";
    style.textContent=`
  ...
  function open(row){
    ...
    const now=Date.now();if(document.getElementById(MODAL_ID)&&now-lastOpenAt<300)return true;lastOpenAt=now;
    ensureStyle();close();
    const id=effectiveId(row),key=rowKey(row),meta=salesMeta(row),packMap=readMap(PACK_KEY),remarkMap=readMap(REMARK_KEY);
    ...
    modal.innerHTML=`<div class="card" role="dialog" aria-modal="true" aria-label="수주 전체 수정"><div class="head"><div><h2>수주 전체 수정</h2><div class="sub">${esc(id)} · 작업지시/LOT ${esc(key||"-")}</div></div><button type="button" class="close" data-sales-edit-close="1">닫기</button></div><form class="body" data-sales-edit-form-v11="1">
      ...
      <h3 class="section">수주 기본정보</h3><div class="grid"><div class="field"><label>수주번호</label><input class="sales-id" name="salesOrderId" value="${esc(id)}" maxlength="60" autocomplete="off"></div><div class="field"><label>고객사</label><input name="customer" value="${esc(row.customer||"")}"></div><div class="field"><label>고객 PO</label><input name="po" value="${esc(row.po||"")}"></div><div class="field"><label>고객 품목코드</label><input name="customerItemCode" value="${esc(meta.customerItemCode||row.customerItemCode||"")}"></div><div class="field"><label>수주구분</label><select name="orderType"><option ${clean(meta.orderType||row.orderType)==="양산"?"selected":""}>양산</option><option ${clean(meta.orderType||row.orderType)==="개발"?"selected":""}>개발</option><option ${clean(meta.orderType||row.orderType)==="샘플"?"selected":""}>샘플</option><option ${clean(meta.orderType||row.orderType)==="긴급"?"selected":""}>긴급</option></select></div><div class="field"><label>제품</label><input name="product" value="${esc(row.product||"")}"></div><div class="field"><label>수량 (kg)</label><input name="qty" value="${esc(row.qty||"")}"></div><div class="field"><label>요청 납기일</label><input type="date" name="due" value="${esc(clean(meta.requestedDue)||clean(row.due))}"></div><div class="field"><label>납품처</label><input name="deliveryPlace" value="${esc(meta.deliveryPlace||row.deliveryPlace||"")}"></div></div>
      ...
      <div class="error" data-sales-edit-error="1"></div><div class="actions"><button type="button" class="cancel" data-sales-edit-close="1">취소</button><button type="submit" class="save">수정 저장</button></div></form></div>`;
    modal.__salesRow={...row,id,key};document.body.appendChild(modal);modal.querySelector('[name="salesOrderId"]')?.focus();return true;
  }
  ...
  async function save(form){
    ...
    const error=form.querySelector('[data-sales-edit-error="1"]'),fail=m=>{if(error){error.textContent=m;error.classList.add("show");}return false;};
    const oldId=clean(row.id),newId=get("salesOrderId"),key=clean(row.key)||rowKey(row);
    ...
    if(rows().some(other=>rowKey(other)!==key&&effectiveId(other)===newId))return fail("이미 다른 수주에서 사용 중인 수주번호입니다.");
    ...
    const previous=metaMap[key]||metaMap[oldId]||row.orderMeta||{};
    ...
    metaMap[key]=meta;if(oldId!==key)delete metaMap[oldId];write(META_KEY,metaMap);
    ...
    if(oldId!==key)delete packMap[oldId];write(PACK_KEY,packMap);
    const remarks=get("remarks");if(remarks)remarkMap[key]=remarks;else delete remarkMap[key];if(oldId!==key)delete remarkMap[oldId];write(REMARK_KEY,remarkMap);

    const current=rows(),index=current.findIndex(other=>rowKey(other)===key||clean(other.id)===oldId);
    ...
    const next=[...current];if(index>=0)next[index]=updated;else next.unshift(updated);write(SALES_KEY,next);
    window.dispatchEvent(new CustomEvent("qmes:erp-data-changed",{detail:{kind:"sales",source:"SALES_EDIT_V11",oldId,newId}}));
    if(typeof window.qmesSalesFromWorkOrderApply==="function"){try{await window.qmesSalesFromWorkOrderApply();}catch(_){}}
    close();setTimeout(()=>window.location.reload(),120);
  }

  function handleOpen(event){
    const button=editButtonFromTarget(event.target);if(!button)return;
    const row=rowFromButton(button);if(!row)return;
    event.preventDefault();event.stopImmediatePropagation();open(row);
  }
  document.addEventListener("pointerdown",handleOpen,true);
  document.addEventListener("click",handleOpen,true);
  document.addEventListener("click",event=>{const target=event.target;if(!(target instanceof Element))return;if(target.closest("[data-sales-edit-close]")){event.preventDefault();close();return;}if(target===document.getElementById(MODAL_ID))close();},true);
  document.addEventListener("submit",event=>{const form=event.target instanceof Element?event.target.closest('[data-sales-edit-form-v11="1"]'):null;if(!form)return;event.preventDefault();event.stopImmediatePropagation();save(form);},true);

  ensureStyle();
  window.qmesSalesEditV11={open,openFromButton:button=>{const row=rowFromButton(button);return row?open(row):false;},close};
})();
