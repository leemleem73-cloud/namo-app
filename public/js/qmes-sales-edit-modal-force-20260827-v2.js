...
(function(){
  ...
  async function saveModal(event){
    ...
    const data=new FormData(form),get=name=>clean(data.get(name)),qty=number(get("qty")),unit=number(get("unitWeight")),count=number(get("packageQty")),packType=get("packagingType"),error=form.querySelector(".qsef-error"),fail=message=>{if(error){error.textContent=message;error.classList.add("show");}return false;};
    ...
    const id=clean(row.__stableId)||clean(row.id),key=clean(row.__stableKey)||rowKey(row)||id,now=new Date().toISOString(),metaMap=readMap(META_KEY),packMap=readMap(PACK_KEY),remarkMap=readMap(REMARK_KEY),previous=metaMap[key]||metaMap[id]||row.orderMeta||{},nextMeta={...previous,customerOverride:get("customer"),poOverride:get("po")||"-",productOverride:get("product"),qtyOverride:qty,requestedDue:get("due"),customerItemCode:get("customerItemCode"),deliveryPlace:get("deliveryPlace"),orderType:get("orderType")||"양산",savedAt:now};
    metaMap[key]=nextMeta;metaMap[id]=nextMeta;write(META_KEY,metaMap);
    if(packTouched){const packaging={type:packType,unitWeight:unit,packageQty:count,total:Number((unit*count).toFixed(3)),savedAt:now};packMap[key]=packaging;packMap[id]=packaging;}else{delete packMap[key];delete packMap[id];}write(PACK_KEY,packMap);
    const remarks=get("remarks");if(remarks){remarkMap[key]=remarks;remarkMap[id]=remarks;}else{delete remarkMap[key];delete remarkMap[id];}write(REMARK_KEY,remarkMap);
    const list=salesRows(),index=list.findIndex(item=>clean(item.id)===id||rowKey(item)===key),packaging=packTouched?packMap[key]:null,updated={...(index>=0?list[index]:row),id,workOrder:key,customer:get("customer"),po:get("po")||"-",product:get("product"),qty,due:get("due"),customerItemCode:nextMeta.customerItemCode,deliveryPlace:nextMeta.deliveryPlace,orderType:nextMeta.orderType,orderMeta:nextMeta,packaging,packagingType:packaging?.type||"",unitPackQty:packaging?.unitWeight||0,packageQty:packaging?.packageQty||0,remarks};
