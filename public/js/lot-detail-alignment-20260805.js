(function(){
  ...
  function renameHeaders(panel){Array.from(panel.querySelectorAll("thead th")).forEach(th=>{const text=clean(th.textContent);if(/원료\s*Lot/i.test(text)||text==="원료 LOT")th.textContent="LOT No.";else if(text==="품명")th.textContent="원재료명";else if(text==="공급사")th.textContent="업체명";});}
  ...
  function addLinks(){
    ...
    headings.forEach(heading=>{
      const panel=panelOf(heading);if(!panel)return;renameHeaders(panel);
      Array.from(panel.querySelectorAll("tbody tr")).forEach(row=>{
        ...
        if(!lot||!cell)return;
        cell.querySelectorAll(".qmes-lot-iqc-link-btn").forEach(node=>node.remove());
        ...
        const button=document.createElement("button");button.type="button";button.className="qmes-lot-iqc-link-btn";button.textContent="바로가기";button.dataset.lot=lot;button.dataset.name=name;button.dataset.supplier=supplier;inner.appendChild(button);
      });
    });
  }
  ...
  function findRow(lot){return Array.from(document.querySelectorAll(".qmes-iqc-ledger-table tbody tr")).find(row=>clean(row.textContent).includes(lot));}
  function clickAction(lot,label){const row=findRow(lot);const button=Array.from(row?.querySelectorAll("button")||[]).find(item=>clean(item.textContent).includes(label));if(button)button.click();}
  ...
  function showContext(){
    ...
    box.querySelector('[data-action="출력"]').onclick=()=>clickAction(lot,"출력");box.querySelector('[data-action="라벨"]').onclick=()=>clickAction(lot,"라벨");anchor.parentElement?.insertBefore(box,anchor);clearLink();return true;
  }
