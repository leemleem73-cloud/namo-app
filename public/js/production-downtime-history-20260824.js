/* QMES production downtime history view - 2026-08-24 v3
 * - Shows all LOT downtime history.
 * - Edit buttons use direct per-row handlers (no delegated lookup dependency).
 * - Re-fetches the latest process record before saving one downtime entry.
 */
(function(){
  "use strict";
  if(window.__QMES_PRODUCTION_DOWNTIME_HISTORY_20260824_V3__) return;
  window.__QMES_PRODUCTION_DOWNTIME_HISTORY_20260824_V3__=true;
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      .qmes-downtime-history-reason{text-align:center!important;font-weight:800;color:#fde68a!important}
      .qmes-downtime-history-note{text-align:center!important;white-space:normal}
      .qmes-downtime-edit-btn{display:inline-flex;align-items:center;justify-content:center;min-width:48px;height:30px;padding:0 9px;border:1px solid #0ea5e9;border-radius:6px;background:#075985;color:#e0f2fe;font-size:11px;font-weight:900;cursor:pointer}
  ...
  function ensureButton(){
    ...
    button.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();openHistory(button);});
  ...
  function renderModal(history){
    ...
    const bodyRows=history.length?history.map((row,index)=>`<tr class="${row.lot===selected?"current-lot":""}"><td>${esc(formatDateTime(row.at))}</td><td>${esc(row.lot)}</td><td>${esc(row.stepName)}</td><td>${esc(row.equipment)}</td><td class="qmes-downtime-history-reason">${esc(row.reason)}</td><td>${esc(durationLabel(row.minutes))}</td><td>${esc(row.workers||"-")}</td><td class="qmes-downtime-history-note">${esc(row.note||"-")}</td><td><button type="button" class="qmes-downtime-edit-btn" data-row-index="${index}">수정</button></td></tr>`).join(""):`<tr><td colspan="9" class="qmes-downtime-history-empty">등록된 비가동 이력이 없습니다.</td></tr>`;
    ...
    overlay.querySelector(".qmes-downtime-history-close")?.addEventListener("click",closeHistory);
    overlay.addEventListener("click",event=>{if(event.target===overlay)closeHistory();});
    overlay.querySelectorAll(".qmes-downtime-edit-btn").forEach(button=>{
  ...
  function openEditModal(item){
    ...
    overlay.querySelector(".qmes-downtime-edit-close")?.addEventListener("click",closeEdit);
    overlay.querySelector(".qmes-downtime-edit-cancel")?.addEventListener("click",closeEdit);
    overlay.querySelector(".qmes-downtime-edit-save")?.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();saveEdit(event.currentTarget);});
    overlay.addEventListener("click",event=>{if(event.target===overlay)closeEdit();});
    document.body.appendChild(overlay);
  ...
  async function saveEdit(button){
    ...
    try{
      ...
      const sameRecord=item=>clean(item?.at)===clean(target.at)&&Number(item?.stepNo||0)===Number(target.stepNo||0);
      if(!downtime[index]||!sameRecord(downtime[index])) index=downtime.findIndex(sameRecord);
      if(index<0) throw new Error("수정할 비가동 기록이 변경되었습니다. 비가동 현황을 다시 열어 확인하세요.");
      const now=new Date().toISOString();
      downtime[index]={...downtime[index],reason,minutes:totalMinutes,note,editedAt:now,editedBy:currentUserName()};
      await saveProcess(target.recordKey,{...payload,downtime,updatedAt:now,updatedBy:currentUserName()});
      closeEdit();
      const refreshed=buildHistory(await fetchRows());
  ...
  async function openHistory(button){
    ...
    try{renderModal(buildHistory(await fetchRows()));}
  ...
  const start=()=>{
    ensureButton();
    let tries=0;
    const timer=setInterval(()=>{tries+=1;if(ensureButton()||tries>=20)clearInterval(timer);},500);
  };
