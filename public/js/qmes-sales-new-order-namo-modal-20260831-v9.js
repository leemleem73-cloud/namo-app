 ...
 * Force-owned production modal. Removes/replaces the uploaded demo popup if it appears.
 */
(function(){
  'use strict';
  ...
  const STYLE='qmes-sales-new-order-modal-v9-style';
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id=STYLE;s.textContent=`
      ...
      #${ID} .n9-grid{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:10px 12px!important}
      #${ID} .n9-field{min-width:0!important}
      #${ID} .n9-field.span2{grid-column:span 2!important}
      #${ID} .n9-field.full{grid-column:1/-1!important}
      ...
      @media(max-width:900px){#${ID} .n9-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}#${ID} .n9-field.span2{grid-column:span 1!important}}
      @media(max-width:560px){#${ID}{padding:8px!important;align-items:flex-start!important}#${ID} .n9-grid{grid-template-columns:1fr!important}#${ID} .n9-field.span2,#${ID} .n9-field.full{grid-column:1!important}}
  ...
  function refresh(modal){
    ...
    set('product',g('product')||'-');set('qty',qty?qty.toLocaleString('ko-KR')+' kg':'-');set('stock',av.toLocaleString('ko-KR')+' kg');set('need',qty?need.toLocaleString('ko-KR')+' kg':'-');set('equipment','생산계획에서 지정');set('status',qty?(need>0?'계획 생성':'재고 출하 검토'):'계획 검토');
  }
  ...
  function open(){
    ...
    modal.innerHTML=`<div class="n9-card" role="dialog" aria-modal="true" aria-label="신규 수주 등록"><div class="n9-head"><div><h2>신규 수주 등록</h2><p>수주 → 생산계획 → 작업지시까지 동일 수주번호로 연결</p></div><button type="button" class="n9-close" data-n9-close>×</button></div><form><div class="n9-body"><datalist id="n9-customers">${customers.map(v=>`<option value="${esc(v)}"></option>`).join('')}</datalist><datalist id="n9-products">${products.map(v=>`<option value="${esc(v)}"></option>`).join('')}</datalist><div class="n9-note">나모케미칼 수주 Master 입력화면입니다. 데모 기본값 없이 실제 수주를 등록합니다.</div><div class="n9-grid">
      <div class="n9-field"><label>수주번호</label><input name="salesOrderId" readonly placeholder="납기 입력 시 자동생성"></div>
      <div class="n9-field"><label>수주일자</label><input type="date" name="orderDate" value="${today()}"></div>
      <div class="n9-field"><label>수주구분 *</label><select name="orderType"><option>양산</option><option>샘플</option><option>개발·평가</option></select></div>
      <div class="n9-field"><label>우선순위</label><select name="priority"><option>일반</option><option>긴급</option></select></div>
      <div class="n9-field"><label>고객사 *</label><input name="customer" list="n9-customers" placeholder="고객사 입력"></div>
      <div class="n9-field"><label>요청 납기일 *</label><input type="date" name="due"></div>
      <div class="n9-field span2"><label>제품 / Grade *</label><input name="product" list="n9-products" placeholder="절연 슬러리 제품 / Grade"></div>
      <div class="n9-field"><label>수주수량 (kg) *</label><input type="number" min="0" step="0.001" name="qty" placeholder="0"></div>
      <div class="n9-field"><label>단가 (원/kg)</label><input type="number" min="0" step="1" name="unitPrice" placeholder="선택 입력"></div>
      <div class="n9-field span2"><label>고객 PO</label><input name="po" placeholder="고객 발주번호"></div>
      <div class="n9-field span2"><label>납품처</label><input name="deliveryPlace" placeholder="고객 지정 납품처"></div>
      <div class="n9-field full"><label>비고 / 고객 요구사항</label><textarea name="remarks" placeholder="포장, 라벨, CoA, 납기, 운송 등 고객 요구사항"></textarea></div>
  ...
  async function save(form,modal){
    ...
    try{
      localStorage.setItem(SALES,JSON.stringify(next));const mm=readMap(META);mm[id]=meta;localStorage.setItem(META,JSON.stringify(mm));
      if(d.remarks){const rm=readMap(REMARK);rm[id]=d.remarks;localStorage.setItem(REMARK,JSON.stringify(rm))}
      if(typeof window.qmesSyncUpsert==='function')await window.qmesSyncUpsert('inventory','erp:sales',{module:'sales',rows:next,savedAt:now,savedBy:user(),source:'NAMO_NEW_ORDER_MODAL_V9'});
      localStorage.removeItem(DRAFT);window.dispatchEvent(new CustomEvent('qmes:erp-data-changed',{detail:{module:'sales',reason:'new-order',id,orderType:d.orderType}}));window.dispatchEvent(new CustomEvent('qmes:data-updated',{detail:{module:'sales',id,orderType:d.orderType}}));close();
    }catch(err){console.error('[NAMO New Order V9]',err);fail(modal,'수주 저장 중 오류가 발생했습니다. '+clean(err?.message));if(btn){btn.disabled=false;btn.textContent='수주 등록'}}
  }
  ...
  window.qmesSalesNewOrderNamoV9=api;window.qmesSalesNewOrderNamoV8=api;window.qmesSalesNewOrderNamoV7=api;window.qmesSalesNewOrderNamoV6=api;window.qmesSalesNewOrderNamoV5=api;window.qmesSalesNewOrderNamo=api;window.qmesSalesNewOrderEnterprise=api;
})();
