(function(){
  "use strict";
  if(window.__QMES_SYNC_SIDEBAR_V9__) return;
  window.__QMES_SYNC_SIDEBAR_V9__=true;
  ...
  style.textContent=`
    ...
    #qmes-sync-sidebar .qmes-side-head{position:relative!important;display:flex!important;align-items:center!important;justify-content:space-between!important;min-height:48px!important;padding:8px 18px 8px 24px!important;margin:0 -10px 8px!important;border:0!important;border-radius:0!important;background:#334a68!important;box-sizing:border-box!important;overflow:hidden!important;transition:background .15s ease!important}
    #qmes-sync-sidebar .qmes-side-head:before{display:none!important;content:none!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active{background:#dfeaff!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active:before{display:block!important;content:''!important;position:absolute!important;left:0!important;top:9px!important;bottom:9px!important;width:4px!important;border-radius:0 3px 3px 0!important;background:#2563eb!important}
    ...
    #qmes-sync-sidebar .qmes-side-close{width:28px!important;height:28px!important;flex:0 0 28px!important;border:0!important;border-radius:6px!important;background:transparent!important;color:#dbe5f0!important;font-size:19px!important;cursor:pointer!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active .qmes-side-close{color:#5b6f8d!important}
    #qmes-sync-sidebar .qmes-side-close:hover{background:rgba(255,255,255,.1)!important;color:#fff!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active .qmes-side-close:hover{background:#cdddf7!important;color:#175cd3!important}
  ...
  let currentGroup='',activeLabel='',internal=false,groupHighlight=false;
  const head=()=>side.querySelector('.qmes-side-head');
  function syncGroupHighlight(){head()?.classList.toggle('is-group-active',!!groupHighlight);}
  ...
  function setPositions(){
    ...
    if(menuBar){const r=menuBar.getBoundingClientRect();document.documentElement.style.setProperty('--qmes-side-top',Math.round(r.bottom)+'px');}
    if(nav&&dash){const navRect=nav.getBoundingClientRect(),dashRect=dash.getBoundingClientRect(),size=32,slotWidth=52;document.documentElement.style.setProperty('--qmes-hamburger-left',Math.round(navRect.left+(slotWidth-size)/2)+'px');document.documentElement.style.setProperty('--qmes-hamburger-top',Math.round(dashRect.top+(dashRect.height-size)/2)+'px');}
    positionDropdown();
  }
  function settlePositions(){setPositions();requestAnimationFrame(setPositions);setTimeout(setPositions,50);setTimeout(setPositions,120);setTimeout(setPositions,220);}
  function render(group){if(!menuMap[group])return;currentGroup=group;side.querySelector('.qmes-side-title').textContent=group;syncGroupHighlight();const wrap=side.querySelector('.qmes-side-items');wrap.replaceChildren();menuMap[group].forEach((item,index)=>{const button=document.createElement('button');button.type='button';button.className='qmes-side-item'+(activeLabel===item.label?' is-active':'');button.dataset.index=String(index);button.textContent=item.label;wrap.appendChild(button);});settlePositions();}
  function open(group,{highlightTitle=true}={}){if(!menuMap[group])return;groupHighlight=highlightTitle;render(group);document.body.classList.add('qmes-side-open');side.style.setProperty('display','block','important');side.style.setProperty('visibility','visible','important');side.style.setProperty('opacity','1','important');side.style.setProperty('pointer-events','auto','important');side.style.setProperty('transform','translate3d(0,0,0)','important');settlePositions();}
  ...
  function selectSub(item){groupHighlight=false;activeLabel=item.label;render(currentGroup);}
  function navigate(item){if(!item)return;selectSub(item);if(item.direct){const top=findTop(item.direct);if(top){internal=true;top.click();setTimeout(()=>{internal=false;},0);}return;}const existing=findSub(item.sub);if(existing){existing.click();return;}const top=findTop(item.group);if(!top)return;internal=true;top.click();setTimeout(()=>{internal=false;},0);requestAnimationFrame(()=>requestAnimationFrame(()=>{const sub=findSub(item.sub);if(sub)sub.click();}));}
  ...
  hamburger.addEventListener('click',()=>{activeLabel='';open(currentGroup||'대시보드',{highlightTitle:true});});
  document.addEventListener('click',event=>{if(internal)return;const top=event.target.closest('.qmes-top-menu-button');if(!top)return;const label=groups.find(group=>topLabel(top)===group);if(!label)return;activeLabel='';open(label,{highlightTitle:true});settlePositions();},true);
  ...
  document.addEventListener('click',event=>{const dropdownButton=event.target.closest('#qmes-all-menu-dropdown button');if(!dropdownButton)return;groupHighlight=false;activeLabel=clean(dropdownButton.textContent);if(currentGroup)render(currentGroup);},true);
  document.addEventListener('click',event=>{const logoButton=event.target.closest('header button');if(!logoButton||!logoButton.querySelector('img[alt="NAMO Chemical"]'))return;activeLabel='';groupHighlight=false;currentGroup='대시보드';close();setTimeout(()=>{currentGroup='대시보드';settlePositions();},220);},true);
