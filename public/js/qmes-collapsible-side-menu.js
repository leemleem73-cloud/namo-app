(function(){
  "use strict";
  if(window.__QMES_SYNC_SIDEBAR_V10__) return;
  window.__QMES_SYNC_SIDEBAR_V10__=true;
  ...
  style.textContent=`
    ...
    #qmes-sync-sidebar .qmes-side-head{position:relative!important;display:flex!important;align-items:center!important;justify-content:space-between!important;min-height:48px!important;padding:8px 18px 8px 24px!important;margin:0 -10px 8px!important;background:#486482!important;box-sizing:border-box!important;transition:background .12s ease!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active{background:#e4efff!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active:before{content:''!important;position:absolute!important;left:0!important;top:9px!important;bottom:9px!important;width:4px!important;background:#2563eb!important;border-radius:0 3px 3px 0!important}
    #qmes-sync-sidebar .qmes-side-title{font-size:14px!important;font-weight:800!important;color:#fff!important;line-height:20px!important;margin:0!important}
    #qmes-sync-sidebar .qmes-side-head.is-group-active .qmes-side-title{color:#175cd3!important}
    #qmes-sync-sidebar .qmes-side-close{width:28px!important;height:28px!important;flex:0 0 28px!important;border:0!important;border-radius:6px!important;background:transparent!important;color:#e2e8f0!important;font-size:19px!important;cursor:pointer!important}
    ...
    #qmes-sync-hamburger:hover{background:#1e3048!important;color:#fff!important}
  ...
  let currentGroup='',activeLabel='',groupHighlight=false,internal=false;
  ...
  function setPositions(){
    ...
    if(bar)document.documentElement.style.setProperty('--qmes-side-top',Math.round(bar.getBoundingClientRect().bottom)+'px');
    if(nav&&dash){
      const nr=nav.getBoundingClientRect(),dr=dash.getBoundingClientRect(),size=32,slot=52;
      document.documentElement.style.setProperty('--qmes-hamburger-left',Math.round(nr.left+(slot-size)/2)+'px');
  ...
  function render(group){
    ...
    const wrap=side.querySelector('.qmes-side-items');wrap.replaceChildren();
    menuMap[group].forEach((item,index)=>{const b=document.createElement('button');b.type='button';b.className='qmes-side-item'+(activeLabel===item.label?' is-active':'');b.dataset.index=index;b.textContent=item.label;wrap.appendChild(b);});
  }
  function open(group,{titleHighlight=true}={}){
    if(!menuMap[group])return;
    groupHighlight=titleHighlight;render(group);document.body.classList.add('qmes-side-open');settle();
  }
  function close(){document.body.classList.remove('qmes-side-open');settle();}
  ...
  function navigate(item){
    ...
    const sub=findSub(item.sub);if(sub){sub.click();return;}
  ...
  side.addEventListener('click',e=>{if(e.target.closest('.qmes-side-close')){close();return;}const b=e.target.closest('.qmes-side-item');if(!b)return;navigate(menuMap[currentGroup]?.[Number(b.dataset.index)]);});
  hamburger.addEventListener('click',()=>{activeLabel='';open(currentGroup||'대시보드',{titleHighlight:true});});

  document.addEventListener('click',e=>{
    if(internal)return;
    const top=e.target.closest('.qmes-top-menu-button');if(!top)return;
    const group=groups.find(g=>topLabel(top)===g);if(!group)return;
    activeLabel='';open(group,{titleHighlight:true});
    setTimeout(()=>document.getElementById('qmes-all-menu-dropdown')?.classList.remove('is-open'),0);
  },false);

  document.addEventListener('click',e=>{
    const b=e.target.closest('#qmes-all-menu-dropdown button');if(!b)return;
    groupHighlight=false;activeLabel=clean(b.textContent);if(currentGroup)render(currentGroup);
  },true);

  document.addEventListener('click',e=>{
    const logo=e.target.closest('header button');if(!logo||!logo.querySelector('img[alt="NAMO Chemical"]'))return;
    currentGroup='대시보드';activeLabel='';groupHighlight=false;close();
  },true);

  const boot=()=>{setPositions();if(!hamburger.classList.contains('is-ready'))requestAnimationFrame(boot);};
  window.addEventListener('resize',settle);window.addEventListener('load',settle);document.fonts?.ready?.then(settle).catch(()=>{});requestAnimationFrame(boot);
})();