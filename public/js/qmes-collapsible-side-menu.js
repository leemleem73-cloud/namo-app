(function(){
  ...
  const style=document.createElement('style');style.id='qmes-sync-sidebar-style';style.textContent=`
    ...
    #qmes-all-menu-dropdown.qmes-force-pinned{opacity:1!important;visibility:visible!important;pointer-events:auto!important;transform:translateY(0)!important}
  ...
  document.addEventListener('click',e=>{if(internal)return;const top=e.target.closest('.qmes-top-menu-button');if(!top)return;const group=groups.find(g=>topLabel(top)===g);if(!group)return;activeLabel='';open(group,{titleHighlight:true});pinTopMenu();requestAnimationFrame(pinTopMenu);setTimeout(pinTopMenu,50)},true);
  document.addEventListener('click',e=>{const b=e.target.closest('#qmes-all-menu-dropdown button');if(!b)return;groupHighlight=false;activeLabel=clean(b.textContent);if(currentGroup)render(currentGroup);pinTopMenu();requestAnimationFrame(pinTopMenu);setTimeout(pinTopMenu,0);setTimeout(pinTopMenu,100);setTimeout(pinTopMenu,250)},true);
  document.addEventListener('click',e=>{if(e.target.closest('.qmes-top-menu-button')||e.target.closest('#qmes-all-menu-dropdown')||e.target.closest('#qmes-sync-sidebar'))return;unpinTopMenu()},false);
  document.addEventListener('click',e=>{const logo=e.target.closest('header button');if(!logo||!logo.querySelector('img[alt="NAMO Chemical"]'))return;unpinTopMenu();currentGroup='대시보드';activeLabel='';groupHighlight=false;close()},true);

  const observer=new MutationObserver(()=>{if(topMenuPinned)topDropdown()?.classList.add('qmes-force-pinned')});
