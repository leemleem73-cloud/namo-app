...
/* 설비 종합관리 확장 — 기존 일일점검 화면은 그대로 유지 */
(function installEquipmentManagementExtension(){
  ...
  EquipmentTab = function EquipmentManagementTab(){
    ...
    const [scheduleForm,setScheduleForm] = useState(()=>({
      equipmentId:EQUIPMENT[0]?.id || "", type:"예방보전", cycleDays:"30", lastDate:"", nextDate:"", owner:"생산부", note:""
    }));
    ...
    const addSchedule = () => {
      ...
      if(typeof auditLog==="function")auditLog("설비관리","예방보전 일정 등록",row.id,`${row.equipmentId} / ${row.type}`);
      setScheduleForm({...scheduleForm,lastDate:"",nextDate:"",note:""});
      flash("예방보전·교정 일정을 등록했습니다.");
    };
    ...
    const nav=[
      ["daily","일일점검"],["master","설비대장"],["schedule","예방보전·교정"],["repair","고장·수리 이력"]
    ];
    ...
    return <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ["등록 설비",master.length,"대"],["30일 이내 일정",soonCount,"건"],["기한 초과",overdueCount,"건"],["미완료 수리",openRepairs,"건"]
        ].map(([label,value,unit],index)=><div key={label} className={`rounded-xl border p-4 ${index===2&&value>0?"border-red-500/50 bg-red-500/10":index===3&&value>0?"border-amber-500/50 bg-amber-500/10":"border-slate-800 bg-slate-900"}`}><div className="text-xs font-bold text-slate-400">{label}</div><div className="mt-2 text-2xl font-black text-white">{value}<span className="ml-1 text-xs font-medium text-slate-500">{unit}</span></div></div>)}
      </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-800 bg-slate-900 p-2">
        {nav.map(([id,label])=><button key={id} onClick={()=>setSection(id)} className={`rounded-lg border px-4 py-2 text-sm font-bold ${section===id?"border-sky-400 bg-sky-500/15 text-white":"border-slate-700 bg-slate-950 text-slate-400 hover:text-white"}`}>{label}</button>)}
      </div>

      {section==="daily"&&<DailyEquipmentCheckTab/>}

      {section==="master"&&<Panel title="설비대장" right={<button onClick={saveMaster} className={`${smallButton} border-sky-500/50 bg-sky-500/10 text-sky-300`}>전체 저장</button>}>
        <div className="overflow-x-auto -mx-4 px-4"><table className="w-full min-w-[1050px] text-sm"><thead><tr className="border-b border-slate-700 text-xs text-slate-400">{["관리번호","설비명","설치위치","담당부서","제조사","모델","설치일","상태","비고"].map(label=><th key={label} className="px-1 py-2 text-left font-medium">{label}</th>)}</tr></thead><tbody>{master.map(row=><tr key={row.id} className="border-b border-slate-800"><td className="px-1 py-2"><input className={`${inputClass} font-mono`} value={row.managementNo||""} onChange={event=>updateMaster(row.id,"managementNo",event.target.value)}/></td><td className="px-1 py-2"><input className={inputClass} value={row.name||""} onChange={event=>updateMaster(row.id,"name",event.target.value)}/></td><td className="px-1 py-2"><input className={inputClass} value={row.location||""} onChange={event=>updateMaster(row.id,"location",event.target.value)}/></td><td className="px-1 py-2"><input className={inputClass} value={row.owner||""} onChange={event=>updateMaster(row.id,"owner",event.target.value)}/></td><td className="px-1 py-2"><input className={inputClass} value={row.maker||""} onChange={event=>updateMaster(row.id,"maker",event.target.value)}/></td><td className="px-1 py-2"><input className={inputClass} value={row.model||""} onChange={event=>updateMaster(row.id,"model",event.target.value)}/></td><td className="px-1 py-2"><input type="date" className={inputClass} value={row.installDate||""} onChange={event=>updateMaster(row.id,"installDate",event.target.value)}/></td><td className="px-1 py-2"><select className={inputClass} value={row.status||"가동"} onChange={event=>updateMaster(row.id,"status",event.target.value)}><option>가동</option><option>점검중</option><option>사용중지</option><option>폐기</option></select></td><td className="px-1 py-2"><input className={inputClass} value={row.note||""} onChange={event=>updateMaster(row.id,"note",event.target.value)}/></td></tr>)}</tbody></table></div>
        <p className="mt-3 text-[11px] text-slate-500">설비 기본정보와 현재 상태를 관리합니다. 사용중지 설비는 상단에 경고가 표시됩니다.</p>
      </Panel>}

      {section==="schedule"&&<>
        <Panel title="예방보전·교정 일정 등록" right={<span className="text-xs text-slate-500">마지막 실시일과 주기를 입력하면 다음 예정일이 계산됩니다.</span>}>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4 lg:grid-cols-7">
            ...
            <label className="text-xs text-slate-400">구분<select className={`${inputClass} mt-1`} value={scheduleForm.type} onChange={event=>setScheduleForm({...scheduleForm,type:event.target.value})}><option>예방보전</option><option>교정</option><option>정기점검</option><option>부품교체</option></select></label>
        ...
        <Panel title="예방보전·교정 현황" right={<span className="text-xs text-slate-400">총 {scheduleRows.length}건</span>}>
