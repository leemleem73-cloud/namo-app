/* QMES inventory API fallback
 * If /api/inventory/* is not deployed yet, transparently use the existing
 * PostgreSQL-backed /api/qmes-sync/inventory endpoint so inventory remains
 * shared across PCs instead of falling back to browser-only storage.
 */
(function(){
  'use strict';
  if(window.__QMES_INVENTORY_API_FALLBACK__) return;
  window.__QMES_INVENTORY_API_FALLBACK__=true;
  ...
  function stockRows(records){
    const tx=txRows(records),map=new Map();
    ...
    return Array.from(map.values()).filter(r=>Math.abs(Number(r.quantity))>.000001).map(r=>({...r,available_qty:r.quality_status==='AVAILABLE'?Number(r.quantity):0}));
  }
  function summary(records){const stock=stockRows(records),cats={};stock.forEach(r=>{const c=r.category||'RM';cats[c]=cats[c]||{category:c,total_qty:0,available_qty:0,pending_qty:0,hold_qty:0};cats[c].total_qty+=Number(r.quantity)||0;if(r.quality_status==='AVAILABLE')cats[c].available_qty+=Number(r.quantity)||0;if(['IQC_PENDING','OQC_PENDING'].includes(r.quality_status))cats[c].pending_qty+=Number(r.quantity)||0;if(['HOLD','NONCONFORM'].includes(r.quality_status))cats[c].hold_qty+=Number(r.quantity)||0;});return{totals:Object.values(cats),safetyAlerts:[],expiryAlerts:stock.filter(r=>r.expiry_date&&new Date(r.expiry_date)<=new Date(Date.now()+30*86400000)),pendingLots:new Set(stock.filter(r=>['IQC_PENDING','OQC_PENDING'].includes(r.quality_status)).map(r=>r.item_code+'|'+r.lot_no)).size};}
  async function fallback(path,init){
    ...
    if(path.startsWith('/transactions')&&(!init||!init.method||init.method==='GET'))return ok(txRows(records));
    if(path==='/counts'&&(!init||!init.method||init.method==='GET'))return ok(countRows(records));
    if(path==='/reservations'&&(!init||!init.method||init.method==='GET'))return ok([]);
    if(path==='/health')return ok({database:'qmes-sync',module:'inventory-fallback'});
  ...
  window.fetch=async function(input,init){
    ...
    try{
      ...
      const type=String(direct.headers.get('content-type')||'').toLowerCase();
      if(direct.ok&&type.includes('application/json'))return direct;
      return await fallback(path,init||{});
    }catch(_error){return await fallback(path,init||{});}
  };
})();
