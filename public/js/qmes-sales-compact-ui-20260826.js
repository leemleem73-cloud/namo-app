...
(function(){
  "use strict";
  if(window.__QMES_SALES_EDIT_DIRECT_V5__) return;
  window.__QMES_SALES_EDIT_DIRECT_V5__=true;
  ...
  const MODAL_ID="qmes-sales-edit-direct-v5";
  ...
  function ensureStyle(){
    if(document.getElementById("qmes-sales-edit-direct-v5-style")) return;
    const s=document.createElement("style");
    s.id="qmes-sales-edit-direct-v5-style";
    s.textContent=`
  ...
  async function save(form){
    ...
    const id=clean(row.id),key=rowKey(row),metaMap=map(META_KEY),packMap=map(PACK_KEY),remarkMap=map(REMARK_KEY),now=new Date().toISOString();
    const meta={...(metaMap[id]||metaMap[key]||row.orderMeta||{}),customerOverride:get("customer"),poOverride:get("po")||"-",productOverride:get("product"),qtyOverride:qty,requestedDue:get("due"),customerItemCode:get("customerItemCode"),deliveryPlace:get("deliveryPlace"),orderType:get("orderType")||"양산",savedAt:now};
    metaMap[id]=meta;if(key&&key!==id)metaMap[key]=meta;write(META_KEY,metaMap);
    let packaging=null;const touched=Boolean(type||unit||count);if(touched){if(!type||unit<=0||count<=0)return fail("포장정보는 포장형태·단위 포장량·포장수량을 모두 입력하세요.");packaging={type,unitWeight:unit,packageQty:count,total:Number((unit*count).toFixed(3)),savedAt:now};packMap[id]=packaging;if(key&&key!==id)packMap[key]=packaging;}else{delete packMap[id];if(key&&key!==id)delete packMap[key];}write(PACK_KEY,packMap);
    ...
    const current=rows(),exists=current.some(r=>clean(r.id)===id),next=exists?current.map(r=>clean(r.id)===id?updated:r):[updated,...current];write(SALES_KEY,next);
    if(typeof window.qmesSalesFromWorkOrderApply==="function"){try{await window.qmesSalesFromWorkOrderApply();}catch(_){}}
    window.dispatchEvent(new CustomEvent("qmes:erp-data-changed",{detail:{kind:"sales",source:"DIRECT_EDIT_V5"}}));close();setTimeout(()=>window.location.reload(),80);
  }
