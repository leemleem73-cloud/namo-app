...
function showTab(tab) {
  ...
  if (tab === 'dashboard') renderDashboardArea();
}
...
function renderDashboardArea() {
  ...
  if (ncOpenCnt) ncOpenCnt.textContent = `${state.nonconform.filter((r) => String(r.status || '').includes('미')).length}건`;
...
function renderLists() {
  ...
  if (pqcList) iqcList && (pqcList.textContent = renderPqcRows(state.pqc));
...
window.saveIqc = async function () {
  await api('/api/iqc', { method: 'POST', body: JSON.stringify(buildIqcPayload()) });
  await loadAll();
  showTab('iqc');
  alert('수입검사 저장 완료');
};
...
window.savePqc = async function () {
  await api('/api/pqc', { method: 'POST', body: JSON.stringify(buildPqcPayload()) });
  await loadAll();
  showTab('pqc');
  alert('공정검사 저장 완료');
};
...
window.saveOqc = async function () {
  await api('/api/oqc', { method: 'POST', body: JSON.stringify(buildOqcPayload()) });
  await loadAll();
  showTab('oqc');
  alert('출하검사 저장 완료');
};
...
window.saveSupplier = async function () {
  await api('/api/suppliers', { method: 'POST', body: JSON.stringify(buildSupplierPayload()) });
  await loadAll();
  showTab('suppliers');
  alert('협력업체 저장 완료');
};
...
window.saveWorklog = async function () {
  await api('/api/worklog', { method: 'POST', body: JSON.stringify(buildWorklogPayload()) });
  await loadAll();
  showTab('worklog');
  alert('작업일지 저장 완료');
};
...
window.searchTrace = function () {
  ...
  resultEl.textContent = rows.length ? JSON.stringify(rows, null, 2) : '조회 결과가 없습니다.';
};
