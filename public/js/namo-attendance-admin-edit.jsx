...
function AttendancePanel({currentUser,users}){
  ...
  return <div style={{flex:1,minHeight:0,overflow:"auto",padding:14,background:"#f8fafc",position:"relative"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginTop:14}}><button onClick={clockIn} disabled={!!mine?.clockIn} style={{height:54,border:"1px solid rgba(255,255,255,.12)",borderRadius:12,background:mine?.clockIn?"#94a3b8":"linear-gradient(135deg,#0f2740,#1d3f5f)",color:"white",boxShadow:mine?.clockIn?"none":"0 7px 16px rgba(15,39,64,.2)",fontSize:18,fontWeight:900,cursor:mine?.clockIn?"default":"pointer"}}>{mine?.clockIn?`출근 ${mine.clockIn}`:"출근하기"}</button><button onClick={clockOut} disabled={!mine?.clockIn||!!mine?.clockOut} style={{height:54,border:"1px solid rgba(255,255,255,.12)",borderRadius:12,background:mine?.clockOut?"#94a3b8":!mine?.clockIn?"#cbd5e1":"linear-gradient(135deg,#334155,#475569)",color:"white",boxShadow:!mine?.clockIn||mine?.clockOut?"none":"0 7px 16px rgba(51,65,85,.18)",fontSize:18,fontWeight:900,cursor:!mine?.clockIn||mine?.clockOut?"default":"pointer"}}>{mine?.clockOut?`퇴근 ${mine.clockOut}`:"퇴근하기"}</button></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:14}}>{[["현재시간",attendanceTime(now,true),"#0f2740","white","#284b6c"],["출근시간",mine?.clockIn||"미등록","#f8fafc","#0f2740","#cbd5e1"],["퇴근시간",mine?.clockOut||"미등록","#f1f5f9","#334155","#cbd5e1"]].map(([l,v,b,c,border])=><div key={l} style={{background:b,border:`1px solid ${border}`,borderRadius:11,padding:"12px 8px",textAlign:"center",boxShadow:"0 3px 10px rgba(15,39,64,.06)"}}><div style={{fontSize:10,color:l==="현재시간"?"#b9c9d8":"#64748b",fontWeight:700}}>{l}</div><div style={{fontSize:16,fontWeight:900,color:c,marginTop:5}}>{v}</div></div>)}</div>
    <div style={{marginTop:14,padding:12,background:"white",border:"1px solid #dbe3ea",borderRadius:11}}>
      <div style={{fontSize:14,fontWeight:900,color:"#172033"}}>휴가 신청</div>
      <div style={{display:"grid",gridTemplateColumns:"120px 140px 1fr auto",gap:7,marginTop:10}}>
        ...
        <input value={leaveReason} maxLength={100} onChange={event=>setLeaveReason(event.target.value)} onKeyDown={event=>{if(event.key==="Enter")submitLeave();}} placeholder="사유 입력 (선택)" style={{height:38,minWidth:0,boxSizing:"border-box",border:"1px solid #cbd5e1",borderRadius:8,padding:"0 9px"}}/>
        <button type="button" onClick={submitLeave} style={{height:38,padding:"0 14px",border:0,borderRadius:8,background:"#0f2740",color:"white",fontWeight:900,cursor:"pointer"}}>신청</button>
      </div>
      <div style={{marginTop:10,overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:620,fontSize:10}}><thead><tr>{["사용일자","신청자","구분","사유","상태","관리"].map(label=><th key={label} style={{padding:8,textAlign:"left",background:"#f1f5f9",borderBottom:"1px solid #dbe3ea"}}>{label}</th>)}</tr></thead><tbody>{visibleLeaveRequests.length?visibleLeaveRequests.map(request=><tr key={request.id}><td style={{padding:8,borderBottom:"1px solid #eef2f6"}}>{request.date}</td><td style={{padding:8,borderBottom:"1px solid #eef2f6"}}>{request.name}</td><td style={{padding:8,borderBottom:"1px solid #eef2f6",fontWeight:800}}>{request.type}</td><td style={{padding:8,borderBottom:"1px solid #eef2f6"}}>{request.reason||"-"}</td><td style={{padding:8,borderBottom:"1px solid #eef2f6",fontWeight:900,color:request.status==="승인"?"#047857":request.status==="반려"?"#b91c1c":"#b45309"}}>{request.status}</td><td style={{padding:6,borderBottom:"1px solid #eef2f6"}}>{isAdmin&&request.status==="승인대기"?<span style={{display:"flex",gap:4}}><button onClick={()=>updateLeaveStatus(request.id,"승인")} style={{height:27,border:"1px solid #059669",borderRadius:6,background:"#ecfdf5",color:"#047857",fontSize:10,fontWeight:900,cursor:"pointer"}}>승인</button><button onClick={()=>updateLeaveStatus(request.id,"반려")} style={{height:27,border:"1px solid #dc2626",borderRadius:6,background:"#fff1f2",color:"#b91c1c",fontSize:10,fontWeight:900,cursor:"pointer"}}>반려</button></span>:request.name===currentUser.name&&request.status==="승인대기"?<button onClick={()=>cancelLeave(request.id)} style={{height:27,border:"1px solid #94a3b8",borderRadius:6,background:"white",fontSize:10,cursor:"pointer"}}>취소</button>:"-"}</td></tr>):<tr><td colSpan={6} style={{padding:18,textAlign:"center",color:"#94a3b8"}}>휴가 신청 내역이 없습니다.</td></tr>}</tbody></table></div>
    </div>
