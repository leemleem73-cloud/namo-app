          ...
          <>
            <div className="qmes-filter-select-field" style={{width:"150px",minWidth:"150px",maxWidth:"150px",margin:0}}>
              ...
              <select style={{width:"150px",height:"42px",boxSizing:"border-box"}} value={recordYear} onChange={(e)=>{setRecordYear(e.target.value);setRecordPage(1);}}>
                {recordYears.map((year)=><option key={year} value={String(year).replace(/년$/,"")}>{String(year).replace(/년$/,"")}년</option>)}
              </select>
            </div>
            <div className="qmes-filter-select-field" style={{width:"150px",minWidth:"150px",maxWidth:"150px",margin:0}}>
              <span>월</span>
              ...
              {displayRecords.map((r) => {
                ...
                <tr key={r.id} className="border-b border-slate-800/60 hover:bg-slate-800/30">
                  ...
                  <td className="py-2.5 px-2 text-center whitespace-nowrap">
                    <div className="qmes-pqc-actions">
                      <button onClick={() => setViewingRecord(r)} title="성적서 미리보기 및 출력" className="qmes-iqc-action-btn qmes-iqc-action-print qmes-unified-inspection-action"><Printer size={12} /> 출력</button>
                      <button onClick={() => editRecord(r)} className="qmes-iqc-action-btn qmes-iqc-action-edit qmes-unified-inspection-action">수정</button>
                      <button onClick={() => deleteRecord(r)} className="qmes-iqc-action-btn qmes-iqc-action-delete qmes-unified-inspection-action">삭제</button>
                    </div>
                  </td>
                </tr>
              )})}
