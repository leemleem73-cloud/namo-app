...
(function installQmesLoginSyncCoordinator(global){
  ...
  function canAccessCommercialErp(){const user=sessionUser()||{};const name=String(user.name||"").replace(/\s+/g,"").trim();const dept=String(user.department||user.dept||"").replace(/\s+/g,"").trim();return dept==="영업부"||["김종혁","김세희","정영기"].includes(name);}
