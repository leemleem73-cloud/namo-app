/* NAMO QMES - login/sync bootstrap coordinator - 2026-09-02 */
(function installQmesLoginSyncCoordinator(global){
  "use strict";
  if(global.__QMES_LOGIN_SYNC_COORDINATOR_20260902_V6__) return;
  global.__QMES_LOGIN_SYNC_COORDINATOR_20260902_V6__=true;
  const SESSION_KEY="qmes-current-user-v1",QUALITY_UI_SRC="./js/qmes-quality-inspection-ui-20260902.js?v=20260902-iqc-edit-isolated3",OQC_REPORT_FIX_SRC="./js/qmes-quality-report-fix-20260902.js?v=20260902-iqc-oqc-shared6",PQC_REPORT_FIX_SRC="./js/qmes-pqc-report-fix-20260902.js?v=20260902-pqc-final8",CERT_PRINT_FIX_SRC="./js/qmes-certificate-print-fix-20260902.js?v=20260902-cert-print26",SHELL_FIX_SRC="./js/qmes-shell-runtime-fix-20260902.js?v=20260902-iqc-preview4",PURCHASE_TERM_SRC="./js/qmes-purchase-terminology-fix-20260901.js?v=20260901-v1-special-notes",nativeFetch=global.fetch.bind(global);
  ...
  if(!document.getElementById(LOGIN_THEME_STYLE_ID)){
    ...
    style.textContent=`html body:not(:has(.qmes-native-shell)){background:#07162b!important;}html body:not(:has(.qmes-native-shell)) #root#root#root#root,html body:not(:has(.qmes-native-shell)) #root#root#root#root > div{min-height:100vh!important;background:linear-gradient(135deg,#07162b,#0c3156)!important;}html,body,#root,select,option,optgroup{color-scheme:light!important;}html body #root select{color-scheme:light!important;background-color:#fff!important;color:#111827!important;border-color:#cbd5e1!important;}`;
    document.head.appendChild(style);
  }
  let hasSavedSession=false;
  try{hasSavedSession=Boolean(sessionStorage.getItem(SESSION_KEY));}catch(_error){}
  let modulesDeferred=true;
  ...
  function releaseModulesAfterLogin(){if(!modulesDeferred)return;let attempts=0;const release=()=>{if(!modulesDeferred)return;attempts+=1;if(!currentUserReady()){if(attempts<200)global.setTimeout(release,50);return;}modulesDeferred=false;loadQualityInspectionUi();loadPurchaseTerminologyFix();};global.setTimeout(release,0);}
  ...
  global.fetch=async function coordinatedFetch(input,init){const url=urlOf(input);if(isAuthMe(url)){if(authState==="anonymous"&&!hasSavedSession)return nativeFetch(input,{...(init||{}),credentials:(init&&init.credentials)||"same-origin",cache:"no-store"});return ensureAuthCheck();}if(isQmesSync(url)&&authState==="pending"){try{await ensureAuthCheck();}catch(_error){}}const response=await nativeFetch(input,init);if(isAuthLogin(url)&&response.ok){authState="authenticated";hasSavedSession=true;authCheckResponse=null;global.__QMES_AUTH_BOOTSTRAP_STATE__=authState;releaseModulesAfterLogin();}else if(isAuthLogout(url)&&response.ok){authState="anonymous";hasSavedSession=false;authCheckResponse=null;global.__QMES_AUTH_BOOTSTRAP_STATE__=authState;}return response;};
  if(currentUserReady())releaseModulesAfterLogin();
})(window);
