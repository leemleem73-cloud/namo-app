...
(function installQmesLoginSyncCoordinator(global){
  ...
  function isDemoCommercialUser(user){const role=String(user?.role||'').toLowerCase(),id=String(user?.id||'').toLowerCase(),uid=String(user?.uid||'').toLowerCase(),name=String(user?.name||user?.loginId||'').toLowerCase();return role==='guest'||role==='demo'||id==='guest'||id==='demo'||uid==='guest'||uid==='demo'||name.includes('demo')||name.includes('데모');}
  function canAccessCommercialErp(){const user=sessionUser()||{};if(isDemoCommercialUser(user))return true;const name=String(user.name||"").replace(/\s+/g,"").trim();const dept=String(user.department||user.dept||"").replace(/\s+/g,"").trim();return dept==="영업부"||["김종혁","김세희","정영기"].includes(name);}
