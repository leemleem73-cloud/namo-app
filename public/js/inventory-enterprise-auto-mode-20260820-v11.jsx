...
function InventoryEnterpriseTab({section='overview'}){
  ...
  return <div className="inv-shell">
    ...
    {loading?<div className="inv-loading">재고 데이터를 불러오는 중...</div>:<>
      ...
      {section==='movement'&&<div className="inv-panel inv-stock-panel inv-movement-panel"><div className="inv-movement-heading"><h3>자동 입출고 처리 내역</h3><span>일시를 선택하면 상세정보와 바코드를 확인할 수 있습니다.</span></div><TxTable rows={visibleMovementRows} onSelect={setSelectedTx} showSequence sequenceStart={(safeMovementPage-1)*movementPageSize}/>{!movementRows.length&&<div className="inv-empty">입출고 처리 내역이 없습니다.</div>}{movementRows.length>0&&<InventoryPager page={safeMovementPage} pages={movementPages} total={movementRows.length} onPage={setMovementPage}/>}</div>}
