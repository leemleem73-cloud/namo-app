...
(function(){
    ...
    return `<div class="qse-card" role="dialog" aria-modal="true" aria-label="수주 전체 수정">
      ...
      <form class="qse-body" data-qse-form="1" data-qse-id="${esc(row.id)}">
        ...
        <div class="qse-grid">
          ...
          <div class="qse-field"><label>요청 납기일</label><input name="due" type="date" value="${esc(clean(meta.requestedDue)||clean(row.due))}" /></div>
  ...
  function open(id){
    ...
    close();
    const modal=document.createElement("div");modal.id=MODAL_ID;modal.innerHTML=modalHtml(row);document.body.appendChild(modal);
  ...
  async function saveForm(form){
    const id=clean(form.dataset.qseId),row=rowById(id);if(!row)return;
    ...
    const nextRows=rows().map(item=>clean(item.id)===id?{...item,customer:get("customer"),po:get("po")||"-",product:get("product"),qty,due:get("due"),customerItemCode:nextMeta.customerItemCode,deliveryPlace:nextMeta.deliveryPlace,orderType:nextMeta.orderType,remarks,orderMeta:nextMeta,packaging:packTouched?packMap[id]:null,packagingType:packTouched?type:"",unitPackQty:packTouched?unit:0,packageQty:packTouched?count:0}:item);
  ...
  document.addEventListener("click",event=>{
    ...
    if(edit){
      ...
      const row=edit.closest("tr"),id=clean(row?.querySelector("[data-qso-id]")?.getAttribute("data-qso-id")||row?.children?.[0]?.textContent);
      if(id)open(id);
      return;
    }
    if(event.target.closest?.("[data-qse-close]")){event.preventDefault();close();return;}
    if(event.target.id===MODAL_ID)close();
  },true);

  document.addEventListener("submit",event=>{
    const form=event.target.closest?.('[data-qse-form="1"]');if(!form)return;
  ...
  window.qmesSalesFullEdit20260827={open,close};
})();
