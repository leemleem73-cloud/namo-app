...
(function(){
  ...
  async function printPublicLabels(sheet){
    ...
    if(!win)return;
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(material)} QR 라벨</title><style>@page{size:100mm 70mm;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;font-family:'Pretendard','Noto Sans KR',Arial,sans-serif;color:#111827}.label{position:relative;width:100mm;height:70mm;padding:4mm;border:.35mm solid #d0d5dd;overflow:hidden;page-break-after:always}.label:last-child{page-break-after:auto}.top{height:10mm;display:flex;align-items:center;justify-content:space-between;border-bottom:.35mm solid #111827;padding-bottom:1.5mm}.top img{display:block;width:auto;height:8mm;max-width:55mm;object-fit:contain}.pkg{font-size:12px;font-weight:900}.body{display:grid;grid-template-columns:1fr 42mm;gap:3mm;height:49mm;padding-top:3mm}.meta{display:flex;min-width:0;flex-direction:column;justify-content:center}.meta small{font-size:9px;color:#64748b;font-weight:700;margin-top:1.2mm}.meta strong{font-size:14px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.qr{display:flex;align-items:center;justify-content:center}.qr img{width:42mm;height:42mm;object-fit:contain}.scan{position:absolute;left:4mm;bottom:2.2mm;font-size:8px;color:#475467;font-weight:700}@media print{.label{break-inside:avoid}}</style></head><body>${labels.map(label=>`<section class="label"><div class="top"><img src="${MAIN_LOGO}" alt="NAMO Chemical" onerror="this.onerror=null;this.src='${FALLBACK_LOGO}'"><div class="pkg">${label.no} / ${count}</div></div><div class="body"><div class="meta"><small>원료명</small><strong>${escapeHtml(material)}</strong><small>LOT</small><strong>${escapeHtml(lot)}</strong><small>위치</small><strong>${escapeHtml(locationCode)}</strong></div><div class="qr"><img src="${label.qr}" alt="입출고 상세 QR"></div></div><div class="scan">휴대폰 카메라로 QR을 스캔해 상세정보 확인</div></section>`).join('')}</body></html>`);
    win.document.close();
    let printed=false;
