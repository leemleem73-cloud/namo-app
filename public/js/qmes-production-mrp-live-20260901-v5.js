/* NAMO QMES - Live Production Plan / MRP V5 - 2026-09-01
 * Production types: Sample / Development / Mass Production.
 * Product fixed to current NAMO insulation slurry: 절연슬러리(NBA20-HM01).
 ...
 * V5 owns the actual .qerp plan page directly; it does NOT depend on a <main> element.
 */
(function(){
  'use strict';
  if(window.__QMES_LIVE_PRODUCTION_MRP_20260901_V5__)return;
  window.__QMES_LIVE_PRODUCTION_MRP_20260901_V5__=true;

  const HOST='qmes-live-production-mrp-v3';
  const STYLE='qmes-live-production-mrp-v3-style';
  ...
  const PRODUCT_ALIAS_GROUPS=[
    [PRODUCT,'DBA1501','절연슬러리','NBA20-HM01']
  ];
  function productAliases(value){
    const key=norm(value);
    const group=PRODUCT_ALIAS_GROUPS.find(items=>items.some(item=>norm(item)===key));
    return group||[clean(value)];
  }
  function sameProduct(left,right){
    const rightKeys=new Set(productAliases(right).map(norm).filter(Boolean));
    return productAliases(left).map(norm).filter(Boolean).some(key=>rightKeys.has(key));
  }

  let host=null,basePage=null,opened=false,loading=false;
  let state={productionType:'샘플',date:today(),product:PRODUCT,qty:'',products:[PRODUCT],workorders:[],iqcRecords:[],inventoryRecords:[],purchases:[],oqcRecords:[],mrp:[],revision:'-',recipeSource:'-',recipeOfficial:false,error:'',currentPossible:0,currentShortage:0,afterIncomingShortage:0,pendingIqc:0,activeWoCount:0};

  const label=el=>clean(el?.textContent).replace(/\s+/g,'');
  const isPlan=t=>t==='생산계획·MRP'||t==='생산계획MRP';
  function findBasePlan(){
    const pages=[...document.querySelectorAll('#root .qerp')];
    return pages.find(page=>[...page.querySelectorAll('.qerp-title,h1,h2')].some(el=>isPlan(label(el))))||null;
  }
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id=STYLE;s.textContent=`
      #${HOST}{color:#172033;font-family:Pretendard,"Noto Sans KR","Malgun Gothic",Arial,sans-serif;padding-bottom:22px;width:100%}#${HOST} *{box-sizing:border-box}
  ...
  function hasData(value){return !!(value&&typeof value==='object'&&Object.keys(value).length)}
  function allWorkorders(records){
    const byLot=new Map();
    (records||[]).forEach(rec=>{
      const key=clean(rec?.record_key),p=payload(rec);
      if(p?.deleted||/^process:|^downtime:/i.test(key))return;
      const doc=hasData(p?.doc)?p.doc:{},batch=hasData(p?.batch)?p.batch:{},lotRecord=hasData(p?.lotRecord)?p.lotRecord:{};
      const wo=clean(p?.lotNo||batch.lot||batch.no||batch.workOrder||doc.workOrder);
      if(!wo||(!hasData(doc)&&!hasData(batch)))return;
      byLot.set(norm(wo),{recordKey:key,doc,batch,lotRecord,wo,state:first(batch,['status','state','progress'])||first(doc,['status','state','progress']),date:first(batch,['date','prodDate','productionDate'])||first(doc,['date','prodDate','productionDate'])});
    });
    try{Object.entries(window.DB?.woDocs||{}).forEach(([wo,doc])=>{const key=norm(wo),batch=(window.DB?.batches||[]).find(row=>norm(row?.no||row?.lot||row?.workOrder)===key)||{},lotRecord=window.DB?.lots?.[wo]||{};if(!byLot.has(key))byLot.set(key,{recordKey:wo,doc:doc||{},batch,lotRecord,wo,state:first(batch,['status','state','progress'])||first(doc,['status','state','progress']),date:first(batch,['date','prodDate','productionDate'])||first(doc,['date','prodDate','productionDate'])})})}catch(_){ }
    return [...byLot.values()];
  }
  function inspectionRows(records,kind){
    const out=[];
    (records||[]).forEach(rec=>{const p=payload(rec);if(!p?.deleted&&Array.isArray(p?.rows))p.rows.forEach(row=>out.push(row))});
    try{const rows=window.DB?.insp?.[kind];if(Array.isArray(rows))rows.forEach(row=>out.push(row))}catch(_){ }
    return out;
  }
  function workorderCompleted(workorder,oqcRows){
    ...
    if(first(result,['completedAt','completeAt','finishedAt'])||first(workorder.doc,['completedAt','completeAt','finishedAt'])||first(workorder.batch,['completedAt','completeAt','finishedAt']))return true;
    const plan=Math.max(0,num(first(workorder.batch,['plan','planQty','plannedQty','qty'])||first(workorder.doc,['plan','planQty','plannedQty','qty'])));
    const done=Math.max(0,num(first(workorder.batch,['done','productionQty','totalQty'])||first(workorder.doc,['done','productionQty','totalQty'])));
    if(plan>0&&done>=plan)return true;
  ...
  function officialRecipe(product){
    ...
    try{const list=Object.values(window.DB?.recipeMaster||{}).filter(r=>r&&r.active!==false&&(sameProduct(r.productName,product)||sameProduct(r.productCode,product))).sort((a,b)=>clean(b.version).localeCompare(clean(a.version),undefined,{numeric:true}));const r=list[0];if(r&&Array.isArray(r.materials)&&r.materials.length)return {official:true,revision:clean(r.version||r.revision)||'현재 승인',source:'현재 승인 Recipe/BOM',materials:r.materials.map(m=>({code:clean(m.code),name:clean(m.name),ratio:num(m.ratio),unit:clean(m.unit)||'kg'})).filter(m=>m.name&&m.ratio>0)}}catch(_){ }
    return null;
  }
  function workorderRecipe(product,workorders){
    const matches=workorders.filter(w=>sameProduct(workorderProduct(w),product)&&materialRows(w.doc).length).sort((a,b)=>clean(b.date).localeCompare(clean(a.date)));
  ...
  function recipeFor(product,workorders,type){const official=officialRecipe(product);if(official)return official;if(type==='양산')return null;return workorderRecipe(product,workorders)}

  function materialMatch(row,mat){const keys=[mat.code,mat.name].map(norm).filter(Boolean),vals=[first(row,['item_code','itemCode','materialCode','code']),first(row,['item_name','itemName','materialName','material','name','원료명'])].map(norm).filter(Boolean);return keys.some(k=>vals.includes(k))}
  function onHand(records,mat){let total=0;(records||[]).forEach(rec=>{const p=payload(rec);if(p?.kind!=='transaction')return;const tx=p.transaction||{};if(!materialMatch(tx,mat))return;const cat=clean(tx.category).toUpperCase();if(cat&&cat!=='RM'&&cat!=='PM')return;const q=Math.max(0,num(tx.quantity)),type=clean(tx.transaction_type).toUpperCase();if(/RECEIPT|RETURN|ADJUSTMENT_IN/.test(type))total+=q;else if(/PRODUCTION_ISSUE|ISSUE|SCRAP|ADJUSTMENT_OUT/.test(type))total-=q});return Math.max(0,total)}
  function reserved(workorders,mat){let total=0;workorders.forEach(w=>materialRows(w.doc).forEach(r=>{const probe={itemCode:first(r,['code','itemCode','materialCode']),itemName:first(r,['name','material','materialName','item','itemName'])};if(materialMatch(probe,mat))total+=Math.max(0,num(first(r,['planQty','plannedQty','requiredQty','qty','quantity','std','소요량','계획량'])))}));return total}
  ...
  function purchaseRows(records){let rows=[];(records||[]).forEach(rec=>{if(clean(rec?.record_key)!=='erp:purchase')return;const p=payload(rec);if(Array.isArray(p.rows))rows=p.rows});if(!rows.length){const local=readLocal('qmes-erp-purchase-v1',[]);if(Array.isArray(local))rows=local}return rows.filter(r=>!(clean(r?.id)==='PO-260824-01'&&/Supplier A/i.test(clean(r?.supplier)))&&!(clean(r?.id)==='PO-260824-02'&&/Supplier B/i.test(clean(r?.supplier))))}
  function incomingBefore(rows,mat,date){let qty=0,count=0;rows.forEach(r=>{if(!materialMatch({itemCode:first(r,['itemCode','code','materialCode']),itemName:first(r,['material','materialName','item','itemName'])},mat))return;const status=clean(r?.status),iqc=clean(r?.iqc);if(/취소|완료|입고완료/i.test(status)||pass(iqc)||fail(iqc))return;const d=clean(r?.expected||r?.expectedDate||r?.due||r?.dueDate);if(date&&d&&d>date)return;const q=Math.max(0,num(r?.qty??r?.quantity));if(q>0){qty+=q;count++}});return {qty,count}}

  function calculate(){
    const qty=Math.max(0,num(state.qty)),all=allWorkorders(state.workorders),oqc=inspectionRows(state.oqcRecords,'OQC'),active=all.filter(workorder=>!workorderCompleted(workorder,oqc)),recipe=recipeFor(PRODUCT,all,state.productionType),iqc=iqcRows(state.iqcRecords);
    if(qty<=0){state={...state,product:PRODUCT,mrp:[],revision:recipe?.revision||'-',recipeSource:recipe?.source||'-',recipeOfficial:!!recipe?.official,error:'',currentPossible:0,currentShortage:0,afterIncomingShortage:0,pendingIqc:0,activeWoCount:active.length};render();return}
    if(!recipe){const msg=state.productionType==='양산'?'양산 생산계획은 절연슬러리(NBA20-HM01)의 현재 승인 Recipe/BOM이 반드시 필요합니다. Recipe/BOM 승인 후 다시 계산하세요.':'절연슬러리(NBA20-HM01)의 Recipe/BOM 또는 실제 작업지시 원료계획을 찾을 수 없습니다.';state={...state,product:PRODUCT,mrp:[],revision:'미등록',recipeSource:'-',recipeOfficial:false,error:msg,currentPossible:0,currentShortage:0,afterIncomingShortage:0,pendingIqc:0,activeWoCount:active.length};render();return}
    const totalRatio=recipe.materials.reduce((s,m)=>s+Math.max(0,num(m.ratio)),0)||100;let pendingTotal=0;
    const mrp=recipe.materials.map(mat=>{const need=qty*Math.max(0,num(mat.ratio))/totalRatio,stock=onHand(state.inventoryRecords,mat),hold=reserved(active,mat),usable=Math.max(0,stock-hold),inc=incomingBefore(state.purchases,mat,state.date),curShort=Math.max(0,need-usable),afterShort=Math.max(0,need-usable-inc.qty),pending=pendingIqc(iqc,mat);pendingTotal+=pending;let status='충족',tone='green';if(generic(mat.name)){status='실제 원료명 확인';tone='orange'}else if(afterShort>0){status='부족 / 발주 검토';tone='red'}else if(curShort>0){status='입고 조건부';tone='orange'}else if(pending>0){status='IQC 대기 확인';tone='orange'}return {...mat,need,stock,reserved:hold,usable,incoming:inc.qty,currentShort:curShort,afterShort,pending,status,tone}});
    ...
    state={...state,product:PRODUCT,mrp,revision:recipe.revision||'-',recipeSource:recipe.source||'-',recipeOfficial:!!recipe.official,error:'',currentPossible:possible,currentShortage:mrp.filter(r=>r.currentShort>0).length,afterIncomingShortage:mrp.filter(r=>r.afterShort>0).length,pendingIqc:pendingTotal,activeWoCount:active.length};render();
  }

  async function loadData(){if(loading)return;loading=true;render();try{if(typeof window.qmesReconcileInventory==='function')await window.qmesReconcileInventory()}catch(_){ }const [workorders,iqc,inventory,oqc]=await Promise.all([syncList('workorder'),syncList('iqc'),syncList('inventory'),syncList('oqc')]);state={...state,product:PRODUCT,products:productsFromLive(allWorkorders(workorders)),workorders,iqcRecords:iqc,inventoryRecords:inventory,purchases:purchaseRows(inventory),oqcRecords:oqc};loading=false;calculate()}

  async function savePlan(){const qty=Math.max(0,num(state.qty));if(qty<=0){window.alert('생산계획량을 입력하세요.');return}if(!state.mrp.length){window.alert('Recipe/BOM 확인 후 MRP를 계산하세요.');return}if(state.productionType==='양산'&&!state.recipeOfficial){window.alert('양산은 승인된 현재 Recipe/BOM으로만 저장할 수 있습니다.');return}const row={date:state.date,product:PRODUCT,qty,revision:state.revision,productionType:state.productionType,source:'LIVE_PRODUCTION_MRP_V4',savedAt:new Date().toISOString()};writeLocal(PLAN_KEY,[row]);try{if(typeof window.qmesSyncUpsert==='function')await window.qmesSyncUpsert('inventory','erp:plan',{module:'erp',schema:4,kind:'plan',stage:state.productionType,rows:[row],updatedAt:new Date().toISOString()})}catch(e){console.warn('[QMES MRP] plan sync',e)}window.dispatchEvent(new CustomEvent('qmes:erp-data-changed',{detail:{kind:'plan',stage:state.productionType}}));window.alert(`${state.productionType} 생산계획을 저장했습니다.`)}

  function esc(v){return clean(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  function render(){
    if(!host)return;
    const rows=state.mrp.length?state.mrp.map(r=>`<tr><td><b>${esc(r.code)||'-'}</b></td><td><b>${esc(r.name)}</b>${generic(r.name)?'<div style="margin-top:2px;color:#c2410c;font-size:9px">실제 원료명 등록 필요</div>':''}</td><td>${num(r.ratio).toFixed(3)}%</td><td>${fmt(r.need)}</td><td>${fmt(r.stock)}</td><td>${fmt(r.reserved)}</td><td>${fmt(r.usable)}</td><td>${fmt(r.incoming)}</td><td>${r.currentShort>0?`<b style="color:#dc2626">${fmt(r.currentShort)}</b>`:'0 kg'}</td><td>${r.afterShort>0?`<b style="color:#dc2626">${fmt(r.afterShort)}</b>`:'0 kg'}</td><td>${r.pending}건</td><td><span class="status ${r.tone}">${esc(r.status)}</span></td></tr>`).join(''):`<tr><td colspan="12" style="text-align:center;color:#94a3b8;padding:28px">${loading?'실제 작업지시·IQC·재고 데이터를 불러오는 중입니다.':'생산구분과 생산계획량을 입력하면 실제 현황 기준으로 계산합니다.'}</td></tr>`;
    host.innerHTML=`<div class="mrp-head"><div><h1>생산계획 · MRP</h1><div class="mrp-sub">제품: <b>${PRODUCT}</b> · 생산구분 <b>샘플 / 개발 / 양산</b> · 실제 작업지시·IQC·재고·입고예정·Recipe/BOM 연계</div></div><div class="mrp-actions"><span class="sync">공용 DB 실제현황</span><button data-refresh ${loading?'disabled':''}>${loading?'불러오는 중':'MRP 재계산'}</button><button data-save>계획 저장</button></div></div>${state.error?`<div class="error">${esc(state.error)}</div>`:''}<div class="info">현재 가용재고는 <b>IQC 합격 후 재고원장에 입고된 원료</b>에서 <b>전체 미완료 작업지시 예약량</b>을 차감해 계산합니다. 입고예정은 현재재고에 포함하지 않고 생산예정일까지의 조건부 수급으로 별도 표시합니다. <b>양산은 현재 승인 Recipe/BOM이 없으면 계획 저장을 차단합니다.</b></div>
      <div class="grid2"><section class="card"><h2>생산계획 입력</h2><div class="form"><div class="field"><label>생산구분</label><select data-type><option value="샘플" ${state.productionType==='샘플'?'selected':''}>샘플</option><option value="개발" ${state.productionType==='개발'?'selected':''}>개발</option><option value="양산" ${state.productionType==='양산'?'selected':''}>양산</option></select></div><div class="field"><label>생산예정일</label><input type="date" data-date value="${esc(state.date)}"></div><div class="field"><label>제품명</label><input readonly value="${PRODUCT}" title="현재 나모케미칼 절연슬러리 제품"></div><div class="field"><label>생산계획량 (kg)</label><input inputmode="decimal" data-qty value="${esc(state.qty)}" placeholder="0"></div><div class="field"><label>적용 Recipe Rev.</label><input readonly value="${esc(state.revision)}" title="${esc(state.recipeSource)}"></div></div></section>
  ...
  function open(){
    const base=findBasePlan();if(!base)return false;ensureStyle();opened=true;basePage=base;
    if(!host||!host.isConnected){host=document.createElement('div');host.id=HOST;base.parentElement.insertBefore(host,base)}
    base.style.setProperty('display','none','important');render();loadData();return true;
  }
  function close(){opened=false;if(host?.isConnected)host.remove();host=null;if(basePage?.isConnected)basePage.style.removeProperty('display');basePage=null}

  let queued=false;
  function scan(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;const base=findBasePlan();if(base){if(!opened||!host?.isConnected)open();else if(basePage!==base){if(basePage?.isConnected)basePage.style.removeProperty('display');basePage=base;base.style.setProperty('display','none','important')}}else if(opened)close()})}
  const observer=new MutationObserver(scan);observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('qmes:erp-runtime-loaded',()=>setTimeout(scan,60));window.addEventListener('qmes:inventory-reconciled',()=>{if(opened)loadData()});window.addEventListener('qmes:mes-master-ready',()=>setTimeout(scan,60));
  document.addEventListener('click',()=>setTimeout(scan,40),true);
  [0,100,350,800,1600,3000].forEach(ms=>setTimeout(scan,ms));
})();
