/* QMES draggable layout sizing - 2026-09-21
 * ADD-ONLY patch.
...
(function(){
  "use strict";
  if(window.__QMES_LAYOUT_RESIZABLE_20260921__) return;
  window.__QMES_LAYOUT_RESIZABLE_20260921__=true;
  ...
  const readSide=()=>{
    ...
    return Number.isFinite(n)?clamp(n,MIN_SIDE,MAX_SIDE):236;
  };
  ...
  function installColumnHandles(){
    ...
    info.headers.forEach((th,index)=>{
      ...
      handle.title="좌우로 드래그하여 열 너비 조절";
  ...
  function installSidebarHandle(){
    ...
    handle.title="좌우로 드래그하여 왼쪽 메뉴 너비 조절";
