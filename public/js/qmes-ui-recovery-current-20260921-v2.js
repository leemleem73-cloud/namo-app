(function(){
  'use strict';
  if(window.__QMES_UI_RECOVERY_CURRENT_20260921_V2__) return;
  window.__QMES_UI_RECOVERY_CURRENT_20260921_V2__=true;
  ...
  function dispatchRoute(config,label){
    ...
    if(config.section){
      window.dispatchEvent(new CustomEvent('qmes:inventory-section',{detail:{section:config.section}}));
    }
  }

  /* CURRENT 2026-09-21:
   * Sidebar navigation is owned only by qmes-collapsible-side-menu-20260921-v2.js.
   * The legacy recovery click interceptor is intentionally removed because it
   * dispatched duplicate route changes and caused visible main-screen flicker.
   */
