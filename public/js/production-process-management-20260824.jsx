...
function ProductionProcessTab() {
  ...
  const addDefect = async () => {
    ...
    const step = process.steps?.[selectedStep];
    if (!step) return;
    const value = window.prompt(`${step.name} 불량수량을 입력하세요.`, step.defectQty || "0");
    ...
    setBusy(true);
    try { await persistProcess({ ...process, steps }); setMessage("불량수량을 저장했습니다."); }
  ...
  const addDowntime = async () => {
    if (!process) return;
    const reason = window.prompt("비가동 사유를 입력하세요. (예: 설비점검, 원료대기)", "");
    if (reason == null || !qmesProcessClean(reason)) return;
    const now = qmesProcessNow();
    const history = Array.isArray(process.downtime) ? process.downtime : [];
    setBusy(true);
    try {
      await persistProcess({ ...process, downtime: [...history, { stepNo: selectedStep + 1, reason: qmesProcessClean(reason), at: now, workers: selectedWorkers.map(worker => worker.name) }] });
  ...
  return (
    <div className="qmes-prod-process">
      <style>{`
        ...
        .qpp-card{border:1px solid #243d58;border-radius:11px;background:#0d1f33;overflow:hidden}.qpp-card-head{display:flex;align-items:center;justify-content:space-between;padding:13px 15px;border-bottom:1px solid #213b56}.qpp-card-head b{font-size:14px}.qpp-card-head span{color:#7dd3fc;font-size:11px}
        ...
        .qpp-grid{display:grid;grid-template-columns:minmax(0,1fr) 315px;gap:12px;margin-top:12px}.qpp-table-wrap{overflow:auto}.qpp-table{width:100%;min-width:820px;border-collapse:collapse;table-layout:fixed}.qpp-table th{padding:10px;background:#10263e;color:#7895af;border-bottom:1px solid #2a445f;text-align:left;font-size:10px}.qpp-table td{padding:10px;border-bottom:1px solid #1e3852;color:#dbe7f2;font-size:11px}.qpp-table tbody tr{cursor:pointer}.qpp-table tbody tr:hover{background:#132c47}.qpp-table tbody tr.active{background:#1c3044;box-shadow:inset 4px 0 0 #f43f5e}.qpp-center{text-align:center}.qpp-right{text-align:right}.qpp-status{display:inline-flex;padding:4px 7px;border-radius:999px;font-size:10px;font-weight:900}.qpp-status.done{background:#123d32;color:#6ee7b7}.qpp-status.run{background:#0e3b57;color:#7dd3fc}.qpp-status.wait{background:#253349;color:#a8b8c8}
        .qpp-side{padding:13px}.qpp-sidebox{padding:12px;border:1px solid #294761;border-radius:8px;background:#122a44;margin-bottom:9px}.qpp-sidebox small{display:block;color:#7895af;font-size:10px;margin-bottom:5px}.qpp-sidebox strong{font-size:15px}.qpp-sidebox p{margin:6px 0 0;color:#9bb0c3;font-size:11px;line-height:1.6}.qpp-actionbar{display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:8px;margin-top:12px}.qpp-actionbar button{min-height:50px}
        ...
        @media(max-width:1100px){.qpp-info{grid-template-columns:repeat(3,1fr)}.qpp-grid{grid-template-columns:1fr}.qpp-actionbar{grid-template-columns:repeat(3,1fr)}}@media(max-width:700px){.qpp-head{align-items:flex-start;flex-direction:column}.qpp-toolbar{width:100%;flex-wrap:wrap}.qpp-select{width:100%;min-width:0}.qpp-info{grid-template-columns:repeat(2,1fr)}.qpp-worker{align-items:flex-start;flex-direction:column}.qpp-actionbar{grid-template-columns:repeat(2,1fr)}.qpp-workers{grid-template-columns:repeat(2,1fr)}.qpp-form{grid-template-columns:1fr}}
      `}</style>
      ...
      {selectedLot && process && <>
        ...
        <div className="qpp-grid">
          <section className="qpp-card">
            ...
            <div className="qpp-table-wrap"><table className="qpp-table"><thead><tr><th style={{width:"7%"}}>순번</th><th style={{width:"23%"}}>공정</th><th style={{width:"17%"}}>설비</th><th style={{width:"14%"}}>시작</th><th style={{width:"14%"}}>종료</th><th style={{width:"12%"}}>불량</th><th style={{width:"13%"}}>상태</th></tr></thead><tbody>
        ...
        <div className="qpp-actionbar">
          ...
          <button type="button" className="qpp-btn red" disabled={busy} onClick={addDefect}>불량 등록</button>
          <button type="button" className="qpp-btn amber" disabled={busy} onClick={addDowntime}>비가동 등록</button>
          <button type="button" className="qpp-btn" onClick={() => alert(`LOT ${selectedLot}\n품목 ${item}\n설비 ${equipment}\n\n작업표준/도면 문서 연결 영역입니다.`)}>작업표준 / 도면</button>
          <button type="button" className="qpp-btn" onClick={() => window.print()}>LOT / 공정 출력</button>
        </div>
      </>}
