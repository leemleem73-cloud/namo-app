...
const IN_SRC='/attendance-card-clockin-20260917.svg?v=20260917-final5';
const OUT_SRC='/attendance-card-clockout-20260917.svg?v=20260917-final5';
const $=(s,r=document)=>r.querySelector(s);
let clockTimer=null,queued=false,revealed=false,observer=null;
...
html[data-namo-attendance-full-ui="v4"] #namoTodayDate{display:flex!important;align-items:center!important;justify-content:flex-start!important;gap:10px!important;width:168px!important;min-width:168px!important;padding:10px 0 10px 14px!important;border-left:1px solid #cbd9e8!important;color:#263a5b!important;white-space:normal!important;font-variant-numeric:tabular-nums!important}
html[data-namo-attendance-full-ui="v4"] #namoTodayDate:before{display:none!important;content:none!important}
html[data-namo-attendance-full-ui="v4"] .namo-digital-clock{display:flex!important;align-items:center!important;justify-content:center!important;min-width:58px!important;height:34px!important;padding:0 8px!important;border-radius:9px!important;background:#0f6fcf!important;color:#fff!important;font-size:16px!important;line-height:1!important;font-weight:950!important;letter-spacing:.4px!important;box-shadow:0 3px 10px rgba(15,111,207,.18)!important;box-sizing:border-box!important}
html[data-namo-attendance-full-ui="v4"] .namo-date-copy{display:flex!important;flex-direction:column!important;gap:3px!important;line-height:1.18!important;min-width:0!important}
...
html[data-namo-attendance-full-ui="v4"] .namo-date-time-label{font-size:10px!important;font-weight:800!important;color:#718096!important;white-space:nowrap!important}
...
function ensureDigitalClock(){
  ...
  if(!holder.querySelector('.namo-digital-clock')){
    ...
    const label=document.createElement('span');label.className='namo-date-time-label';label.textContent='현재시간';
    copy.append(date,label);holder.append(digital,copy);
  }
...
function updateDigitalClock(){
  ...
  const nextDate=`${now.getFullYear()}년 ${now.getMonth()+1}월 ${now.getDate()}일 (${days[now.getDay()]})`;
...
function schedule(){if(queued||revealed)return;queued=true;requestAnimationFrame(run)}
observer=new MutationObserver(schedule);
observer.observe(document.documentElement,{subtree:true,childList:true});
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
clockTimer=setInterval(updateDigitalClock,1000);
window.addEventListener('pageshow',()=>{if(!revealed)schedule();else updateDigitalClock()});
