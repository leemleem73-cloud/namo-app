...
function askInitialMailLink(me){
  return new Promise(resolve=>{
    ...
    overlay.innerHTML=`<div style="width:min(420px,100%);background:#fff;border-radius:18px;padding:20px;box-shadow:0 18px 50px rgba(15,31,55,.28)">
      ...
      <div style="margin-top:8px;font-size:11px;line-height:1.5;color:#7b8798">인증정보는 서버 DB에 암호화 저장되며 화면에는 다시 표시되지 않습니다.</div>
...
async function doSend(payload){
  return api('/api/attendance/direct-mail',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
}
...
async function sendDirectMail(button){
  ...
  try{
    ...
    try{
      ...
      await api('/api/attendance/mail-link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password})});
      button.textContent=`${me.name||'본인'} 메일로 바로 보내는 중...`;
      result=await doSend(payload);
    }
...
function prepareButton(){
  ...
  if(!button||button.dataset.directMail==='5')return;
  button.dataset.directMail='5';
