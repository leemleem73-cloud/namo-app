...
async function loadData(){
  try{
    ...
    bindApprovedRows();
  }catch(e){console.warn('[NAMO approved detail TEST]',e)}
}
...
function normalizedItem(item){
  ...
  return {...item,
    employee_name:item?.employee_name||me.name||'-',
    employee_department:item?.employee_department||me.department||'-',
    reviewer_name:item?.reviewer_name||reviewer?.name||'-',
    reviewer_email:item?.reviewer_email||reviewer?.email||''
  };
}
...
function openApprovedDetail(raw){
  ...
  if(reviewerText){
    const when=fmtDateTime(reviewedAt(item));
    reviewerText.textContent=`${item.reviewer_name||'검토자'}${when?` · ${when}`:''}`;
  }
