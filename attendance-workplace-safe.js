...
function install(app){
  ...
  app.put('/api/attendance/workplace-v2',requireLogin,async(req,res)=>{try{await ensureSchema();const place=wp(req.body?.workplaceCode);if(!place)return fail(res,400,'변경할 근무지를 선택해주세요.');const q=await pool.query("UPDATE attendance_logs SET workplace_code=$2,workplace_name=$3,workplace_address=$4,updated_at=NOW() WHERE user_id=$1 AND work_date=(NOW() AT TIME ZONE 'Asia/Seoul')::date RETURNING *",[req.session.user.id,place.code,place.name,place.address]);if(!q.rowCount)return ok(res,{workplaceCode:place.code,workplaceName:place.name,workplaceAddress:place.address},`${place.name}을(를) 다음 출근 근무지로 선택했습니다.`);return ok(res,dto(q.rows[0]),`${place.name}으로 근무지를 변경했습니다.`)}catch(e){console.error('[Attendance workplace update]',e);return fail(res,500,'근무지 변경에 실패했습니다.')}});
