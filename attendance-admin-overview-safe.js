...
function installClient(){try{const file=path.resolve(__dirname,'public','attendance.html');if(!fs.existsSync(file))return;let html=fs.readFileSync(file,'utf8');html=html.replace(/<script src="\/attendance-admin-overview\.js\?v=[^"]+"><\/script>/g,'').replace(/<script src="\/attendance-mobile-member-sync\.js\?v=[^"]+"><\/script>/g,'');html=html.replace('</body>','<script src="/attendance-admin-overview.js?v=20260907-statutory1"></script><script src="/attendance-mobile-member-sync.js?v=20260907-mobile-delete1"></script></body>');fs.writeFileSync(file,html,'utf8');console.log('[Attendance admin overview] admin clients installed')}catch(e){console.error('[Attendance admin overview] client install failed',e)}}
installClient();
