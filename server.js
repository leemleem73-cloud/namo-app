  ...
  try {
    ...
    await runAsync(
      `INSERT INTO iqc (id, date, lot, supplier, item, inspector, qty, fail)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.id, data.date, data.lot, data.supplier, data.item, data.inspector, data.qty || 0, data.fail || 0]
    );
  ...
  try {
    ...
    await runAsync(
      `UPDATE iqc
       ...
       WHERE id=?`,
      [d.date, d.lot, d.supplier, d.item, d.inspector, d.qty || 0, d.fail || 0, req.params.id]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `INSERT INTO ipqc (id, date, product, lot, visual, viscosity, solid, particle, qty, fail, judge)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.id, data.date, data.product, data.lot, data.visual, data.viscosity, data.solid, data.particle, data.qty || 0, data.fail || 0, data.judge || '합격']
    );
    res.json({ ok: true, id: data.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `UPDATE ipqc
       ...
       WHERE id=?`,
      [d.date, d.product, d.lot, d.visual, d.viscosity, d.solid, d.particle, d.qty || 0, d.fail || 0, d.judge || '합격', req.params.id]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `INSERT INTO oqc (id, date, customer, product, lot, visual, viscosity, solid, particle, adhesion, resistance, swelling, moisture, qty, fail, judge)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.id, data.date, data.customer, data.product, data.lot, data.visual, data.viscosity, data.solid, data.particle, data.adhesion, data.resistance, data.swelling, data.moisture, data.qty || 0, data.fail || 0, data.judge || '합격']
    );
    res.json({ ok: true, id: data.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `UPDATE oqc
       ...
       WHERE id=?`,
      [d.date, d.customer, d.product, d.lot, d.visual, d.viscosity, d.solid, d.particle, d.adhesion, d.resistance, d.swelling, d.moisture, d.qty || 0, d.fail || 0, d.judge || '합격', req.params.id]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `INSERT INTO suppliers (id, name, manager, phone, category, status)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [data.id, data.name, data.manager, data.phone, data.category, data.status]
    );
    res.json({ ok: true, id: data.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `UPDATE suppliers
       ...
       WHERE id=?`,
      [d.name, d.manager, d.phone, d.category, d.status, req.params.id]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `INSERT INTO worklog (id, workDate, finishedLot, seq, material, supName, inputQty, inputRatio, lotNo, inputTime, worker, note)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.id, data.workDate, data.finishedLot, data.seq, data.material, data.supName, data.inputQty, data.inputRatio, data.lotNo, data.inputTime, data.worker, data.note]
    );
    res.json({ ok: true, id: data.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `UPDATE worklog
       ...
       WHERE id=?`,
      [d.workDate, d.finishedLot, d.seq, d.material, d.supName, d.inputQty, d.inputRatio, d.lotNo, d.inputTime, d.worker, d.note, req.params.id]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  ...
  try {
    ...
    await runAsync(
      `INSERT INTO nonconform (id, date, type, lot, item, issue, cause, action, owner, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.id, data.date, data.type, data.lot, data.item, data.issue, data.cause, data.action, data.owner, data.status]
    );
  ...
  try {
    ...
    await runAsync(
      `UPDATE nonconform
       ...
       WHERE id=?`,
      [d.date, d.type, d.lot, d.item, d.issue, d.cause, d.action, d.owner, d.status, req.params.id]
    );
