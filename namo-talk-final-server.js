...
function install(app){
  ...
  app.get(P+'/backup-export',async(req,res)=>{
    ...
    try{
      ...
      const [accounts,messages,reads,settings,channels,members,prefs,attachments]=await Promise.all([
        ...
        client.query('SELECT * FROM namo_talk_standalone_room_preferences ORDER BY user_name,peer_name'),
        client.query(`SELECT id,file_name,mime_type,file_size,created_at,
          CASE WHEN file_data IS NULL THEN NULL ELSE encode(file_data,'base64') END AS "fileDataBase64"
          FROM namo_talk_standalone_attachments ORDER BY id`)
      ]);
      const data={format:'namo-talk-pc-backup-v1',createdAt:new Date().toISOString(),createdBy:me.name,
        accounts:accounts.rows,messages:messages.rows,channelReads:reads.rows,settings:settings.rows,
        channels:channels.rows,channelMembers:members.rows,roomPreferences:prefs.rows,attachments:attachments.rows};
      const counts={accounts:data.accounts.length,messages:data.messages.length,attachments:data.attachments.length,
        channelReads:data.channelReads.length,channels:data.channels.length,channelMembers:data.channelMembers.length,
        roomPreferences:data.roomPreferences.length};
      await client.query('COMMIT');
      ok(res,{backup:data,counts,restoreEnabled:false,storage:'client-pc'});
