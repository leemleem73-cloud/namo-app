...
function reviewerKind(u){
  const title=String(u?.title||'').replace(/\s/g,'');
  const role=String(u?.role||'').toLowerCase();
  const name=String(u?.name||'').replace(/\s/g,'');
...
async function reviewerList(){
  ...
  return q.rows
    .filter(u=>activeStatus(u.status)&&reviewerEligible(u))
...
function install(app){
  ...
  app.get('/api/attendance/reviewers',requireLogin,async(req,res)=>{
    try{await ensureSchema();return ok(res,await reviewerList());}catch(e){console.error('[Attendance reviewers]',e);return fail(res,500,'검토자 목록을 불러오지 못했습니다.');}
  });
