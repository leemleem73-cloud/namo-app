...
function install(app){
  if(app.__namoMobileDailyAttendanceInstalled)return;
  app.__namoMobileDailyAttendanceInstalled=true;

  app.get('/api/attendance/admin/daily',requireLogin,requireAdmin,async(req,res)=>{
    try{
      await ensureSchema();
      const workDate=dateOk(req.query.date)?String(req.query.date):todayKst();
      ...
      const rows=q.rows.map(r=>({
        ...
        attendanceUpdatedAt:r.attendance_updated_at,
        leaveType:r.leave_type||'',
        leaveDays:Number(r.leave_days||0),
        leaveStatus:r.leave_status||'',
        correctionPending:Number(r.correction_pending||0),
        monthDays:Number(r.month_days||0),
        monthMinutes:Number(r.month_minutes||0)
      }));
      return ok(res,{date:workDate,rows});
    }catch(e){console.error('[Mobile daily attendance] list',e);return fail(res,500,'일 근무관리 현황을 불러오지 못했습니다.');}
  });
  ...
  app.get('/api/attendance/admin/daily/:userId/:date/history',requireLogin,requireAdmin,async(req,res)=>{
    try{
      ...
      const q=await pool.query(`
        SELECT id,original_clock_in,original_clock_out,new_clock_in,new_clock_out,reason,editor_name,created_at
        FROM attendance_admin_adjustments
        WHERE user_id=$1 AND work_date=$2::date
        ORDER BY created_at DESC LIMIT 50
      `,[req.params.userId,req.params.date]);
      return ok(res,q.rows);
    }catch(e){console.error('[Mobile daily attendance] history',e);return fail(res,500,'수정 이력을 불러오지 못했습니다.');}
  });
  ...
  app.put('/api/attendance/admin/daily/:userId/:date',requireLogin,requireAdmin,async(req,res)=>{
    ...
    try{
      ...
      if(!dateOk(workDate))return fail(res,400,'근무일을 확인해주세요.');
      ...
      const current=await client.query('SELECT * FROM attendance_logs WHERE user_id=$1 AND work_date=$2::date FOR UPDATE',[userId,workDate]);
      ...
      if(before){
        saved=await client.query(`UPDATE attendance_logs SET
          ...
          updated_at=NOW()
          WHERE user_id=$1 AND work_date=$2::date RETURNING *`,[userId,workDate,clockIn,clockOut]);
      }else{
        saved=await client.query(`INSERT INTO attendance_logs(user_id,work_date,clock_in,clock_out,gps_in,gps_out,device_info)
          VALUES($1,$2::date,
            ...
            '{}'::jsonb,'{}'::jsonb,'모바일 관리자 수기등록') RETURNING *`,[userId,workDate,clockIn,clockOut]);
      }
      ...
      await client.query(`INSERT INTO attendance_admin_adjustments
        ...
        VALUES($1,$2::date,$3,$4,$5,$6,$7,$8,$9)`,[
          userId,workDate,before?.clock_in||null,before?.clock_out||null,after.clock_in||null,after.clock_out||null,
          reason.slice(0,1000),String(editor.id||''),String(editor.name||'관리자').slice(0,100)
        ]);
      await client.query('COMMIT');
      return ok(res,{clockIn:after.clock_in,clockOut:after.clock_out},'근태 시간이 수정되었습니다. 수정 이력에 기록했습니다.');
    }catch(e){
      try{await client.query('ROLLBACK')}catch(_e){}
      console.error('[Mobile daily attendance] edit',e);
      return fail(res,500,'근태 시간 수정에 실패했습니다.');
    }finally{client.release();}
  });
}

const originalUse=express.application.use;
express.application.use=function namoMobileDailyAttendanceUse(...args){
  const result=originalUse.apply(this,args);
  if(!this.__namoMobileDailyAttendanceInstalled){
    const fns=args.flat().filter(v=>typeof v==='function');
    if(fns.some(fn=>fn.name==='session'||/session/i.test(String(fn.name||'')))){
      install(this);
      console.log('[Mobile daily attendance] routes installed');
    }
  }
  return result;
};

module.exports={installMobileDailyAttendance:install};
