...
const createAttendanceMailLinkTest = require('./attendance-mail-link-test.js');
...
const mailLink = createAttendanceMailLinkTest({ rootDir: __dirname, mailConfig, fetchCurrentAttendanceUser });
...
app.post('/api/attendance/test-direct-mail', async (req, res) => {
  ...
  try {
    ...
    return res.status(401).json({ success: false, code: 'LOGIN_SENDER_NOT_FOUND', message: error.message || '로그인 당사자의 회사메일을 확인할 수 없습니다.' });
  }

  const credential = mailLink.credentialFor(sender);
  if (!credential) {
    return res.status(503).json({ success: false, code: 'SMTP_SENDER_NOT_CONFIGURED', message: '최초 1회 메일 연동이 필요합니다.' });
  }
  ...
  try {
    ...
    return res.json({ success: true, data: { sent: recipients.length, messageId: info.messageId || null, sender: { id: sender.id || null, name: sender.name || '', email: sender.email } } });
  } catch (error) {
    console.error('[NAMO TEST direct mail]', error);
    const authFailed = error?.code === 'EAUTH' || Number(error?.responseCode) === 535;
    return res.status(502).json({ success: false, code: authFailed ? 'SMTP_AUTH_FAILED' : 'SMTP_SEND_FAILED', message: authFailed ? '로그인 당사자의 이카운트 메일 인증정보를 확인해 주세요.' : `메일 발송 실패: ${error.message}` });
  }
});
...
app.listen(port, '127.0.0.1', () => {
  ...
  console.log('Direct mail TEST: current login user; first-time encrypted mail linking, then one-click send.');
